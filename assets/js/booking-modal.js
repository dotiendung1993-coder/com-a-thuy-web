...
let bookingState = {
  ...
  // Task 1 (17/09/2026) — combo/tặng món tự động: xem refreshPreorderComboPreview().
  comboPreview: null, _comboSig: '',
};
...
function resetModal() {
  bookingState = { date: null, time: null, tableMode: 'any', tableNo: null, zone: null, orderItems: {}, comboPreview: null, _comboSig: '' };
  showStep(1);
...
// "<món> (<size>)" giống hệt cách /api/orders đặt tên khi enrich item, để nhất quán trên bill/bếp.
...
function buildPreorderItemList(rows) {
  ...
  rows
    ...
    .forEach(r => {
      ...
      // QA (2026-09-23): jsonb OBJECT (size_prices) không giữ thứ tự khoá lúc ghi — ưu tiên đọc
      // size_order (mảng, giữ đúng thứ tự) rồi mới rơi về Object.keys() cho dữ liệu cũ chưa có
      // size_order, cùng cách sell.js/order.html đã sửa. Task 2 (2026-08-02): bỏ size XS (trùng suất
      // Cơm phần) — chỉ còn S/M/L, khớp order.html.
      const sizeOrder = Array.isArray(r.size_order) && r.size_order.length ? r.size_order : Object.keys(sizePrices);
      const sizes = sizeOrder.filter(sz => sizePrices[sz] != null && sz.toUpperCase() !== 'XS');
...
function renderPreorderMenu() {
  ...
  menuDiv.innerHTML = `
    ...
    <div class="preorder-combo-hint" id="preorder-combo-hint" style="display:none"></div>
  `;
  // Task 1 (17/09/2026) — combo/tặng món tự động: xem refreshPreorderComboPreview().
  updateComboHint();
  const sig0 = preorderCartSignature();
  if (sig0 !== bookingState._comboSig) { bookingState._comboSig = sig0; schedulePreorderComboPreview(sig0); }
}
...
function renderPreorderCard(card) {
  ...
  const sizeRows = card.variants.map(v => {
    ...
    // QA (2026-09-23): tên mức tự do (không phải S/M/L quen thuộc) không có "serves" tra được — chỉ
    // hiện đúng tên mức, không thêm tiền tố "Size " (đọc "Size Suất lớn" sẽ lạ/thừa chữ).
    const sizeLabel = serves ? `Size ${escHtml(v.size)} <span class="preorder-size-serves">(${serves})</span>` : escHtml(v.size);
    return `
      <div class="preorder-size-chip${qty > 0 ? ' active' : ''}" data-key="${v.key}">
        <span class="preorder-size-label">${sizeLabel}</span>
...
function changePreorderQty(key, delta) {
  ...
  // Task 1 (17/09/2026) — combo/tặng món tự động: giỏ đổi thì dò lại preview server (debounce).
  const sig = preorderCartSignature();
  if (sig !== bookingState._comboSig) { bookingState._comboSig = sig; schedulePreorderComboPreview(sig); }
}
...
// ── Task 1 (17/09/2026) — combo/tặng món tự động cho phần "Gọi món ngay bây giờ" ────────────────
// Chỉ hiện gợi ý — booking chưa phải đơn hàng thật (đơn thật + giảm giá thật tính lúc chuyển booking
// thành đơn qua POS Manager/POS web order, cả 2 đã wire /api/promotions/preview + auto-apply).
// Dùng lại NGUYÊN /api/promotions/preview của pos-server.js — KHÔNG lặp lại logic khớp combo ở đây.
function preorderCartSignature() {
  return Object.values(bookingState.orderItems).map(v => `${v.id}:${v.size || ''}:${v.qty}`).sort().join('|');
}
let preorderComboTimer = null;
function schedulePreorderComboPreview(sig) {
  clearTimeout(preorderComboTimer);
  preorderComboTimer = setTimeout(() => refreshPreorderComboPreview(sig), 300);
}
async function refreshPreorderComboPreview(sig) {
  const items = Object.values(bookingState.orderItems);
  if (!items.length) { bookingState.comboPreview = null; updateComboHint(); return; }
  try {
    const res = await fetch(`${API_BASE}/api/promotions/preview`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ items: items.map(v => ({ id: v.id, size: v.size || '', qty: v.qty })) }),
    });
    const data = await res.json();
    if (preorderCartSignature() !== sig) return; // giỏ đổi tiếp trong lúc chờ — bỏ kết quả cũ
    bookingState.comboPreview = data.best || null;
    updateComboHint();
  } catch (e) { /* preview lỗi không được chặn đặt bàn */ }
}
function updateComboHint() {
  const el = document.getElementById('preorder-combo-hint');
  if (!el) return;
  const best = bookingState.comboPreview;
  if (best && best.amount > 0) {
    el.style.display = '';
    el.innerHTML = `🎁 ${escHtml(best.name)} — tiết kiệm ${formatVnd(best.amount)}`;
  } else {
