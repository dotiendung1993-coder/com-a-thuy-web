...
// Đối chiếu giá vốn NVL (2026-09-18) — báo cáo BỔ SUNG, độc lập với giaVonView ở trên.
import * as doiChieuVonView from './views/doi-chieu-von.js';
...
const ROUTES = {
  ...
  'doi-chieu-von': { title: 'Đối chiếu giá vốn NVL', requiresAuth: true, view: doiChieuVonView },
  ...
  sidebar.innerHTML = `
    ...
    <div class="sidebar-footer">
      <div class="sidebar-footer-row">
        ...
        <!-- Việc "Website v2" (03/09/2026), chốt lại 10/09/2026 — từng thử tách "Đổi giao diện"
             thành hẳn một dòng chữ riêng cho v2 (ảnh mẫu), nhưng chủ quán muốn gọn như giao diện cũ:
             luôn icon-only, cùng hàng với "Cài đặt" ở CẢ 2 giao diện — xem bug-615/t104 cho lịch sử
             chữ "Đổi giao diện". -->
...
if (searchWrap && searchResults) {
  ...
  // bug-645: cú bấm MỞ ô tìm kiếm (lúc còn nhỏ, đang đóng) cũng bị tính luôn là "bấm ra ngoài" nên
  // tự đóng ngay — nhấp nháy, không hiện ra được. Do 'focus' mở modal (đổi .app-search sang
  // position:fixed phủ kín màn hình) diễn ra GIỮA mousedown và mouseup/click của CHÍNH cú bấm đó,
  // nên toạ độ chuột lúc mouseup/click rơi trúng nền mờ vừa hiện ra (che hết ô nhỏ ban đầu) thay vì
  // ô nhập → e.target === searchWrap ngay trên cú bấm mở. Ghi nhận modal đã mở hay chưa NGAY LÚC
  // mousedown (trước khi 'focus' kịp đổi layout) — chỉ đóng nếu modal đã mở SẴN từ trước cú bấm này.
  let searchOpenBeforeThisClick = false;
  searchWrap.addEventListener('mousedown', () => {
    searchOpenBeforeThisClick = !searchResults.classList.contains('hidden');
  });
  searchWrap.addEventListener('click', (e) => {
    if (e.target === searchWrap && searchOpenBeforeThisClick) closeSearchModal();
  });
}
