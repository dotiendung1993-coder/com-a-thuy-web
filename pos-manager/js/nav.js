...
export const FEATURES = [
  ...
  // Đối chiếu giá vốn NVL (2026-09-18) — báo cáo bổ sung, độc lập với 'gia-von' ở trên.
  { route: 'doi-chieu-von',     label: 'Đối chiếu vốn NVL', perm: 'report',       group: 'Báo cáo' },
...
export const GROUP_ORDER = [
  ...
  // Việc "Website v2" (03/09/2026) — ảnh mẫu xếp "Nhân viên" (= nhóm 'Quản lý' của app này) NGAY
  // SAU Đối tác và TRƯỚC Thuế, còn Báo cáo đứng cuối khối QUẢN LÝ. Chỉ đổi THỨ TỰ HIỆN, không
  // đụng tên nhóm/quyền.
  'Quản lý',
  'Thuế',
  'Quản lý hoá đơn', 'Báo cáo',
];
...
// Việc header (23/08/2026) — bản NHIỀU kết quả cho khung thả xuống của ô tìm kiếm (giống SoBanHang:
// gõ tới đâu hiện danh sách khớp tới đó, không phải chỉ nhảy vào mục đầu tiên khi bấm Enter).
// Cùng luật so khớp với matchFeatureLabel ở trên, giới hạn số dòng hiện ra cho gọn màn hình.
export function matchFeatureLabels(query, features = FEATURES, limit = 8) {
  const q = String(query || '').trim().toLowerCase();
  if (!q) return [];
  return features.filter((f) => f.label.toLowerCase().includes(q)).slice(0, limit);
}
...
// Việc "Website v2" (03/09/2026) — SỬA kết luận sai của Đợt 2 (15/08): nhóm "Bán hàng" KHÔNG có
// nhãn cấp cao. Đối chiếu lại toàn bộ ảnh mẫu (Website v2\Tổng quan\*, Website v2\Bán hàng\Đơn
// Hàng\*) thì cột trái chỉ có ĐÚNG 2 nhãn cấp cao: "TỔNG QUAN" (trên mục Tổng quan) và "QUẢN LÝ"
// (trên Tài chính). Ngay trên "Bán hàng" không có nhãn nào — trước đây hiện "BÁN HÀNG" rồi ngay
// dưới lại là nút nhóm "BÁN HÀNG" nữa, lặp chữ.
...
const GROUP_LABEL_V2 = {
  'Sổ tiền': 'Tài chính',
  // Việc "Website v2" (03/09/2026) — ảnh mẫu ghi "Quản lý kho" và "Nhân viên", không phải "Kho"
  // và "Quản lý". Chỉ đổi CHỮ HIỂN THỊ ở sidebar/breadcrumb; khoá nhóm gốc ('Kho', 'Quản lý')
  // giữ nguyên nên GROUP_ORDER, quyền và localStorage posmgr.sidebar.cg không đổi.
  Kho: 'Quản lý kho',
  'Quản lý': 'Nhân viên',
};
