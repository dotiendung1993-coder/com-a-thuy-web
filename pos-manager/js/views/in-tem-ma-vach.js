...
import { escapeHtml, formatVND, toast, parseNumberInput, attachNumberInput } from '../ui.js';
...
export async function render(container, { staff } = {}) {
  ...
  function renderPrintItems() {
      ...
      <tbody>
        ...
        <tr style="border-bottom:1px solid var(--border,#f5f5f5)">
          ...
          <td style="padding:6px">
            <input type="text" inputmode="numeric" value="${item.copies}" data-idx="${i}"
    ...
    el.querySelectorAll('.itm-copies').forEach((inp) => {
      attachNumberInput(inp, { decimals: 0 });
      inp.addEventListener('change', () => {
        printItems[+inp.dataset.idx].copies = Math.min(99, Math.max(1, parseNumberInput(inp.value) || 1));
      });
    });
