...
export async function render(container, { staff } = {}) {
  ...
  container.innerHTML = `
    <div class="page-head">
      ...
      <button id="itm-pdf" class="btn btn-primary">
        <span style="margin-right:4px"></span> Xuất file PDF
      </button>
    </div>
  ...
  function renderPrintItems() {
    ...
    if (!printItems.length) {
      el.innerHTML = `<div style="text-align:center;padding:24px;color:#999">
        ...
        Chưa có sản phẩm nào để in tem<br>
        <small>Tìm theo mã SKU hoặc tên sản phẩm ở trên để thêm vào danh sách in</small>
      </div>`;
      return;
    }
