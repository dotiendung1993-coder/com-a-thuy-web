...
import { escapeHtml, toast, confirmDialog, promptDialog, formatNumberInput, parseNumberInput, attachNumberInput } from '../ui.js';
...
export async function render(container, { staff } = {}) {
  ...
  function renderDetailScreen() {
    ...
    return `<div class="stock-table-row">
            ...
            <span>${editable
    ? `<input data-menu="${it.menu_id}" type="text" inputmode="decimal" value="${formatNumberInput(counted, { decimals: 3 })}" />`
    : counted}</span>
    ...
    container.querySelectorAll('[data-menu]').forEach((input) => {
      ...
      input.addEventListener('input', () => {
        pending.set(input.dataset.menu, Math.max(0, parseNumberInput(input.value) || 0));
        renderPendingNote();
      });
    });
    renderPendingNote();
