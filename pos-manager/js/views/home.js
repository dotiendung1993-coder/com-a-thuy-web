...
function openGoalPicker(current, onSaved) {
  const { overlay, close } = openModal(`
    ...
    <input id="goal-input" type="text" inputmode="numeric" value="${formatNumberInput(Number(current) || 0)}">
    <div class="modal-close-row">
      <button class="btn" id="goal-cancel">Huỷ</button>
      <button class="btn btn-primary" id="goal-save">Lưu</button>
    </div>
  `);
  attachNumberInput(overlay.querySelector('#goal-input'), { decimals: 0 });
  ...
  overlay.querySelector('#goal-save').addEventListener('click', async () => {
    const val = Math.max(0, Math.round(parseNumberInput(overlay.querySelector('#goal-input').value) || 0));
    try {
