...
function reviewCardHtml(review) {
  const rows = (review || []).map((r) => `
    <a class="review-row" href="#/${r.route}">
      <span class="review-ico">${icon(REVIEW_ICON[r.key] || 'canh-bao')}</span>
      <span class="review-label">${escapeHtml(r.label)}</span>
      <span class="review-count ${r.count > 0 ? 'has' : ''}">${r.count}</span>
...
function quickActionsHtml() {
  return `
    <div class="card quick-card dash-quick">
      ...
      <div class="qa-grid">
        <a class="qa-tile" href="#/ban-hang">${icon('them')}<span>Tạo đơn hàng</span></a>
        <a class="qa-tile" href="#/quan-ly-ban">${icon('quan-ly-ban')}<span>Tạo đơn tại bàn</span></a>
        <a class="qa-tile" href="#/san-pham">${icon('san-pham')}<span>Tạo sản phẩm mới</span></a>
        <button type="button" class="qa-tile" id="qa-refresh">${icon('chuyen-tien')}<span>Làm mới dữ liệu</span></button>
        <a class="qa-tile" href="#/nhap-hang">${icon('tai-xuong')}<span>Nhập kho</span></a>
        <a class="qa-tile" href="#/kiem-kho">${icon('kiem-kho')}<span>Kiểm kho</span></a>
        <button type="button" class="qa-tile" id="qa-thu">${icon('thu-chi')}<span>Tạo khoản thu</span></button>
        <button type="button" class="qa-tile" id="qa-chi">${icon('thu-chi')}<span>Tạo khoản chi</span></button>
        <a class="qa-tile" href="#/so-no">${icon('so-no')}<span>Tạo nợ khách hàng</span></a>
      </div>
    </div>`;
}
    ...
    function paintKpi() {
      ...
      kpiRow.innerHTML = `
        ...
        <div class="kpi kpi-cash">
          <div class="kpi-head"><div class="kpi-label">SỐ DƯ HÔM NAY
              ${accounts.length > 1
