...
export async function render(container, { staff } = {}) {
  ...
  container.innerHTML = `
    ...
    <div class="card" style="margin:12px 0">
      <label style="display:flex;align-items:center;gap:8px;flex-wrap:wrap">
        ...
        <input id="dcv-target" type="text" inputmode="numeric" placeholder="vd 20.000"
          value="${formatNumberInput(targetProfit ?? '')}" style="width:140px" /> đồng
      </label>
    </div>
  ...
  attachNumberInput(container.querySelector('#dcv-target'), { decimals: 0 });
  ...
  function compareCell(costPerDish, avgPrice) {
    ...
    return diff >= 0
      ? `<span class="rp-positive"><span class="inline-ico">${icon('ok')}</span> Đạt (dư ${formatVND(diff)})</span>`
      : `<span class="rp-negative"><span class="inline-ico">${icon('dong')}</span> Vượt ${formatVND(-diff)}</span>`;
  }
