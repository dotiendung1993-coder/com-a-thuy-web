...
import { formatVND, escapeHtml, toast, openModal, confirmDialog, formatNumberInput, parseNumberInput, attachNumberInput } from '../ui.js';
  ...
  function openModal_(item) {
    ...
    const modal = openModal(`
      ...
      <div class="field"><label>Giá vốn</label>
        <input id="nvl-m-cost" type="text" inputmode="numeric" value="${formatNumberInput(item?.cost_price ?? 0)}" /></div>
      <div class="field"><label>Tồn kho</label>
        <input id="nvl-m-stock" type="text" inputmode="decimal" value="${formatNumberInput(item?.on_hand ?? 0, { decimals: 3 })}" ${!isNew ? 'readonly style="background:var(--input-disabled-bg,#f5f5f5)"' : ''} />
        ${!isNew ? '<p class="hint" style="margin:4px 0 0">Điều chỉnh tồn qua màn "Nhập / Xuất NVL"</p>' : ''}
      </div>
      <div class="field">
        <label>Cảnh báo tồn thấp</label>
        <input id="nvl-m-min" type="text" inputmode="decimal" value="${formatNumberInput(item?.min_qty ?? '', { decimals: 3 })}" placeholder="Nhập tồn kho để đặt cảnh báo" />
      </div>
    ...
    attachNumberInput(ov.querySelector('#nvl-m-cost'), { decimals: 0 });
    attachNumberInput(ov.querySelector('#nvl-m-stock'), { decimals: 3 });
    attachNumberInput(ov.querySelector('#nvl-m-min'), { decimals: 3 });
    ...
    ov.querySelector('#nvl-m-save').addEventListener('click', async () => {
      ...
      const body = {
        name,
        unit,
        cost_price: parseNumberInput(ov.querySelector('#nvl-m-cost').value) || 0,
        min_qty: parseNumberInput(ov.querySelector('#nvl-m-min').value) || 0,
      };
      if (isNew) {
        const initStock = parseNumberInput(ov.querySelector('#nvl-m-stock').value) || 0;
