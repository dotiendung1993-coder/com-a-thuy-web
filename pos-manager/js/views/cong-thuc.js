...
import { escapeHtml, formatVND, toast, openModal, formatNumberInput, parseNumberInput, attachNumberInput } from '../ui.js';
import { icon } from '../icons.js';
import { ingredientOptionsHtml, recipeSummaryHtml, resolveIngredientPick, validateRecipeRows } from '../recipe-shared.js';
...
export async function render(container, { staff } = {}) {
  ...
  async function openCreateModal() {
    ...
    // Chưa có NVL nào cũng mở được: tạo NVL ngay trong lúc soạn (chọn "+ Tạo nguyên vật liệu mới…" ở ô chọn).
    ...
    function rebuildModal() {
        ...
        <div style="margin-top:16px">
          ...
          ${rows.length
            ? (() => {
                    ...
                    ${rows.map((r, i) => {
                      ...
                      return `<tr>
                        <td><select class="ct-row-ing" data-i="${i}" style="width:100%;border:none;background:transparent">
                          ${ingredientOptionsHtml(allIngredients, r.ingredient_id)}
                        </select></td>
                        <td><input class="ct-row-qty" data-i="${i}" type="text" inputmode="decimal" value="${formatNumberInput(r.qty_per_dish, { decimals: 3 })}" style="width:100%;text-align:right;border:none;background:transparent" /></td>
                ...
                ${recipeSummaryHtml(rows, allIngredients, { label: 'Giá vốn/sản phẩm' })}`;
              })()
      ...
      if (spSearch) {
        spSearch.focus();
        // rebuildModal() thay toàn bộ innerHTML mỗi lần gõ -> input cũ bị huỷ, input mới dựng lại
        // luôn đặt con trỏ về đầu chuỗi (0) khi focus() dù đã có value sẵn -> gõ ký tự tiếp theo bị
        // chèn NGƯỢC vào đầu, gõ nhanh/gõ dài ra chuỗi đảo ngược hoàn toàn (bug tìm thấy 2026-09-18
        // khi test tìm sản phẩm cho công thức). Ép con trỏ về CUỐI chuỗi sau khi focus.
        spSearch.setSelectionRange(spSearch.value.length, spSearch.value.length);
      ...
      if (addNvl) addNvl.addEventListener('click', () => {
        rows.push({ ingredient_id: null, qty_per_dish: 0 });
        rebuildModal();
      });

      box.querySelectorAll('.ct-row-ing').forEach((sel) => {
        sel.addEventListener('change', async (e) => {
          const row = rows[+e.target.dataset.i];
          // "+ Tạo nguyên vật liệu mới…": tạo xong dòng này chọn sẵn NVL vừa tạo; công thức đang nhập nằm
          // trong `rows` nên dựng lại giao diện không làm mất gì.
          const pick = await resolveIngredientPick(e.target.value, row.ingredient_id, allIngredients);
          if (pick.items) allIngredients = pick.items;
          row.ingredient_id = pick.id;
          rebuildModal();
        });
      });
      box.querySelectorAll('.ct-row-qty').forEach((inp) => {
        attachNumberInput(inp, { decimals: 3 });
        inp.addEventListener('input', (e) => { rows[+e.target.dataset.i].qty_per_dish = parseNumberInput(e.target.value) || 0; });
      });
      ...
      box.querySelector('#ct-save').addEventListener('click', async () => {
        ...
        const invalid = validateRecipeRows(rows, allIngredients);
        if (invalid) { toast(invalid, 'error'); return; }
        try {
          const saved = await api.put(`/api/mgr/ingredients/recipes/menu/${selectedMenuId}`, {
            ingredients: rows.map((r) => ({ ingredient_id: r.ingredient_id, qty_per_dish: r.qty_per_dish, note: null })),
          });
          toast(rows.length ? `Đã tạo công thức. Giá vốn món tự cập nhật: ${formatVND(saved.menu_cost_price)}` : 'Đã tạo công thức');
          modal.close();
          await load();
  ...
  async function openEditModal(menuId) {
    ...
    try {
      ...
      recipeData = rd;
      allIngredients = id.items || []; // chưa có NVL nào vẫn mở được: tạo ngay trong lúc soạn
    ...
    function buildRows() {
      return rows.map((r, i) => {
        ...
        return `<tr>
          ...
          <td><input class="ct-row-qty" data-i="${i}" type="text" inputmode="decimal" value="${formatNumberInput(r.qty_per_dish, { decimals: 3 })}" style="width:80px" /></td>
    ...
    div.innerHTML = `
      <div class="modal" style="background:var(--card-bg,#fff);border-radius:12px;padding:20px;width:100%;max-width:640px;margin:auto">
        ...
        <div id="ct-sum"></div>
        <p class="hint" style="margin:8px 0 0">Giá vốn món tự cập nhật theo công thức này. Thiếu nguyên vật liệu? Chọn "+ Tạo nguyên vật liệu mới…" ở ô chọn của dòng.</p>
    ...
    const refreshSum = () => { div.querySelector('#ct-sum').innerHTML = recipeSummaryHtml(rows, allIngredients); };
    function rebind() {
      div.querySelector('#ct-tbody').innerHTML = buildRows();
      div.querySelectorAll('.ct-row-ing').forEach((sel) => {
        sel.addEventListener('change', async (e) => {
          const row = rows[+e.target.dataset.i];
          // "+ Tạo nguyên vật liệu mới…": tạo xong dòng này chọn sẵn NVL vừa tạo, các dòng khác giữ nguyên.
          const pick = await resolveIngredientPick(e.target.value, row.ingredient_id, allIngredients);
          if (pick.items) allIngredients = pick.items;
          row.ingredient_id = pick.id;
          rebind();
      ...
      div.querySelectorAll('.ct-row-qty').forEach((inp) => {
        attachNumberInput(inp, { decimals: 3 });
        inp.addEventListener('input', (e) => { rows[+e.target.dataset.i].qty_per_dish = parseNumberInput(e.target.value) || 0; refreshSum(); });
      });
    ...
    div.querySelector('#ct-add-row').addEventListener('click', () => {
      rows.push({ ingredient_id: null, qty_per_dish: 0, note: '' });
      rebind();
    });
    div.querySelector('#ct-cancel').addEventListener('click', close);
    div.querySelector('#ct-save').addEventListener('click', async () => {
      div.querySelectorAll('.ct-row-ing').forEach((sel) => { rows[+sel.dataset.i].ingredient_id = Number(sel.value) || null; });
      div.querySelectorAll('.ct-row-qty').forEach((inp) => { rows[+inp.dataset.i].qty_per_dish = parseNumberInput(inp.value) || 0; });
      div.querySelectorAll('.ct-row-note').forEach((inp) => { rows[+inp.dataset.i].note = inp.value; });
      const invalid = validateRecipeRows(rows, allIngredients);
      if (invalid) { toast(invalid, 'error'); return; }
      try {
        const saved = await api.put(`/api/mgr/ingredients/recipes/menu/${menuId}`, {
          ingredients: rows.map((r) => ({ ingredient_id: r.ingredient_id, qty_per_dish: r.qty_per_dish, note: r.note || null })),
        });
        toast(rows.length
          ? `Đã lưu công thức. Giá vốn món tự cập nhật: ${formatVND(saved.menu_cost_price)}`
          : 'Đã bỏ công thức. Giá vốn giữ nguyên số cũ, nhập tay ở màn Giá vốn');
        close();
        await load();
      } catch (err) { toast(err?.body?.message || 'Lỗi khi lưu', 'error'); }
    });
