...
export async function render(container, { staff } = {}) {
    ...
    <div class="hd-tools">
      ...
      <div class="ord-dd sq-multiselect" id="sq-direction-dd">
        <button type="button" class="btn ord-dd-btn" id="sq-direction-btn" aria-haspopup="listbox" aria-expanded="false">
          <span class="ord-dd-label" id="sq-direction-label">Thu &amp; Chi</span>
      ...
      <div class="ord-cols-wrap" id="sq-cols-wrap">
        <button id="sq-cols-btn" class="btn ord-cols-btn" type="button" aria-haspopup="true" aria-expanded="false">Hiện thị cột ▾</button>
        <div id="sq-cols-pop" class="ord-cols-pop hidden" role="dialog" aria-label="Chọn cột hiển thị"></div>
      </div>
    </div>
    <div class="hd-stats" id="sq-stats"></div>
  ...
  container.querySelector('#sq-clear-filter').addEventListener('click', () => {
    ...
    container.querySelector('#sq-direction-label').textContent = 'Thu & Chi';
  ...
  function renderStats() {
    ...
    el.innerHTML = [
      ['Số dư đầu kỳ', formatVND(opening)],
      ['Tổng thu', formatVND(thu)],
      ['Tổng chi', formatVND(chi)],
      ['Số dư cuối kỳ', formatVND(closing)],
    ].map(([label, value]) => `
      <div class="hd-stat">
  ...
  function renderLedger() {
    ...
    el.innerHTML = `
          ...
          <tbody>
            ${transactions.map((t) => `
              <tr class="${t.voided_at ? 'hd-row-voided' : ''}">
                ...
                ${canManage ? `<td class="hd-act-col">${!t.voided_at ? `<button type="button" class="ord-kebab" data-menu="${t.id}" aria-haspopup="menu" aria-expanded="false" aria-label="Thao tác với phiếu ${escapeHtml(t.code)}">${icon('them')}</button>` : ''}</td>` : ''}
              </tr>`).join('')}
          </tbody>
          <tfoot>
      ...
      <p class="hint">Hiện ${transactions.length} phiếu.</p>
    `;
    if (canManage) {
  ...
  function renderPagination() {
    ...
    el.innerHTML = `
      <span class="sq-page-info">Hiển thị ${from}–${to} / ${total} kết quả</span>
      <select id="sq-page-size" aria-label="Số dòng mỗi trang">
        ${[30, 50, 100].map((n) => `<option value="${n}" ${n === pageSize ? 'selected' : ''}>${n}/trang</option>`).join('')}
      </select>
      <button type="button" class="btn btn-ghost" id="sq-page-prev" ${page <= 1 ? 'disabled' : ''}>‹</button>
      <span class="sq-page-cur">${page}/${totalPages}</span>
      <button type="button" class="btn btn-ghost" id="sq-page-next" ${page >= totalPages ? 'disabled' : ''}>›</button>
    `;
