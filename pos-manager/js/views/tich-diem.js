...
import { escapeHtml, toast, openModal, pageTabsHtml, parseNumberInput, attachNumberInput } from '../ui.js';
...
export async function render(container, { staff } = {}) {
  ...
  // API /customers/loyalty trả { entries, summary } (không phải rows/total — xem customer-service.js::listLoyalty).
  let data = { entries: [], summary: null };
  ...
  if (canManage) {
    ...
    container.querySelector('#td-sync').addEventListener('click', async () => {
      try {
        ...
        // Server trả { scanned, awarded, skipped } — hoặc { disabled } / { misconfigured } khi tích điểm đang tắt/sai cấu hình.
        if (r.disabled) toast('Tích điểm đang tắt trong Cài đặt', 'error');
        else if (r.misconfigured) toast('Cấu hình tích điểm chưa hợp lệ (số tiền mỗi điểm)', 'error');
        else toast(`Đồng bộ xong — đã tích ${r.awarded} đơn`);
        await load();
  ...
  function openAddForm() {
    const modal = openModal(`
      ...
      <div class="field"><label>Số điểm cộng</label>
        <input id="td-pts" type="text" inputmode="numeric" value="1" /></div>
    ...
    `);
    attachNumberInput(modal.overlay.querySelector('#td-pts'), { decimals: 0 });
    modal.overlay.querySelector('#td-do-add').addEventListener('click', async () => {
      ...
      const points = parseNumberInput(modal.overlay.querySelector('#td-pts').value);
      ...
      try {
        ...
        toast(`Đã cộng ${points} điểm (còn ${r.points} điểm)`);
        modal.close();
        await load();
  ...
  function openRedeemForm() {
    const modal = openModal(`
      ...
      <div class="field"><label>Số điểm đổi</label>
        <input id="td-rpts" type="text" inputmode="numeric" value="1" /></div>
      <div class="field"><label>Ghi chú (tuỳ chọn)</label>
    ...
    `);
    attachNumberInput(modal.overlay.querySelector('#td-rpts'), { decimals: 0 });
    modal.overlay.querySelector('#td-do-redeem').addEventListener('click', async () => {
      ...
      const points = parseNumberInput(modal.overlay.querySelector('#td-rpts').value);
      ...
      try {
        ...
        toast(`Đã đổi ${points} điểm (còn ${r.points} điểm)`);
        modal.close();
        await load();
  ...
  async function load() {
    // Tên tham số theo server: pageSize / customerId (limit / customer_id bị bỏ qua → luôn ra 50 dòng, không lọc được khách).
    const params = new URLSearchParams({ pageSize: '200' });
    if (state.customer_id) params.set('customerId', state.customer_id);
    ...
    try {
      ...
      console.error('[tich-diem] load lỗi:', err);
