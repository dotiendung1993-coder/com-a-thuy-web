...
export function openPaymentModal(order, fees, { onDone } = {}) {
  ...
  function renderCash() {
    detail.innerHTML = `
      ...
      <input id="cash-given" type="text" inputmode="numeric" value="${formatNumberInput(order.total)}" />
    ...
    const recalc = () => {
      const given = parseNumberInput(input.value) || 0;
    ...
    detail.querySelector('#cash-confirm').addEventListener('click', () => {
      submitPay(
        { method: 'tien-mat', cash_given: parseNumberInput(input.value) || 0 },
