    ...
    function renderLines() {
      ...
      box.innerHTML = `<div class="stock-table">
        ...
        ${lines.map((l, i) => `<div class="stock-table-row" data-i="${i}">
          ...
          <span><input data-f="qty" data-i="${i}" type="text" inputmode="decimal" value="${formatNumberInput(l.qty, { decimals: 3 })}" style="width:80px" /> <span class="hint">/ ${l.on_hand}</span></span>
      ...
      box.querySelectorAll('[data-f]').forEach((input) => {
        ...
        input.addEventListener('input', () => {
          ...
          lines[i].qty = Math.max(0, parseNumberInput(input.value) || 0);
          renderLines();
        });
      });
