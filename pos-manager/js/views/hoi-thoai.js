...
export async function render(container) {
  ...
  // Task 2 (19/09/2026) — Chat Center giờ dùng CHUNG phiên đăng nhập POS Manager: không còn màn
  // nhập PIN riêng ở đây nữa. Đã đăng nhập POS Manager rồi thì vào thẳng, tự lấy token phiên hiện
  // có (route mới /api/mgr/auth/chat-center-token, JS không đọc được cookie httpOnly nên phải qua
  // API) rồi nhúng vào URL iframe. "Quên mật khẩu"/xem lại PIN nhân viên đều là màn có sẵn của POS
  // Manager (đăng nhập lại / Cài đặt > Nhân viên) — không có màn nào tương ứng cần dựng ở đây.
  async function renderChatPane(isRetry) {
    if (_activeIframeHandler) {
      window.removeEventListener('message', _activeIframeHandler);
      _activeIframeHandler = null;
    }
    ...
    let token;
    try {
      ({ token } = await api.get('/api/mgr/auth/chat-center-token'));
    } catch (err) {
      pane.innerHTML = `<p style="padding:24px;color:#ef4444">Không lấy được phiên đăng nhập Chat Center. Thử tải lại trang.</p>`;
      return;
    }

    const src = base + '/admin/chat?t=' + encodeURIComponent(token);
    pane.innerHTML = `
      <div style="display:flex;flex-direction:column;height:calc(100dvh - var(--header-h,56px) - var(--tabbar-h,56px) - 100px);min-height:500px">
    ...
    _activeIframeHandler = (e) => {
      if (e.data?.type === 'cc-auth-ok') {
        ...
        window.removeEventListener('message', _activeIframeHandler);
        _activeIframeHandler = null;
        // Token có thể vừa hết hạn đúng lúc mở — thử lấy token mới đúng 1 lần trước khi báo lỗi
        // hẳn (tránh vòng lặp vô hạn nếu POS Manager cũng đã hết phiên).
        if (!isRetry) { renderChatPane(true); return; }
        pane.innerHTML = `<p style="padding:24px;color:#ef4444">Không đăng nhập được Chat Center. Thử tải lại trang hoặc đăng nhập lại POS Manager.</p>`;
      }
    };
