...
import { formatVND, escapeHtml, toast, openModal, alertDialog, parseNumberInput, attachNumberInput } from '../ui.js';
...
export async function render(container, { staff } = {}) {
  ...
  function openSupplierModal(sup) {
    ...
    const modal = openModal(`
      ...
      <div id="ncc-debt-fields" class="hidden">
        <div class="field"><label>Số tiền quán còn nợ nhà cung cấp</label>
          <input id="ncc-debt-amount" type="text" inputmode="numeric" value="0" /></div>
    ...
    if (debtToggle) attachNumberInput(ov.querySelector('#ncc-debt-amount'), { decimals: 0 });
    ...
    ov.querySelector('#ncc-save').addEventListener('click', async () => {
      ...
      if (isNew && debtToggle.checked) {
        const amount = Math.max(0, parseNumberInput(ov.querySelector('#ncc-debt-amount').value) || 0);
        if (amount > 0) {
