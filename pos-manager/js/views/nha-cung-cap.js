...
export async function render(container, { staff } = {}) {
  ...
  container.innerHTML = `
    ...
    <div id="ncc-kpi" style="display:grid;grid-template-columns:repeat(auto-fit,minmax(200px,1fr));gap:12px;margin-bottom:16px"></div>

    <div class="filter-row">
      <input id="ncc-q" type="search" placeholder="Tìm kiếm nhà cung cấp" />
    </div>

    <div style="overflow-x:auto">
      <table class="sp-table" id="ncc-table" style="width:100%;min-width:560px">
        <thead><tr>
          <th>NHÀ CUNG CẤP</th>
          <th>EMAIL</th>
  ...
  function renderKpi() {
    ...
    container.querySelector('#ncc-kpi').innerHTML = `
      <div class="kpi-card">
        <div style="font-size:12px;color:var(--text-2);margin-bottom:6px">Tổng phiếu chờ xác nhận</div>
        <div class="kpi-val">${formatVND(k.pending_amount)}</div>
        <div class="kpi-label">${k.pending_count} phiếu</div>
      </div>
      <div class="kpi-card">
        <div style="font-size:12px;color:var(--text-2);margin-bottom:6px">Tổng phiếu hoàn thành</div>
        <div class="kpi-val">${formatVND(k.completed_amount)}</div>
        <div class="kpi-label">${k.completed_count} phiếu</div>
      </div>
      <div class="kpi-card">
        <div style="font-size:12px;color:var(--text-2);margin-bottom:6px">Nợ phải thu</div>
        <div class="kpi-val" style="color:var(--primary,#16a34a)">${formatVND(k.phai_thu)}</div>
      </div>
      <div class="kpi-card">
        <div style="font-size:12px;color:var(--text-2);margin-bottom:6px">Nợ phải trả</div>
        <div class="kpi-val" style="color:#c00">${formatVND(k.phai_tra)}</div>
      </div>`;
  }
  ...
  function renderList() {
    ...
    tbody.innerHTML = paged.map((s) => `
      <tr data-sid="${s.id}" class="${s.active === false ? 'row-inactive' : ''}">
        ...
        ${canManage ? `<td><button class="ord-kebab" data-menu="${s.id}" aria-label="Thao tác">⋯</button></td>` : ''}
      </tr>`).join('');
