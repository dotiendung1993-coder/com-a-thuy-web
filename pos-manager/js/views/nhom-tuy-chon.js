  ...
  function openForm(g) {
    ...
    function renderOptions() {
        ...
        ? `<table class="sp-table ntc-opt-table" style="width:100%;margin-bottom:4px">
            ...
            <tbody>
              ...
              <tr>
                ...
                <td><input data-opt="price" data-i="${i}" type="text" inputmode="numeric" value="${formatNumberInput(o.price)}" style="width:100%;text-align:right;border:none;padding:2px 0;background:transparent" /></td>
                <td><input data-opt="cost_price" data-i="${i}" type="text" inputmode="numeric" value="${formatNumberInput(o.cost_price)}" style="width:100%;text-align:right;border:none;padding:2px 0;background:transparent" /></td>
                <td>
      ...
      el.querySelectorAll('[data-opt]').forEach((input) => {
        ...
        if (field === 'price' || field === 'cost_price') attachNumberInput(input, { decimals: 0 });
        const ev = input.tagName === 'SELECT' ? 'change' : 'input';
        input.addEventListener(ev, () => {
          if (field === 'price' || field === 'cost_price') state.options[i][field] = parseNumberInput(input.value) || 0;
