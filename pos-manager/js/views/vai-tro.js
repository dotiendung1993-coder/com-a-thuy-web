  ...
  function renderList() {
    ...
    container.innerHTML = `
      ...
      <div class="filter-row">
        <input id="vt-q" type="search" placeholder="Tìm vai trò…" value="${escapeHtml(state.q)}" />
      </div>
      <div style="overflow-x:auto">
        <table class="sp-table" style="width:100%;min-width:480px">
            ...
            ${!pageItems.length
              ...
              <tr>
                ...
                <td style="text-align:right">
                  ${r.builtin || !isOwner ? '' : `<button class="ord-kebab" data-menu="${r.id}" aria-haspopup="menu" aria-expanded="false"
                    aria-label="Thao tác với vai trò ${escapeHtml(r.name)}">${icon('them')}</button>`}
                </td>
              </tr>`).join('')}
          </tbody>
        </table>
      ...
      <div style="display:flex;align-items:center;justify-content:space-between;padding:12px 0;gap:12px;flex-wrap:wrap">
        <span style="color:var(--text-2);font-size:13px">Hiển thị ${filtered.length ? start + 1 : 0}–${Math.min(start + state.pageSize, filtered.length)} / ${filtered.length} vai trò</span>
        <div style="display:flex;gap:4px;align-items:center">
          <button class="btn" type="button" id="vt-prev" ${state.page <= 1 ? 'disabled' : ''}>‹</button>
          <span style="font-size:13px;padding:0 6px">${state.page}</span>
          <button class="btn" type="button" id="vt-next" ${state.page >= totalPages ? 'disabled' : ''}>›</button>
        </div>
      </div>
      <p style="color:var(--text-sub,var(--text-2));margin:4px 0 24px;font-size:0.9em">
        Vai trò <b>Nhân viên ghi đơn</b> chỉ thấy đơn của chính mình trong màn Đơn hàng.
      </p>
    `;
    ...
    container.querySelectorAll('.ord-kebab').forEach((btn) => {
