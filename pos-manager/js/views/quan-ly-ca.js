...
import { formatVND, escapeHtml, toast, openModal, pageTabsHtml, formatNumberInput, parseNumberInput, attachNumberInput } from '../ui.js';
  ...
  function openShiftForm() {
    const modal = openModal(`
      ...
      <div class="field"><label>Tiền mặt đầu ca (VND)</label>
        <input id="qlc-cash" type="text" inputmode="numeric" value="0" /></div>
    ...
    `);
    attachNumberInput(modal.overlay.querySelector('#qlc-cash'), { decimals: 0 });
    modal.overlay.querySelector('#qlc-do-open').addEventListener('click', async () => {
      const opening_cash = parseNumberInput(modal.overlay.querySelector('#qlc-cash').value) || 0;
  ...
  function closeShiftForm(s) {
    const modal = openModal(`
      ...
      <div class="field"><label>Tiền mặt thực tế đếm được (VND)</label>
        <input id="qlc-actual" type="text" inputmode="numeric" value="${formatNumberInput(s.totals?.expected_cash ?? 0)}" /></div>
      <div class="field"><label>Ghi chú (tuỳ chọn)</label>
    ...
    `);
    attachNumberInput(modal.overlay.querySelector('#qlc-actual'), { decimals: 0 });
    modal.overlay.querySelector('#qlc-do-close').addEventListener('click', async () => {
      const actual_cash = parseNumberInput(modal.overlay.querySelector('#qlc-actual').value) || 0;
