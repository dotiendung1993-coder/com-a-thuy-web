...
import { toast, todayVN } from '../ui.js';
...
function placeOf(it) {
  if (it.delivery_type === 'tai-quan' && it.table_no) return `Bàn ${esc(it.table_no)}`;
...
function cardHtml(key, group, mode) {
  ...
  else heading = customerLabel(first);
  ...
  return `
    <div class="kds-card kds-card-${first.status}" data-card="${esc(key)}">
      <div class="kds-card-head">
        ...
        <span class="kds-dot-status"></span>
        <span class="kds-card-title">${heading}</span>
        ${statusBadge}
        ${oldestChip(mins)}
...
.kds-pane-title { font-size:1rem; font-weight:800; color:var(--text); margin:0 0 10px; }
...
.kds-check { width:17px; height:17px; flex:none; accent-color:var(--primary); cursor:pointer; }

.kds-card-body { border-top:1px solid var(--line); }
.kds-row { display:flex; align-items:flex-start; gap:8px; padding:12px 14px; border-bottom:1px solid var(--line); }
.kds-card-body .kds-row:last-child { border-bottom:none; }
.kds-qty {
  background:var(--primary-soft); color:var(--primary-dark); border-radius:6px; padding:2px 7px;
      ...
      <div id="kds-board" class="kds-board">
        <div class="kds-col" id="kds-pane-left">
          <div class="kds-col-header">
            <h3 class="kds-pane-title">Đang xử lý</h3>
            ${toolsHtml('pending')}
          </div>
