...
export function render(el, { staff } = {}) {
  // Bếp chỉ dành cho owner/manager/kitchen (PERMISSIONS.kitchen). Thiếu quyền mà vẫn dựng màn thì polling bị
  // 403 mỗi vài giây (mỗi lần kèm 1 toast đỏ) và màn hiện "Không có món chờ chế biến" như thể bếp đang rảnh.
  if (!staff?.perms?.kitchen) {
    el.innerHTML = '<p>Bạn không có quyền xem màn hình này.</p>';
    return;
  }
  container = el;
  // Khởi tạo state
