...
export async function render(container, { staff } = {}) {
  ...
  function renderList() {
    ...
    ? `<input type="text" inputmode="decimal" data-adjust="${p.id}" value="${formatNumberInput(p.on_hand, { decimals: 3 })}" style="width:90px;padding:4px 8px" />`
    ...
    tbody.querySelectorAll('[data-adjust]').forEach((input) => {
      attachNumberInput(input, { decimals: 3 });
  ...
  async function adjustStock(menuId, input) {
    ...
    if (!p) return;
    const target = parseNumberInput(input.value);
    if (target === null || !Number.isFinite(target) || target < 0) { input.value = formatNumberInput(p.on_hand, { decimals: 3 }); toast('Số lượng không hợp lệ', 'error'); return; }
    const diff = Math.round((target - p.on_hand) * 1000) / 1000;
    if (diff === 0) return;
    if (!(await confirmDialog(`Sửa tồn kho "${p.name}" từ ${p.on_hand} → ${target}?`))) { input.value = formatNumberInput(p.on_hand, { decimals: 3 }); return; }
    try {
      ...
      input.value = formatNumberInput(p.on_hand, { decimals: 3 });
  ...
  function openBarcodeDialog() {
    const modal = openModal(`
      ...
      <div class="field"><label>Số tem mỗi món</label>
        <input id="tkbc-copies" type="text" inputmode="numeric" value="1" /></div>
      <button id="tkbc-go" class="btn btn-primary" style="width:100%">Mở trang in</button>
    `);
    attachNumberInput(modal.overlay.querySelector('#tkbc-copies'), { decimals: 0 });
    modal.overlay.querySelector('#tkbc-go').addEventListener('click', () => {
      const copies = Math.min(50, Math.max(1, parseNumberInput(modal.overlay.querySelector('#tkbc-copies').value) || 1));
