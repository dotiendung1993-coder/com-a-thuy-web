...
export async function render(container, { staff } = {}) {
  ...
  container.innerHTML = `
    ...
    <div id="tk-kpi" style="display:grid;grid-template-columns:repeat(4,1fr);gap:12px;margin-bottom:16px"></div>

    <div class="filter-row">
      <input id="tk-q" type="search" placeholder="Tìm kiếm mã SKU/ Tên SP" />
      <div style="position:relative">
      ...
      <div style="position:relative;margin-left:auto">
        <button id="tk-col-btn" class="btn" style="font-size:13px">Hiển thị cột ▾</button>
    ...
    <div style="overflow-x:auto">
      <table class="sp-table" id="tk-table" style="width:100%;min-width:480px">
  ...
  function renderKpi() {
    ...
    container.querySelector('#tk-kpi').innerHTML = `
      <div class="kpi-card"><div class="kpi-val">${data.items.reduce((sum, p) => sum + (p.on_hand || 0), 0)}</div><div class="kpi-label">Tổng số lượng</div></div>
      <div class="kpi-card"><div class="kpi-val" style="color:var(--primary,#16a34a)">${formatVND(s.total_value || 0)}</div><div class="kpi-label">Giá trị kho</div></div>
      <div class="kpi-card"><div class="kpi-val" style="color:#c00">${data.items.filter((p) => (p.on_hand || 0) <= 0).length}</div><div class="kpi-label">Số hàng hết</div></div>
      <div class="kpi-card"><div class="kpi-val">${neverMoved}</div><div class="kpi-label">Chưa mở tồn</div></div>
  ...
  function renderList() {
    ...
    : `<span style="color:${p.low_stock ? '#c00' : 'inherit'}">${p.on_hand}${p.unit ? ' ' + escapeHtml(p.unit) : ''}</span>`}</td>` : ''}
        ...
        <td><button class="ord-kebab" data-menu="${p.id}" aria-label="Thao tác">⋯</button></td>
      </tr>`).join('');
