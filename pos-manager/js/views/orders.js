...
import { formatVND, escapeHtml, toast, openModal, confirmDialog, tableName, formatNumberInput, parseNumberInput, attachNumberInput } from '../ui.js';
...
export async function render(container, { staff } = {}) {
  ...
  async function loadStaffFilter() {
    // GET /api/mgr/staff chỉ cho CHỦ QUÁN (cả quản lý cũng bị 403). Gọi rồi để 403 rơi vào catch không đủ
    // "không báo lỗi": api.js tự hiện toast đỏ "Bạn không có quyền…" cho mọi 403 → thu ngân mở Đơn hàng
    // lần nào cũng thấy thông báo lỗi. Biết trước không có quyền thì đừng gọi.
    if (staff?.role !== 'owner') {
  ...
  async function openReturnModal(code, onDone) {
    ...
    const modal = openModal(`
      ...
      <table class="ret-table"><thead><tr>
        ...
        ${rows.map((it) => `<tr data-index="${it.index}">
          ...
          <td class="num ret-qty-cell">
            ...
            <input type="text" class="ret-qty" value="0" min="0" max="${it.remain_qty}" inputmode="numeric" />
      ...
      <div class="field"><label for="ret-fee">Phí hoàn trả (quán giữ lại)</label>
        <input id="ret-fee" type="text" min="0" value="0" inputmode="numeric" /></div>
      <div class="field"><label for="ret-extra">Trả thêm cho khách (đền bù)</label>
        <input id="ret-extra" type="text" min="0" value="0" inputmode="numeric" /></div>
    ...
    function picked() {
      return [...modal.overlay.querySelectorAll('.ret-table tbody tr')].map((tr) => ({
        ...
        qty: Math.max(0, Math.min(parseNumberInput(tr.querySelector('.ret-qty').value) || 0,
    ...
    function recalc() {
      ...
      const fee = Math.max(0, parseNumberInput($('#ret-fee').value) || 0);
      const extra = Math.max(0, parseNumberInput($('#ret-extra').value) || 0);
    ...
    modal.overlay.querySelectorAll('.ret-table tbody tr').forEach((tr) => {
      ...
      attachNumberInput(qtyEl, { decimals: 0 });
      const setQty = (v) => {
        qtyEl.value = formatNumberInput(Math.max(0, Math.min(max, v)));
        pick.checked = (parseNumberInput(qtyEl.value) || 0) > 0;
        recalc();
      };
      tr.querySelector('.ret-minus').addEventListener('click', () => setQty((parseNumberInput(qtyEl.value) || 0) - 1));
      tr.querySelector('.ret-plus').addEventListener('click', () => setQty((parseNumberInput(qtyEl.value) || 0) + 1));
      qtyEl.addEventListener('input', () => setQty(parseNumberInput(qtyEl.value) || 0));
      // Tích ô vuông = trả TOÀN BỘ số còn lại của dòng đó (việc hay làm nhất), bỏ tích = 0.
      pick.addEventListener('change', () => { qtyEl.value = formatNumberInput(pick.checked ? max : 0); recalc(); });
    });
    attachNumberInput($('#ret-fee'), { decimals: 0 });
    attachNumberInput($('#ret-extra'), { decimals: 0 });
