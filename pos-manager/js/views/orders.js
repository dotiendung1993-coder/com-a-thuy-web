...
export async function render(container) {
  ...
  const ddInner = (label) => `
          ...
          <div class="ord-dd-pop hidden" role="listbox">
            <div class="ord-dd-search"><input type="search" placeholder="Tìm kiếm" aria-label="Tìm trong ${escapeHtml(label)}" /></div>
  ...
  container.innerHTML = `
    ...
    <div class="orders-filters">
      <div class="ord-filter-grid" id="ord-filter-grid">
        <input id="ord-search" class="ord-span2" type="search" placeholder="Tìm mã đơn / tên khách / SĐT" />
        <!-- Trước đây là HAI ô ngày rời. Chủ quán xin gộp thành MỘT nút, bấm vào mở 2 bảng lịch
             chọn thẳng khoảng ngày (không phải gõ tay 2 lần). -->
        ...
        <div class="ord-dd ord-span2" id="ord-pay-state">${ddInner('Trạng thái thanh toán')}
        </div>
        <div class="ord-dd ord-span2" id="ord-cash-account">${ddInner('Nguồn tiền nhận')}
        </div>
        <!-- Task 4 (10/08/2026) — bộ lọc "TT giao hàng" như ảnh chủ quán gửi. -->
        <div class="ord-dd ord-span2" id="ord-deliv-state">${ddInner('TT giao hàng')}
        </div>
        <!-- Task 3 (10/08/2026) — bộ lọc "Hẹn giao". Không dùng ô chọn-nhiều: 5 mục này loại trừ
             nhau (không thể vừa "Không hẹn lịch" vừa "Hôm nay"), và "Tuỳ chọn…" còn phải mở thêm
             2 ô ngày nên một <select> đọc rõ ràng hơn. -->
        <select id="ord-schedule" class="ord-span2" aria-label="Lọc theo hẹn giao">
          <option value="">Hẹn giao: tất cả</option>
          ${SCHEDULE_FILTERS.map(([k, v]) => `<option value="${k}">${escapeHtml(v)}</option>`).join('')}
        </select>
        <!-- Task 1 (12/08/2026) — chọn "Tuỳ chọn…" thì hiện ĐÚNG MỘT nút; bấm vào mở luôn 2 bảng
             lịch (lịch trái chọn ngày đầu, lịch phải chọn ngày cuối) y như nút "Mọi thời gian" ở
             hàng trên. Bản cũ là 2 ô <input type="date"> phải gõ/bấm 2 lần rời nhau. -->
        <div class="ord-date-wrap ord-sched-range hidden ord-span2" id="ord-sched-range">
        ...
        <div class="ord-actions">
          ...
          <button id="ord-export" class="btn btn-excel" type="button"><span class="inline-ico">${icon('tai-xuong')}</span> Xuất Excel</button>
          <div class="ord-cols-wrap" id="ord-cols-wrap">
            <button id="ord-cols-btn" class="btn ord-cols-btn" type="button" aria-haspopup="true" aria-expanded="false">Hiện thị cột ▾</button>
  ...
  function renderKpi() {
    ...
    if (!el) return;
    el.innerHTML = `
      <div class="kpi kpi-orders"><div class="kpi-val">${counts['tat-ca'] ?? 0}</div><div class="kpi-lbl">Đơn hàng</div></div>
      <div class="kpi kpi-revenue"><div class="kpi-val">${formatVND(kpis.total_revenue)}</div><div class="kpi-lbl">Doanh thu</div></div>
      <div class="kpi kpi-pending"><div class="kpi-val">${counts['cho-xac-nhan'] ?? 0}</div><div class="kpi-lbl">Chờ xác nhận</div></div>
      <div class="kpi kpi-pending-rev"><div class="kpi-val">${formatVND(kpis.pending_revenue)}</div><div class="kpi-lbl">Doanh thu chưa xử lý</div></div>
    `;
  }
    ...
    const rows = orders.map((o) => {
      ...
      return `
        <tr class="ord-row" data-code="${escapeHtml(o.order_code)}">
          ...
          <td data-col="col-deliv" class="ord-td-deliv">${escapeHtml(deliv)}${o.table_no ? ' · Bàn ' + escapeHtml(String(o.table_no)) : ''}</td>
          ...
          <td class="ord-td-total">${formatVND(o.total)}</td>
          <td class="ord-td-act">
  ...
  function ddOpen(id, open) {
    ...
    if (!root) return;
    root.querySelector('.ord-dd-pop').classList.toggle('hidden', !open);
    root.querySelector('.ord-dd-btn').setAttribute('aria-expanded', open ? 'true' : 'false');
    if (open) ddRenderList(id);
  }
