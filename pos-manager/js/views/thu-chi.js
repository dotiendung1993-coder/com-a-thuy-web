...
import { formatVND, escapeHtml, toast, openModal, todayVN, promptDialog, pageTabsHtml, formatNumberInput, parseNumberInput, attachNumberInput } from '../ui.js';
...
export function openTxnModal(initialDirection, { accounts, defaultAccountId, onSaved }) {
  ...
  async function renderForm() {
    ...
    box.innerHTML = `
      ...
      <div class="field"><label>Số tiền</label><input id="tx-amount" type="text" inputmode="numeric" value="${escapeHtml(formatNumberInput(preserved?.amount || ''))}" /></div>
    ...
    attachNumberInput(box.querySelector('#tx-amount'), { decimals: 0 });
    ...
    box.querySelector('#tx-submit').addEventListener('click', async () => {
      const amount = parseNumberInput(box.querySelector('#tx-amount').value);
...
export async function render(container, { staff } = {}) {
  ...
  container.innerHTML = `
    ...
    <div class="hd-head">
      ...
      <div class="hd-head-actions">
        <div style="position:relative" id="tc-thao-tac-wrap">
          <button type="button" class="btn" id="tc-thao-tac-btn" aria-haspopup="menu" aria-expanded="false">Thao tác <svg class="ord-dd-caret" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true"><path fill-rule="evenodd" d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" clip-rule="evenodd"/></svg></button>
          <div class="row-menu hidden" id="tc-thao-tac-menu" role="menu" style="position:absolute;top:100%;right:0;min-width:180px;z-index:200">
            <button type="button" role="menuitem" id="tc-export">Tải Excel</button>
          </div>
        </div>
    ...
    <div class="sbh-kpi" id="tc-stats"></div>
    <div class="hd-tools">
      ...
      <div class="tc-direction-dd" id="tc-direction-dd">
        <button type="button" class="btn tc-dd-btn" id="tc-direction-btn" aria-haspopup="listbox" aria-expanded="false">
          <span id="tc-direction-label">Loại giao dịch</span>
        </button>
      ...
      <div class="tc-cols-wrap" id="tc-cols-wrap">
        <button id="tc-cols-btn" class="btn pm-col-btn" type="button" aria-haspopup="true" aria-expanded="false">Hiển thị cột ▾</button>
    ...
    <div class="sbh-card" style="padding:0">
      <div id="tc-list"><p style="padding:16px">Đang tải…</p></div>
  ...
  // Thao tác dropdown
  const tcTaBtn = container.querySelector('#tc-thao-tac-btn');
  const tcTaMenu = container.querySelector('#tc-thao-tac-menu');
  if (tcTaBtn && tcTaMenu) {
    tcTaBtn.addEventListener('click', (e) => {
      e.stopPropagation();
      const open = !tcTaMenu.classList.contains('hidden');
      tcTaMenu.classList.toggle('hidden', open);
      tcTaBtn.setAttribute('aria-expanded', String(!open));
    });
    document.addEventListener('click', () => { tcTaMenu.classList.add('hidden'); tcTaBtn.setAttribute('aria-expanded', 'false'); }, { capture: true });
    container.querySelector('#tc-export')?.addEventListener('click', () => {
      tcTaMenu.classList.add('hidden');
      toast('Tính năng xuất Excel sẽ có sớm.', 'info');
    });
  }
  ...
  function syncDirectionLabel() {
    ...
    else label.textContent = 'Loại giao dịch';
  }
  ...
  function renderTotals() {
    ...
    container.querySelector('#tc-stats').innerHTML = rows.map(([label, value, sign, color]) => `
      <div class="kpi-card">
        ...
        <div class="kpi-val" style="color:${color}">${sign}${escapeHtml(formatVND(Math.abs(value)))}</div>
      </div>`).join('');
  }
  ...
  function renderList() {
    ...
    el.innerHTML = `
      <div style="overflow-x:auto">
        <table class="sp-table" style="width:100%;min-width:660px;border-radius:0">
          <thead><tr>
  ...
  function renderPagination() {
    ...
    el.innerHTML = `<div class="ord-pager">
      <span style="color:var(--text-2,#888);font-size:13px">Hiển thị ${start}-${end} / ${count} kết quả</span>
      <div class="ord-pager-ctrl">
        <span style="font-size:13px;color:var(--text-2,#888)">Hiển thị dòng</span>
        <select id="tc-page-size-sel" style="height:32px;padding:0 6px;border:1px solid var(--border,#ddd);border-radius:6px;font-size:13px">
          ${[10, 30, 50, 100].map((n) => `<option value="${n}" ${n === pageSize ? 'selected' : ''}>${n}</option>`).join('')}
        </select>
        <button class="ord-page-btn" id="tc-page-prev" ${page <= 1 ? 'disabled' : ''}>&#8249;</button>
        <span class="ord-page-cur">${page} / ${lastPage}</span>
        <button class="ord-page-btn" id="tc-page-next" ${page >= lastPage ? 'disabled' : ''}>&#8250;</button>
      </div>
    </div>`;
