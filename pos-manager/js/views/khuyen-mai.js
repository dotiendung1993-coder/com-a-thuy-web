  ...
  function openForm(p) {
    ...
    const modal = openModal(`
      ...
      <div class="field"><label>Đơn tối thiểu (VND, 0 = không giới hạn)</label>
        <input id="km-min" type="number" min="0" step="1000" value="${p?.min_order_total ?? 0}" /></div>
    ...
    modal.overlay.addEventListener('click', (e) => {
      if (e.target.id === 'km-combo-add') {
        modal.overlay.querySelector('#km-combo-rows')?.insertAdjacentHTML('beforeend', comboRowHtml());
    ...
    modal.overlay.querySelector('#km-save').addEventListener('click', async () => {
      ...
      const body = {
        ...
        kind: currentKind,
        min_order_total: Number(modal.overlay.querySelector('#km-min').value) || 0,
      ...
      if (currentKind === 'tang-mon') {
        ...
        body.combo_items = rows.map((row) => ({
          ...
          qty: Number(row.querySelector('.km-combo-qty').value) || 1,
        })).filter((it) => it.menu_id);
        body.combo_price = Number(modal.overlay.querySelector('#km-combo-price').value) || 0;
  ...
  function comboRowHtml(menuId, qty) {
    return `
      <div class="km-combo-row" style="display:flex;gap:8px;align-items:center;margin-bottom:6px">
        ...
        <input class="km-combo-qty" type="number" min="1" value="${qty ?? 1}" style="width:70px" />
    ...
    if (k === 'combo') {
      ...
      return `
        ...
        <div class="field"><label>Giá combo cố định (VND)</label>
          <input id="km-combo-price" type="number" min="0" step="1000" value="${p?.combo_price ?? ''}" /></div>
