...
import { formatVND, escapeHtml, toast, openModal, confirmDialog, pageTabsHtml, formatNumberInput, parseNumberInput, attachNumberInput } from '../ui.js';
...
export async function render(container, { staff } = {}) {
  ...
  function openForm(acc) {
    ...
    const modal = openModal(`
      ...
      ${isEdit ? '' : '<div class="field"><label>Số dư ban đầu</label><input id="nt-opening" type="text" inputmode="numeric" value="0" /></div>'}
      <div class="field">
        <label>Ngưỡng cảnh báo sắp hết (để trống = không cảnh báo)</label>
        <input id="nt-low-threshold" type="text" inputmode="numeric" value="${formatNumberInput(acc && acc.low_balance_threshold != null ? acc.low_balance_threshold : '')}" placeholder="VD: 500.000" />
      </div>
    ...
    `);
    if (!isEdit) attachNumberInput(modal.overlay.querySelector('#nt-opening'), { decimals: 0 });
    attachNumberInput(modal.overlay.querySelector('#nt-low-threshold'), { decimals: 0 });
    modal.overlay.querySelector('#nt-submit').addEventListener('click', async () => {
      ...
      const payload = {
        ...
        low_balance_threshold: parseNumberInput(modal.overlay.querySelector('#nt-low-threshold').value),
      };
      try {
        if (isEdit) {
          ...
          payload.opening_balance = parseNumberInput(modal.overlay.querySelector('#nt-opening').value) || 0;
  ...
  function openTransfer() {
    ...
    const modal = openModal(`
      ...
      <div class="field"><label>Số tiền</label><input id="nt-amt" type="text" inputmode="numeric" placeholder="0" /></div>
    ...
    `);
    attachNumberInput(modal.overlay.querySelector('#nt-amt'), { decimals: 0 });
    ...
    modal.overlay.querySelector('#nt-ok').addEventListener('click', async () => {
      ...
      const amount = parseNumberInput(modal.overlay.querySelector('#nt-amt').value);
