  ...
  container.innerHTML = `
    ...
    <div class="hd-tools">
      <label class="hd-search">
        ...
        <input id="ntien-search" type="search" placeholder="Tìm nguồn tiền..." aria-label="Tìm nguồn tiền" />
      </label>
    </div>
  ...
  function renderList() {
    ...
    el.innerHTML = `
      <div class="hd-table-wrap">
        <table class="hd-table nt-table">
          <thead><tr><th>Tên nguồn tiền</th><th class="hd-num">Số dư</th>${canManage ? '<th></th>' : ''}</tr></thead>
          <tbody>
            ${filtered.map((a) => `
