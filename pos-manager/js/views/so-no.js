...
export async function render(container, { staff } = {}) {
  ...
  container.innerHTML = `
    ...
    <div class="sbh-kpi" id="sn-stats"></div>
    <div class="hd-tools">
      ...
      <div class="ord-cols-wrap" id="sn-cols-wrap">
        <button id="sn-cols-btn" class="btn pm-col-btn" type="button" aria-haspopup="true" aria-expanded="false">Hiển thị cột ▾</button>
  ...
  function renderList() {
    ...
    el.innerHTML = `
      <div class="sbh-card" style="padding:0"><div style="overflow-x:auto">
        <table class="sp-table" id="sn-table" style="width:100%;min-width:600px;border-radius:0">
          <thead><tr>
  ...
  function renderTotals() {
    ...
    const SN_KPI_CLS = { 'sn-kpi-thu': 'kpi-c1', 'sn-kpi-tra': 'kpi-c4', 'sn-kpi-thu-qh': 'kpi-c4', 'sn-kpi-tra-qh': 'kpi-c4' };
    container.querySelector('#sn-stats').innerHTML = cards.map(([cls, label, value]) => `
      <div class="kpi-card ${SN_KPI_CLS[cls] || 'kpi-c3'}">
  ...
  function openDebtModal(presetKind = 'phai-thu') {
    ...
    const modal = openModal(`
      ...
      <div class="field"><label>Số tiền nợ</label><input id="dn-amount" type="text" inputmode="numeric" /></div>
    ...
    attachNumberInput(modal.overlay.querySelector('#dn-amount'), { decimals: 0 });
    ...
    modal.overlay.querySelector('#dn-submit').addEventListener('click', async () => {
      ...
      const amount = parseNumberInput(modal.overlay.querySelector('#dn-amount').value);
  ...
  function openPayModal(debt, detailModal) {
    ...
    const payModal = openModal(`
      ...
      <div class="field"><label>Số tiền</label><input id="pd-amount" type="text" inputmode="numeric" value="${formatNumberInput(debt.remaining)}" /></div>
    ...
    `);
    attachNumberInput(payModal.overlay.querySelector('#pd-amount'), { decimals: 0 });
    payModal.overlay.querySelector('#pd-submit').addEventListener('click', async () => {
      const amount = parseNumberInput(payModal.overlay.querySelector('#pd-amount').value);
