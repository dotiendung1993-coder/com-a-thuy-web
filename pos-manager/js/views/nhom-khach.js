...
export async function render(container, { staff } = {}) {
  ...
  container.innerHTML = `
    ...
    <div class="sbh-card" style="padding:0">
      <div class="sbh-card-tools">
        <input id="nk-q" class="sbh-card-search" type="search" placeholder="Tìm kiếm nhóm khách hàng" />
      </div>
      <div id="nk-list"><p style="padding:16px">Đang tải…</p></div>
    </div>
  `;
  ...
  function openForm(g) {
    ...
    const modal = openModal(`
      ...
      <details style="margin-top:14px">
        ...
        <div class="field" style="margin-top:10px"><label for="nk-disc">Giảm giá tự động (%)</label>
          <input id="nk-disc" type="text" inputmode="decimal" value="${formatNumberInput(g?.discount_percent ?? 0, { decimals: 1 })}" /></div>
    ...
    attachNumberInput(modal.overlay.querySelector('#nk-disc'), { decimals: 1 });
    ...
    modal.overlay.querySelector('#nk-save').addEventListener('click', async () => {
      ...
      const body = {
        name,
        discount_percent: parseNumberInput(modal.overlay.querySelector('#nk-disc').value) || 0,
