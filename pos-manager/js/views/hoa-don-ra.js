...
export async function render(container, { staff } = {}) {
  ...
  container.innerHTML = `
    ...
    <div class="sbh-kpi" id="hdr-stats"></div>
  ...
  function renderList() {
    ...
    el.innerHTML = `
      <div class="sbh-card" style="padding:0"><div style="overflow-x:auto">
        <table class="sp-table" style="width:100%;min-width:720px;border-radius:0">
          <thead><tr>
            ${cols.map(([, label]) => `<th>${escapeHtml(label)}</th>`).join('')}
            ${canManage ? '<th class="hd-act-col" style="width:200px">Thao tác</th>' : '<th>Trạng thái</th>'}
          </tr></thead>
          <tbody>
            ${data.invoices.map((r) => `
              <tr>
  ...
  function openIssue(r) {
    const { overlay, close } = openModal(`
      ...
      <form id="hdr-form">
        ...
        <div class="field"><label>Thuế suất (%)</label>
          <input name="tax_rate" type="text" inputmode="decimal" value="${formatNumberInput(taxRate, { decimals: 1 })}" /></div>
        <div class="dlg-actions">
    ...
    `);
    attachNumberInput(overlay.querySelector('[name="tax_rate"]'), { decimals: 1 });
    ...
    overlay.querySelector('#hdr-form').addEventListener('submit', async (e) => {
      ...
      // tax_rate hiện dấu chấm/phẩy như mọi ô số khác (TASK-dinh-dang-so.md) — FormData ở trên lấy
      // nguyên chuỗi hiển thị, phải gỡ định dạng riêng trước khi gửi, không thì máy chủ Number("8,5")
      // ra NaN.
      payload.tax_rate = String(parseNumberInput(payload.tax_rate) ?? '');
      try {
