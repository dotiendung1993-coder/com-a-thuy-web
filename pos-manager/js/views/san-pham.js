...
import { formatVND, escapeHtml, toast, openModal, confirmDialog, resolveImg, formatNumberInput, parseNumberInput, attachNumberInput } from '../ui.js';
import { icon } from '../icons.js';
import { costFieldState, ingredientOptionsHtml, manualCostChange, recipeSummaryHtml, recipeTotals, resolveIngredientPick, validateRecipeRows } from '../recipe-shared.js';
...
// Task 2 (23/09/2026) — món nhiều mức giá hiện khoảng THẤP NHẤT–CAO NHẤT (gọn, không liệt kê hết mức
// kẻo vỡ bảng danh sách — xem TASK-form-va-gia-theo-size.md). Món một giá hiện như cũ.
export function priceCell(p) {
  const values = p.size_prices ? Object.values(p.size_prices).map(Number) : [];
  if (!values.length) return formatVND(p.price);
...
export async function render(container, { staff } = {}) {
  ...
  const canRecipe = !!perms.ingredient_manage; // sửa công thức = quyền NVL (PUT /ingredients/recipes/menu/:id)
  const canCost = !!perms.report_manage; // nhập tay giá vốn = quyền báo cáo-quản lý (chủ quán), cùng màn Giá vốn
  ...
  container.innerHTML = `
    ...
    <!-- KPI — Đợt 4 (04/09/2026): dải 4 số trong MỘT thẻ ngăn nhau bằng vạch dọc (ảnh mẫu),
         xem .sbh-kpi trong theme-v2.css. Bỏ lưới 4 cột viết thẳng trong style. -->
    <div id="sp-kpi" class="sbh-kpi"></div>

    <!-- Hàng lọc — ảnh mẫu chỉ có MỘT hàng: ô tìm · "Danh mục" · nút vuông chọn cột sát mép phải.
         Trước đây nút "Hiển thị cột ▾" nằm ở một hàng riêng bên dưới, tốn thêm một dòng. -->
    <div class="sbh-card" style="padding:0">
    <div class="sbh-card-tools sp-filter-row">
      <input id="sp-q" class="sp-search sbh-card-search" type="search" placeholder="Tìm kiếm mã SKU/ Tên SP" />
      <div style="position:relative">
      ...
      <div class="sp-bar-right">
        <div style="position:relative">
          <button id="sp-col-btn" class="btn pm-col-btn ord-cols-icon" aria-label="Hiển thị cột" title="Hiển thị cột">${icon('cot-hien-thi')}</button>
    ...
    <!-- Tabs (Tất cả / Đang bán / Ngừng bán / Sắp hết) — ảnh mẫu KHÔNG có hàng tab ở màn này,
         nhưng quán vẫn cần lọc nhanh nên giữ lại; đặt DƯỚI hàng lọc cho khớp màn Đơn hàng. -->
    <div class="tab-row sbh-tabs" id="sp-tabs" style="padding:0 14px"></div>
    ...
    <div style="overflow-x:auto">
      <table class="sp-table" id="sp-table" style="width:100%;min-width:500px;border-radius:0">
    ...
    <!-- Paginator -->
    <div id="sp-paginator" class="ord-pager">
      <span id="sp-page-info" style="color:var(--text-2,#888);font-size:13px"></span>
      <div class="ord-pager-ctrl">
        <span style="font-size:13px;color:var(--text-2,#888)">Hiển thị dòng</span>
        <select id="sp-page-size">
  ...
  if (canManage) {
    container.querySelector('#sp-detail-edit').addEventListener('click', () => {
      // bug-655 (21/09/2026): closeDetail() gán detailProduct = null, nên gọi nó TRƯỚC rồi mới
      // openForm(detailProduct) thì openForm nhận null → mở nhầm form "Tạo sản phẩm mới" (trống)
      // thay vì "Sửa sản phẩm". Giữ lại tham chiếu trước khi đóng bảng chi tiết.
      const p = detailProduct;
      if (p) { closeDetail(); openForm(p); }
    });
  }
  ...
  // ─── Render helpers ─────────────────────────────────────────────────────────
  // Đợt 4 (04/09/2026) — 3 cột sắp xếp được đúng ảnh mẫu (SẢN PHẨM · GIÁ BÁN · CÓ THỂ BÁN).
  // Sắp xếp NGAY TRÊN MÁY vì load() vốn đã lấy sẵn tối đa 300 dòng rồi tự chia trang ở đây —
  // gọi lại máy chủ chỉ để đổi thứ tự là thừa.
  const SORT_KEYS = {
    name:     (p) => String(p.name || '').toLowerCase(),
    giaban:   (p) => Number(p.price) || 0,
    cotheban: (p) => (p.selling ? 1 : 0),
  };
  let sortBy = '';
  let sortDir = 1; // 1 = tăng dần, -1 = giảm dần

  const sortTh = (key, label, extra = '') =>
    `<th ${extra} class="sp-th-sort ${sortBy === key ? 'is-sorted' : ''}" data-sort="${key}">` +
    `${label} <span class="sp-sort-ico">${icon('sap-xep')}</span></th>`;
  ...
  function renderTableHead() {
    container.querySelector('#sp-thead').innerHTML = `<tr>
      ...
      ${visibleCols.has('giaban')   ? sortTh('giaban', 'GIÁ BÁN', 'style="width:110px"') : ''}
      ${visibleCols.has('cotheban') ? sortTh('cotheban', 'CÓ THỂ BÁN', 'style="width:120px"') : ''}
      ${visibleCols.has('tonkho')   ? '<th style="width:80px">TỒN KHO</th>' : ''}
      ${canManage ? '<th style="width:40px"></th>' : ''}
    </tr>`;
    container.querySelectorAll('#sp-thead .sp-th-sort').forEach((th) => {
      th.addEventListener('click', () => {
        const key = th.dataset.sort;
        if (sortBy === key) sortDir = -sortDir; else { sortBy = key; sortDir = 1; }
        page = 1;
        renderTableHead(); renderList(); renderPaginator();
      });
    });
  ...
  function renderKpi() {
    ...
    // Đợt 4 (04/09/2026) — ảnh mẫu: NHÃN xám nhỏ đứng trên, SỐ to đứng dưới, mỗi ô một màu
    // (xanh lá / xanh dương / đen / đỏ). Sửa luôn lỗi chữ "Chưa bắt tồn kho" → "Chưa bật tồn kho".
    container.querySelector('#sp-kpi').innerHTML = `
      <div class="kpi-card kpi-c1"><div class="kpi-label">Số lượng</div><div class="kpi-val">${tatCa}</div></div>
      <div class="kpi-card kpi-c2"><div class="kpi-label">Giá trị tồn</div><div class="kpi-val">${formatVND(giaTri)}</div></div>
      <div class="kpi-card kpi-c3"><div class="kpi-label">Thiếu hàng</div><div class="kpi-val">${sapHet}</div></div>
      <div class="kpi-card kpi-c4"><div class="kpi-label">Chưa bật tồn kho</div><div class="kpi-val">${chuaBat}</div></div>`;
  }
  ...
  /** Danh sách đã sắp xếp theo cột đang chọn; chưa chọn cột nào thì giữ nguyên thứ tự máy chủ. */
  ...
  function renderList() {
      ...
      <tr class="${p.selling ? '' : 'row-inactive'}" data-pid="${p.id}">
        ...
        <td>
          <div class="sp-name-cell" data-detail="${p.id}" style="cursor:pointer">
            <!-- Đợt 4 (04/09/2026) — ảnh mẫu có ô ảnh vuông bo góc đứng trước tên. Chưa có ảnh thì
                 vẽ ô xám có icon món ăn, KHÔNG để trống (nếu không cột tên nhảy so le nhau). -->
            <div class="sp-thumb">${p.image_path
              ? `<img src="${escapeHtml(resolveImg(p.image_path))}" alt="" loading="lazy" />`
              : `<span class="sp-thumb-ico">${icon('mon-an')}</span>`}</div>
        ...
        ${visibleCols.has('giaban')   ? `<td style="font-size:13px;font-weight:500">${priceCell(p)}</td>` : ''}
    ...
    // Ảnh món chưa đồng bộ / đường dẫn hỏng: thay bằng icon thay vì để trình duyệt vẽ hình vỡ.
    tbody.querySelectorAll('.sp-thumb img').forEach((im) => {
      im.addEventListener('error', () => {
        const box = im.closest('.sp-thumb');
        if (box) box.innerHTML = `<span class="sp-thumb-ico">${icon('mon-an')}</span>`;
      }, { once: true });
    });
  ...
  function renderPaginator() {
    ...
    container.querySelector('#sp-page-info').textContent =
      total ? `Hiển thị ${start}–${end} / ${total} kết quả` : '';
  ...
  function openDetailPanel(p) {
    ...
    container.querySelector('#sp-detail-body').innerHTML = `
      ...
      <div class="pform-card" style="margin-bottom:12px">
        ...
        <div style="display:grid;grid-template-columns:100px 1fr;gap:6px;font-size:13px">
          <span style="color:#888">Giá bán</span><span style="font-weight:600">${priceCell(p)}</span>
          <span style="color:#888">Giá vốn</span><span>${p.cost_price ? formatVND(p.cost_price) : '—'}${p.recipe_count > 0 ? ' <small style="color:#888">(tự tính từ công thức)</small>' : ''}</span>
        </div>
      </div>
      ${p.track_stock ? `
  ...
  // Thêm mục Công thức (21/09/2026) dùng chung khuôn này nên có thêm cột và móc tuỳ chọn — Phân loại / Đơn
  // vị quy đổi không truyền thì chạy y như cũ:
  //   cột type 'select' — optionsHtml(row, ctx) dựng <option>; onPick(value, row, ctx) trả giá trị mới của ô
  //   cột type 'static' — ô chỉ đọc html(row, ctx), tự tính lại mỗi lần gõ; cột số emptyIfZero hiện ô trống thay vì 0
  //   prepare() nạp dữ liệu phụ vào ctx trước khi dựng · footer(rows, ctx) khối tổng dưới danh sách
  //   validate(rows, ctx) chặn lưu · onSaved(res) thay toast mặc định · wide mở cửa sổ rộng hơn
  function openRowEditor({ title, hint, product, columns, blankRow, loadUrl, saveUrl, payloadKey,
    prepare, footer, validate, onSaved, wide }) {
    let rows = [];
    let loaded = false; // chưa tải được thì KHÔNG cho lưu: lưu danh sách rỗng sẽ xoá sạch dữ liệu đang có
    const ctx = {};
    ...
    `);
    if (wide) modal.overlay.querySelector('.modal-box').style.maxWidth = '680px';

    function refreshDerived() {
      modal.overlay.querySelectorAll('[data-s]').forEach((el) => {
        const col = columns.find((c) => c.key === el.dataset.s);
        el.innerHTML = col.html(rows[Number(el.dataset.i)], ctx);
      });
      if (footer) modal.overlay.querySelector('#rw-foot').innerHTML = footer(rows, ctx);
    }

    function renderRows() {
      const box = modal.overlay.querySelector('#rw-rows');
      if (!rows.length) { box.innerHTML = '<p class="hint">Chưa có dòng nào.</p>'; refreshDerived(); return; }
      box.innerHTML = rows.map((r, i) => `
        <div style="display:flex;gap:6px;align-items:center;margin-bottom:6px;flex-wrap:wrap">
          ${columns.map((c) => {
            if (c.type === 'select') {
              return `<select data-f="${c.key}" data-i="${i}" style="flex:${c.flex || 1};min-width:150px">${c.optionsHtml(r, ctx)}</select>`;
            }
            if (c.type === 'static') {
              return `<span data-s="${c.key}" data-i="${i}" style="flex:0 0 auto;min-width:70px;text-align:right;font-size:13px;color:#666"></span>`;
            }
            return c.type === 'bool'
              ...
              : c.type === 'number'
              ? `<input data-f="${c.key}" data-i="${i}" type="text" inputmode="${c.decimals ? 'decimal' : 'numeric'}"
                      placeholder="${escapeHtml(c.label)}"
                      value="${escapeHtml(c.emptyIfZero && !r[c.key] ? '' : formatNumberInput(r[c.key] ?? '', { decimals: c.decimals || 0 }))}"
                      style="flex:${c.flex || 1};min-width:90px" />`
              : `<input data-f="${c.key}" data-i="${i}" type="text"
      ...
      box.querySelectorAll('[data-f]').forEach((input) => {
        ...
        if (col.type === 'select') {
          // Dựng lại cả danh sách sau khi chọn (đổi NVL thì ĐVT/thành tiền đổi theo); số lượng đang gõ nằm sẵn
          // trong rows nên không mất. Chọn "+ Tạo mới" thì onPick mở cửa sổ tạo nhanh rồi trả NVL vừa tạo.
          input.addEventListener('change', async () => {
            rows[i][col.key] = await col.onPick(input.value, rows[i], ctx);
            renderRows();
          });
          return;
        }
        if (col.type === 'number') attachNumberInput(input, { decimals: col.decimals || 0 });
        const apply = () => {
          if (col.type === 'bool') { rows[i][col.key] = input.checked; refreshDerived(); return; }
          if (col.type !== 'number') { rows[i][col.key] = input.value; refreshDerived(); return; }
          // type=number đổi sang type=text nên mất validation min= của trình duyệt — bù lại bằng tay
          // (không để lọt số âm cho các cột giá/số lượng, đều dùng min:0 sẵn có trong cấu hình cột).
          let n = parseNumberInput(input.value) || 0;
          if (col.min !== undefined) n = Math.max(col.min, n);
          rows[i][col.key] = n;
          refreshDerived();
        };
    ...
    modal.overlay.querySelector('#rw-save').addEventListener('click', async () => {
      if (!loaded) { toast('Chưa tải được dữ liệu, hãy đóng cửa sổ rồi mở lại', 'error'); return; }
      const bad = validate ? validate(rows, ctx) : null;
      if (bad) { toast(bad, 'error'); return; }
      let res;
      try { res = await api.put(saveUrl, { [payloadKey]: rows }); }
      catch (err) { toast(err?.body?.message || 'Không lưu được', 'error'); return; }
      modal.close();
      if (onSaved) await onSaved(res); else toast('Đã lưu');
    });
    (async () => {
      try {
        if (prepare) Object.assign(ctx, await prepare());
        const res = await api.get(loadUrl);
        rows = (res[payloadKey] || []).map((r) => ({ ...r }));
        loaded = true;
      } catch { /* báo lỗi ngay dưới, không cho lưu */ }
      if (!loaded) {
        modal.overlay.querySelector('#rw-rows').innerHTML = '<p class="hint">Không tải được dữ liệu. Hãy đóng cửa sổ rồi mở lại.</p>';
        return;
      }
      renderRows();
    })();
  ...
  function openUnitForm(p) {
    openRowEditor({
      ...
      columns: [
        ...
        { key: 'factor',  label: `Bằng bao nhiêu ${p.unit || 'đơn vị'}`, type: 'number', decimals: 3, min: 0 },
        { key: 'price',   label: 'Giá bán', type: 'number', min: 0 },
  ...
  // Công thức của món (mục Công thức trong form sửa món, 21/09/2026). Dùng LẠI API công thức sẵn có
  // (GET/PUT /api/mgr/ingredients/recipes/menu/:id) — không thêm bảng hay endpoint mới. Lưu xong MÁY CHỦ tự
  // tính lại giá vốn món (ingredient-service.js::recomputeMenuCosts), ở đây chỉ hiện số xem trước + báo kết quả.
  function openRecipeForm(p, onFormSaved) {
    const recipeUrl = `/api/mgr/ingredients/recipes/menu/${p.id}`;
    const ingOf = (r, c) => c.ingredients.find((x) => x.id === Number(r.ingredient_id));
    openRowEditor({
      title: 'Công thức', product: p, wide: true,
      hint: 'Mỗi phần bán ra dùng bao nhiêu nguyên vật liệu. Giá vốn món TỰ ĐỘNG tính theo công thức này và tự đổi khi giá nhập nguyên vật liệu đổi. Thiếu nguyên vật liệu? Chọn "+ Tạo nguyên vật liệu mới…" ngay ở ô chọn của dòng. Xoá hết các dòng là bỏ công thức: giá vốn giữ nguyên số hiện có và nhập tay lại được ở ô Giá vốn trong form sửa món (hoặc màn Giá vốn).',
      columns: [
        { key: 'ingredient_id', label: 'Nguyên vật liệu', type: 'select', flex: 3,
          optionsHtml: (r, c) => ingredientOptionsHtml(c.ingredients, r.ingredient_id),
          onPick: async (value, r, c) => {
            const pick = await resolveIngredientPick(value, r.ingredient_id, c.ingredients);
            if (pick.items) c.ingredients = pick.items;
            return pick.id;
          } },
        { key: 'qty_per_dish', label: 'Số lượng', type: 'number', decimals: 3, min: 0, emptyIfZero: true },
        { key: '_unit', label: 'ĐVT', type: 'static', html: (r, c) => escapeHtml(ingOf(r, c)?.unit || '') },
        { key: '_line', label: 'Thành tiền', type: 'static',
          html: (r, c) => {
            const ing = ingOf(r, c);
            if (!ing) return '';
            return ing.cost_price ? formatVND(recipeTotals([r], c.ingredients).total) : '<span style="color:var(--warn,#b45309)">chưa có giá</span>';
          } },
      ],
      blankRow: { ingredient_id: null, qty_per_dish: 0, note: '' },
      loadUrl: recipeUrl, saveUrl: recipeUrl, payloadKey: 'ingredients',
      prepare: async () => ({ ingredients: (await api.get('/api/mgr/ingredients')).items || [] }),
      footer: (rows, c) => recipeSummaryHtml(rows, c.ingredients),
      validate: (rows, c) => validateRecipeRows(rows, c.ingredients),
      onSaved: async (res) => {
        toast(res.ingredients?.length
          ? `Đã lưu công thức. Giá vốn món tự cập nhật: ${formatVND(res.menu_cost_price)}`
          : 'Đã bỏ công thức. Giá vốn giữ nguyên số cũ, nhập tay lại được ở form sửa món hoặc màn Giá vốn');
        await load();
        if (onFormSaved) onFormSaved(res);
      },
    });
  }
  ...
  function openForm(p) {
    ...
    // Món CÓ công thức thì giá vốn TỰ TÍNH từ công thức (máy chủ cập nhật mỗi khi đổi công thức/giá nhập NVL) —
    // ô giá vốn khoá và nói rõ vì sao, chứ không để chủ quán tưởng gõ đè được. Món CHƯA có công thức: chủ quán
    // nhập tay ngay tại đây, hoặc bấm "Theo công thức" để giá vốn tự tính (costFieldState quyết định ô nào mở).
    // `let` (không phải const): sau khi lưu công thức ở cửa sổ con (chồng lên form, xem applyRecipeState
    // bên dưới) phải cập nhật lại NGAY tại đây, không đóng-mở lại cả form — cùng họ bug-655.
    let recipeN = Number(cur.recipe_count) || 0;
    let recipeMissing = Number(cur.recipe_missing_cost) || 0;
    let costUi = costFieldState({ isNew, recipeCount: recipeN, canCost, canRecipe });
    function missingNoteHtml() {
      return recipeMissing
        ? `<span style="color:var(--warn,#b45309)">Còn ${recipeMissing} nguyên vật liệu chưa khai giá nhập nên giá vốn đang thiếu phần này.</span>`
        : '';
    }
    function costHintText() {
      return recipeN
        ? `Giá vốn TỰ TÍNH từ công thức (${recipeN} nguyên vật liệu) và tự đổi khi giá nhập nguyên vật liệu đổi, nên không nhập tay được. Muốn đổi, sửa mục Công thức bên dưới; muốn nhập tay lại thì xoá hết các dòng công thức. ${missingNoteHtml()}`
        : [
          costUi.inputEditable ? 'Nhập giá vốn vào ô trên (để trống = chưa có giá vốn).' : 'Chỉ chủ quán mới nhập tay được giá vốn.',
          costUi.canPickRecipe ? 'Hoặc chọn "Theo công thức" để giá vốn tự tính từ nguyên vật liệu.'
            : isNew ? 'Muốn tính theo công thức: lưu món trước rồi mở lại để khai Công thức.' : '',
        ].filter(Boolean).join(' ');
    }
    function recipeSectionHint() {
      return isNew ? 'Lưu món trước rồi mở lại để khai công thức (nguyên vật liệu cho mỗi phần).'
        : recipeN ? `Đã có công thức: <b>${recipeN}</b> nguyên vật liệu, giá vốn <b>${formatVND(cur.cost_price)}</b>. ${missingNoteHtml()}`
          : 'Chưa có công thức. Khai công thức để giá vốn món tự tính theo nguyên vật liệu.';
    }
    const costHint = costHintText();
    // Task 2 (23/09/2026) — giá nhiều mức (size/suất/đĩa): size_order (đúng thứ tự) nếu có, không thì lấy
    // thứ tự khoá của size_prices (dữ liệu Cơm mâm cũ trước khi có size_order, thường chỉ 4 mức không đáng
    // lo thứ tự jsonb sắp lại). Rỗng = "Một giá".
    let tiers = (() => {
      if (!cur.size_prices) return [];
      const names = (cur.size_order && cur.size_order.length ? cur.size_order : Object.keys(cur.size_prices))
        .filter((n) => cur.size_prices[n] != null);
      return names.map((n) => ({ name: n, price: Number(cur.size_prices[n]) || 0 }));
    })();
    const modal = openModal(`
      ...
      <div class="pform-body">
        <div class="pform-col">
          ...
          <section class="pform-card"><h4>Giá sản phẩm</h4>
            <div class="field pform-full"><label>Kiểu giá bán</label>
              <div class="seg" id="sp-pricemode" role="group" aria-label="Kiểu giá bán">
                <button type="button" class="seg-btn ${tiers.length ? '' : 'active'}" data-price-mode="single" aria-pressed="${tiers.length ? 'false' : 'true'}">Một giá</button>
                <button type="button" class="seg-btn ${tiers.length ? 'active' : ''}" data-price-mode="tiers" aria-pressed="${tiers.length ? 'true' : 'false'}">Nhiều mức giá</button>
              </div></div>
            <div class="pform-grid">
              <div class="field" id="sp-price-single-wrap"><label>Giá bán <i class="req">*</i></label>
                <input id="sp-price" type="text" inputmode="numeric" value="${formatNumberInput(Number(cur.price) || 0)}" ${tiers.length ? 'readonly' : ''} /></div>
              <div class="field"><label id="sp-cost-label">Giá vốn${recipeN ? ' (tự tính từ công thức)' : ''}</label>
                <input id="sp-cost" type="text" inputmode="numeric" value="${formatNumberInput(cur.cost_price ?? '')}" placeholder="Chưa có giá vốn" ${costUi.inputEditable ? '' : 'readonly'} /></div>
              <div class="field pform-full"><label>Cách tính giá vốn</label>
                <div class="seg" id="sp-costmode" role="group" aria-label="Cách tính giá vốn">
                  <button type="button" class="seg-btn ${costUi.mode === 'manual' ? 'active' : ''}" data-cost-mode="manual" aria-pressed="${costUi.mode === 'manual'}" ${costUi.canPickManual ? '' : 'disabled'}>Nhập tay</button>
                  <button type="button" class="seg-btn ${costUi.mode === 'recipe' ? 'active' : ''}" data-cost-mode="recipe" aria-pressed="${costUi.mode === 'recipe'}" ${costUi.canPickRecipe ? '' : 'disabled'}>Theo công thức</button>
                </div></div>
            </div>
            <p class="hint" id="sp-cost-hint">${costHint}</p>
            <div id="sp-price-tiers-wrap" ${tiers.length ? '' : 'hidden'}>
              <p class="hint">Nhiều mức giá (size/suất/đĩa…) — Giá bán ở trên tự = mức thấp nhất, đổi tên/giá/thứ tự tuỳ ý, tối đa 8 mức.</p>
              <div id="sp-tiers-rows"></div>
              <div style="display:flex;gap:8px;margin-top:4px">
                <button type="button" class="btn" id="sp-tiers-add" style="flex:1">+ Thêm mức</button>
                <button type="button" class="btn btn-ghost" id="sp-tiers-preset" style="flex:1">Điền nhanh XS/S/M/L</button>
              </div>
            </div>
          </section>
          <section class="pform-card"><h4>Công thức</h4>
            <p class="hint" id="sp-recipe-hint">${recipeSectionHint()}</p>
            ${isNew || !canRecipe ? '' : `<button type="button" class="pform-link" id="sp-go-recipe">${recipeN ? 'Sửa công thức' : '+ Thêm công thức'}</button>`}
          </section>
        ...
        <div class="pform-col pform-col-side">
          ...
          <section class="pform-card pform-switches">
            ...
            <div class="field pform-min ${cur.track_stock ? '' : 'hidden'}" id="sp-min-wrap">
              ...
              <input id="sp-min" type="text" inputmode="decimal" value="${formatNumberInput(cur.min_stock, { decimals: 3 })}" /></div>
    ...
    attachNumberInput(modal.overlay.querySelector('#sp-price'), { decimals: 0 });
    attachNumberInput(modal.overlay.querySelector('#sp-min'), { decimals: 3 });

    // Task 2 (23/09/2026) — giá nhiều mức: danh sách `tiers` [{name,price}] giữ NGUYÊN thứ tự người dùng
    // sắp (mảng JS, khác object size_prices) — gửi lên server đúng thứ tự này để server tách ra
    // size_prices + size_order (xem stock-service.js::validateSizeTiers).
    const priceSingleEl = modal.overlay.querySelector('#sp-price');
    function updatePriceFromTiers() {
      if (!tiers.length) return;
      priceSingleEl.value = formatNumberInput(Math.min(...tiers.map((t) => Number(t.price) || 0)));
    }
    function renderTiers() {
      const box = modal.overlay.querySelector('#sp-tiers-rows');
      box.innerHTML = tiers.length ? tiers.map((row, i) => `
        <div style="display:flex;gap:6px;align-items:center;margin-bottom:6px">
          <input data-tf="name" data-i="${i}" type="text" placeholder="Tên mức (VD: XS)" value="${escapeHtml(row.name)}" style="flex:2;min-width:100px" />
          <input data-tf="price" data-i="${i}" type="text" inputmode="numeric" placeholder="Giá" value="${formatNumberInput(row.price || 0)}" style="flex:1;min-width:90px" />
          <button type="button" data-t-up="${i}" aria-label="Đưa mức ${i + 1} lên" ${i === 0 ? 'disabled' : ''}>&#8593;</button>
          <button type="button" data-t-down="${i}" aria-label="Đưa mức ${i + 1} xuống" ${i === tiers.length - 1 ? 'disabled' : ''}>&#8595;</button>
          <button type="button" data-t-rm="${i}" aria-label="Xoá mức ${i + 1}" style="color:#c00">Xoá</button>
        </div>`).join('') : '<p class="hint">Chưa có mức nào — bấm "+ Thêm mức".</p>';
      box.querySelectorAll('[data-tf]').forEach((input) => {
        const i = Number(input.dataset.i), f = input.dataset.tf;
        if (f === 'price') attachNumberInput(input, { decimals: 0 });
        input.addEventListener('input', () => {
          tiers[i][f] = f === 'price' ? (parseNumberInput(input.value) || 0) : input.value;
          if (f === 'price') updatePriceFromTiers();
        });
      });
      box.querySelectorAll('[data-t-up]').forEach((b) => b.addEventListener('click', () => {
        const i = Number(b.dataset.tUp); [tiers[i - 1], tiers[i]] = [tiers[i], tiers[i - 1]]; renderTiers();
      }));
      box.querySelectorAll('[data-t-down]').forEach((b) => b.addEventListener('click', () => {
        const i = Number(b.dataset.tDown); [tiers[i], tiers[i + 1]] = [tiers[i + 1], tiers[i]]; renderTiers();
      }));
      box.querySelectorAll('[data-t-rm]').forEach((b) => b.addEventListener('click', () => {
        tiers.splice(Number(b.dataset.tRm), 1); renderTiers(); updatePriceFromTiers();
      }));
    }
    function renderPriceMode() {
      const single = !tiers.length;
      modal.overlay.querySelector('#sp-price-tiers-wrap').hidden = single;
      priceSingleEl.readOnly = !single;
      modal.overlay.querySelectorAll('#sp-pricemode .seg-btn').forEach((b) => {
        const mine = (b.dataset.priceMode === 'single') === single;
        b.classList.toggle('active', mine);
        b.setAttribute('aria-pressed', String(mine));
      });
      if (!single) updatePriceFromTiers();
    }
    modal.overlay.querySelectorAll('#sp-pricemode .seg-btn').forEach((b) => {
      b.addEventListener('click', () => {
        if (b.dataset.priceMode === 'tiers' && !tiers.length) tiers.push({ name: '', price: parseNumberInput(priceSingleEl.value) || 0 });
        else if (b.dataset.priceMode === 'single') tiers = [];
        renderTiers(); renderPriceMode();
      });
    });
    modal.overlay.querySelector('#sp-tiers-add').addEventListener('click', () => {
      if (tiers.length >= 8) { toast('Tối đa 8 mức giá', 'error'); return; }
      tiers.push({ name: '', price: 0 }); renderTiers();
    });
    modal.overlay.querySelector('#sp-tiers-preset').addEventListener('click', () => {
      tiers = ['XS', 'S', 'M', 'L'].map((n) => ({ name: n, price: 0 }));
      renderTiers(); renderPriceMode(); updatePriceFromTiers();
    });
    renderTiers();
    ...
    // "Theo công thức" mở cửa sổ công thức (cùng nút "Thêm/Sửa công thức" bên dưới); "Nhập tay" chỉ đưa con trỏ vào ô.
    // Nút bị `disabled` (đã có công thức / món mới / thiếu quyền) thì trình duyệt không phát sự kiện click.
    const costEl = modal.overlay.querySelector('#sp-cost');
    attachNumberInput(costEl, { decimals: 0 });

    // bug-655 (họ lỗi "dọn state rồi mới đọc chính state đó"), báo lại 22/09/2026: mở Công thức/Đơn vị/Phân
    // loại KHÔNG được đóng form cha nữa — cửa sổ con mở CHỒNG LÊN (cùng z-index 60, mở sau nên nổi trên,
    // xem .modal-overlay trong app.css), đóng cửa sổ con là form cha hiện lại NGUYÊN VẸN vì DOM của nó chưa
    // hề bị gỡ. Lưu công thức xong thì form cha phải tự cập nhật lại (không re-render cả form, tránh mất
    // các ô khác đang gõ dở) — applyRecipeState() làm việc đó từ kết quả PUT trả về.
    function applyRecipeState() {
      costUi = costFieldState({ isNew, recipeCount: recipeN, canCost, canRecipe });
      modal.overlay.querySelector('#sp-cost-label').textContent = `Giá vốn${recipeN ? ' (tự tính từ công thức)' : ''}`;
      costEl.value = formatNumberInput(cur.cost_price ?? '');
      costEl.readOnly = !costUi.inputEditable;
      modal.overlay.querySelectorAll('#sp-costmode .seg-btn').forEach((b) => {
        const mine = b.dataset.costMode === costUi.mode;
        b.classList.toggle('active', mine);
        b.setAttribute('aria-pressed', String(mine));
        b.disabled = b.dataset.costMode === 'manual' ? !costUi.canPickManual : !costUi.canPickRecipe;
      });
      modal.overlay.querySelector('#sp-cost-hint').innerHTML = costHintText();
      modal.overlay.querySelector('#sp-recipe-hint').innerHTML = recipeSectionHint();
      const goRecipeBtn = modal.overlay.querySelector('#sp-go-recipe');
      if (goRecipeBtn) goRecipeBtn.textContent = recipeN ? 'Sửa công thức' : '+ Thêm công thức';
    }
    function onRecipeSaved(res) {
      recipeN = (res.ingredients || []).length;
      cur.recipe_count = recipeN;
      cur.cost_price = res.menu_cost_price;
      recipeMissing = (res.ingredients || []).filter((x) => !Number(x.cost_price)).length;
      applyRecipeState();
    }
    modal.overlay.querySelectorAll('#sp-costmode .seg-btn').forEach((b) => {
      b.addEventListener('click', () => {
        if (b.dataset.costMode === 'recipe') { if (p) openRecipeForm(p, onRecipeSaved); }
        else costEl.focus();
      });
    });

    if (!isNew) {
      modal.overlay.querySelector('#sp-go-units').addEventListener('click', () => openUnitForm(p));
      modal.overlay.querySelector('#sp-go-variants').addEventListener('click', () => openVariantForm(p));
      modal.overlay.querySelector('#sp-go-recipe')?.addEventListener('click', () => openRecipeForm(p, onRecipeSaved));
    }
    ...
    modal.overlay.querySelector('#sp-save').addEventListener('click', async () => {
      ...
      // Giá vốn nhập tay: chỉ xét khi ô đang mở (chủ quán + món chưa có công thức) và có đổi so với lúc mở form.
      const costChange = costUi.inputEditable ? manualCostChange(costEl.value, cur.cost_price) : { changed: false };
      if (costChange.error) { toast(costChange.error, 'error'); costEl.focus(); return; }
      // Giá nhiều mức: kiểm nhẹ phía trình duyệt (tên/trùng) cho phản hồi ngay — máy chủ vẫn là nơi
      // chặn cuối (giới hạn số mức, độ dài tên, số nguyên không âm) — xem stock-service.js::validateSizeTiers.
      let sizeTiers = null;
      if (tiers.length) {
        const cleaned = tiers.map((t) => ({ name: String(t.name || '').trim(), price: Number(t.price) || 0 }));
        if (cleaned.some((t) => !t.name)) { toast('Mỗi mức giá phải có tên', 'error'); return; }
        const lower = cleaned.map((t) => t.name.toLowerCase());
        if (new Set(lower).size !== lower.length) { toast('Tên mức giá bị trùng (không phân biệt hoa/thường)', 'error'); return; }
        sizeTiers = cleaned;
      }
      const payload = { name, price: parseNumberInput(modal.overlay.querySelector('#sp-price').value) || 0, category: modal.overlay.querySelector('#sp-cat-in').value.trim(), unit: modal.overlay.querySelector('#sp-unit').value.trim(), barcode: modal.overlay.querySelector('#sp-barcode').value.trim(), visible: modal.overlay.querySelector('#sp-visible').checked, availability, image_path: imgEl.value.trim(), track_stock: trackEl.checked, min_stock: parseNumberInput(modal.overlay.querySelector('#sp-min').value) || 0, size_tiers: sizeTiers };
      try {
        let productId = p?.id;
        ...
        // Giá vốn KHÔNG nằm trong PATCH /products (máy chủ không nhận, đúng ranh giới quyền): đi qua đúng API màn Giá vốn
        // — cùng quyền report_manage, cùng lớp chặn món đã có công thức (409). Món đã lưu xong rồi nên lỗi ở bước này
        // chỉ báo riêng, không làm như chưa lưu gì.
        let costFailed = false;
        if (costChange.changed && productId) {
          try { await api.patch('/api/mgr/reports/cost-prices', { items: [{ id: productId, cost_price: costChange.value }] }); }
          catch (costErr) { costFailed = true; toast(`Đã lưu món nhưng chưa lưu được giá vốn: ${costErr?.body?.message || 'lỗi không rõ'}`, 'error'); }
        }
        if (!costFailed) toast(isNew ? 'Đã tạo món mới' : 'Đã lưu');
        modal.close();
  ...
  function openBarcodeDialog() {
    const modal = openModal(`
      ...
      <div class="field"><label>Số tem mỗi món</label>
        <input id="bc-copies" type="text" inputmode="numeric" value="1" /></div>
    ...
    attachNumberInput(modal.overlay.querySelector('#bc-copies'), { decimals: 0 });
    modal.overlay.querySelector('#bc-go').addEventListener('click', () => {
      const copies = Math.min(50, Math.max(1, parseNumberInput(modal.overlay.querySelector('#bc-copies').value) || 1));
      const params = new URLSearchParams({ ids: [...selected].join(','), size: modal.overlay.querySelector('#bc-size').value, copies: String(copies), price: modal.overlay.querySelector('#bc-price').checked ? '1' : '0' });
