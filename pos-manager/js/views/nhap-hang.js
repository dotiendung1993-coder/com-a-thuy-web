...
import { formatVND, escapeHtml, toast, openModal, confirmDialog, promptDialog, formatNumberInput, parseNumberInput, attachNumberInput } from '../ui.js';
...
export async function render(container, { staff } = {}) {
  ...
  async function openDetailModal(id) {
    ...
    try {
      ...
      modal.overlay.querySelector('.modal-box').innerHTML = `
        ...
        <div style="margin-top:12px">
          ...
          <div id="nh-attach-list" style="display:flex;gap:8px;flex-wrap:wrap;margin-top:6px">
            ${(d.attachment_urls || []).map((u) => `<img data-att="${escapeHtml(u)}" alt="Ảnh chứng từ" style="width:64px;height:64px;object-fit:cover;border-radius:6px;border:1px solid var(--border,#ddd)" />`).join('')}
      ...
      `;
      // A4: ảnh chứng từ đọc qua route CÓ ĐĂNG NHẬP (cookie) → tải bằng fetch rồi hiện qua blob (thẻ <img src> trực tiếp không đảm bảo gửi
      // cookie chéo miền như các cuộc gọi API khác).
      const { getApiBase: apiBaseOf } = await import('../api.js');
      modal.overlay.querySelectorAll('img[data-att]').forEach(async (img) => {
        try {
          const r = await fetch(`${apiBaseOf()}${img.dataset.att}`, { credentials: 'include' });
          if (!r.ok) throw new Error('HTTP ' + r.status);
          img.src = URL.createObjectURL(await r.blob());
        } catch (e) { console.error('[nhap-hang] không tải được ảnh chứng từ', img.dataset.att, e.message); }
      });
  ...
  async function openCreateModal() {
    ...
    const modal = openModal(`
      ...
      <div class="field" style="margin-top:12px"><label>Giảm giá (VND)</label>
        <input id="nh-cm-discount" type="text" inputmode="numeric" value="0" /></div>
      <div class="field"><label>Chi phí phát sinh (VND)</label>
        <input id="nh-cm-extra" type="text" inputmode="numeric" value="0" /></div>
      ...
      <div class="field"><label>Tiền trả nhà cung cấp</label>
        <input id="nh-cm-paid" type="text" inputmode="numeric" value="0" /></div>
    ...
    function renderLines() {
      ...
      box.innerHTML = `<div class="stock-table">
        ...
        ${lines.map((l, i) => `<div class="stock-table-row" data-i="${i}">
          ...
          <span><input data-f="qty" data-i="${i}" type="text" inputmode="decimal" value="${formatNumberInput(l.qty, { decimals: 3 })}" style="width:70px" /></span>
          <span><input data-f="unit_price" data-i="${i}" type="text" inputmode="numeric" value="${formatNumberInput(l.unit_price)}" style="width:90px" /></span>
          <span><input data-f="discount_amount" data-i="${i}" type="text" inputmode="numeric" value="${formatNumberInput(l.discount_amount)}" style="width:80px" /></span>
          <span><input data-f="tax_percent" data-i="${i}" type="text" inputmode="numeric" value="${formatNumberInput(l.tax_percent)}" style="width:60px" /></span>
      ...
      box.querySelectorAll('[data-f]').forEach((input) => {
        attachNumberInput(input, { decimals: input.dataset.f === 'qty' ? 3 : 0 });
        input.addEventListener('input', () => {
          const i = Number(input.dataset.i);
          lines[i][input.dataset.f] = Math.max(0, parseNumberInput(input.value) || 0);
          renderLines();
        });
      });
    ...
    function renderTotal() {
      ...
      const discount = parseNumberInput(modal.overlay.querySelector('#nh-cm-discount').value) || 0;
      const extra = parseNumberInput(modal.overlay.querySelector('#nh-cm-extra').value) || 0;
      const total = Math.max(0, subtotal - discount + extra);
      const paidInput = modal.overlay.querySelector('#nh-cm-paid');
      if (!isDebt) paidInput.value = formatNumberInput(total);
      const paid = parseNumberInput(paidInput.value) || 0;
    ...
    ['nh-cm-discount', 'nh-cm-extra', 'nh-cm-paid'].forEach((id) => {
      const el = modal.overlay.querySelector(`#${id}`);
      attachNumberInput(el, { decimals: 0 });
    ...
    modal.overlay.querySelector('#nh-cm-debt').addEventListener('change', (e) => {
      ...
      if (isDebt) modal.overlay.querySelector('#nh-cm-paid').value = formatNumberInput(0);
      renderTotal();
    });
    ...
    modal.overlay.querySelector('#nh-cm-submit').addEventListener('click', async () => {
      ...
      try {
        await api.post('/api/mgr/stock/documents', {
          ...
          discount_amount: parseNumberInput(modal.overlay.querySelector('#nh-cm-discount').value) || 0,
          extra_cost: parseNumberInput(modal.overlay.querySelector('#nh-cm-extra').value) || 0,
          payment_method: modal.overlay.querySelector('#nh-cm-pay').value,
          paid_amount: parseNumberInput(modal.overlay.querySelector('#nh-cm-paid').value) || 0,
          is_debt: isDebt,
