...
export async function render(container, { staff } = {}) {
  ...
  container.innerHTML = `
    ...
    <div id="bck-kpi" class="sbh-kpi"></div>
    ...
    <div style="overflow-x:auto">
      <table class="sp-table" id="bck-table" style="width:100%;min-width:760px;border-radius:0">
    ...
    <div id="bck-paginator" class="ord-pager">
      <span id="bck-page-info" style="color:var(--text-2,#888);font-size:13px"></span>
      <div class="ord-pager-ctrl">
        <span style="font-size:13px;color:var(--text-2,#888)">Hiển thị dòng</span>
        <select id="bck-page-size" style="height:32px;padding:0 6px;border:1px solid var(--border,#ddd);border-radius:6px;font-size:13px">
  ...
  function renderKpi() {
    ...
    container.querySelector('#bck-kpi').innerHTML = `
      <div class="kpi-card"><div class="kpi-label">Giá trị tồn</div><div class="kpi-val" style="color:var(--primary)">${formatVND(s.total_value || 0)}</div></div>
      <div class="kpi-card"><div class="kpi-label">Tổng số lượng</div><div class="kpi-val" style="color:#2563EB">${(s.total_qty || 0).toLocaleString('vi')}</div></div>
      <div class="kpi-card"><div class="kpi-label">Sắp hết hàng</div><div class="kpi-val" style="color:#92400E">${s.low_stock_count || 0}</div></div>
      <div class="kpi-card"><div class="kpi-label">Xuất trong kỳ</div><div class="kpi-val" style="color:var(--danger)">${(s.total_outflow || 0).toLocaleString('vi')}</div></div>
    `;
  }
