...
export async function render(container, { staff } = {}) {
  ...
  container.innerHTML = `
    ...
    <div class="hd-stats" id="hdt-kpi"></div>
  ...
  container.querySelector('#hdt-kpi').innerHTML = [
    ...
    ['Cần bổ sung', 0, 'warn'],
    ['Sắp hết hạn', 0, 'warn'],
    ['Đã phát hành tháng này', 0, 'ok'],
  ].map(([label, value, tone]) => `
    <div class="kpi-card">
      <div style="font-size:12px;color:var(--text-2);margin-bottom:6px">${escapeHtml(label)}</div>
      <div class="kpi-val" style="color:${tone === 'warn' ? 'var(--danger,#c00)' : 'var(--primary,#16a34a)'}">${value}</div>
    </div>`).join('');
