...
import { formatVND, escapeHtml, toast, openModal, zoneLabel, confirmDialog, promptDialog, resolveImg, tableName } from '../ui.js';
...
export async function render(container) {
  ...
  // có ✏️ icon bên cạnh để override giá trị tạm cho đơn này (không đổi cấu hình gốc).
  function renderSurchargesUI() {
    ...
    function lineLabel(s) {
      ...
      if (s.value_type === 'percent') {
        const pct = ov.value != null ? Number(ov.value) : s.value;
        return `${escapeHtml(s.name)} (+${pct}%${ov.value != null ? ' ✏️' : ''})`;
      }
      if (s.value_type === 'per_item') {
        const unitPrice = ov.value != null ? Number(ov.value) : s.value;
        const qty = ov.qty != null ? Number(ov.qty) : (itemCounts[s.per_item_category] || 0);
        return `${escapeHtml(s.name)} (${formatVND(unitPrice)}×${qty}${ov.qty != null ? ' ✏️' : ''})`;
      }
      const amt = ov.value != null ? Number(ov.value) : s.value;
      return `${escapeHtml(s.name)} (+${formatVND(amt)}${ov.value != null ? ' ✏️' : ''})`;
    }
    el.innerHTML = `
      ${autoSurcharges.map((s) => `
        <div class="cart-item-size" style="display:flex;align-items:center;gap:6px">
          <span>${lineLabel(s)} — tự động</span>
          <button data-override-sc="${s.id}" style="background:none;border:none;cursor:pointer;padding:0;font-size:.9em" title="Ghi đè giá trị cho đơn này">✏️</button>
        </div>`).join('')}
      ${manualSurcharges.map((s) => `
        <label style="display:flex;align-items:center;gap:8px;margin:4px 0">
          ...
          ${cart.surchargeIds.includes(s.id) ? `<button data-override-sc="${s.id}" style="background:none;border:none;cursor:pointer;padding:0;font-size:.9em" title="Ghi đè giá trị cho đơn này">✏️</button>` : ''}
        </label>`).join('')}
    `;
    ...
    el.querySelectorAll('[data-override-sc]').forEach((btn) => {
      btn.addEventListener('click', async (e) => {
        ...
        if (s.value_type === 'per_item') {
          ...
          if (raw === null) return;
          cart.surchargeOverrides = { ...cart.surchargeOverrides, [id]: { ...curOv, qty: Math.max(0, Number(raw) || 0) } };
        } else {
          ...
          if (raw === null) return;
          cart.surchargeOverrides = { ...cart.surchargeOverrides, [id]: { ...curOv, value: Math.max(0, Number(raw) || 0) } };
        }
        persist();
        renderCartItems();
      });
