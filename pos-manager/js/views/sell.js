...
import { formatVND, escapeHtml, toast, openModal, zoneLabel, confirmDialog, promptDialog, resolveImg, tableName, formatNumberInput, parseNumberInput, attachNumberInput } from '../ui.js';
...
function blankCart(id) {
  return {
    ...
    customerPhone: '', customerName: '', items: [], surchargeIds: [], surchargeOverrides: {},
...
function normalizeCart(raw, id) {
  ...
  c.surchargeOverrides = (raw.surchargeOverrides && typeof raw.surchargeOverrides === 'object') ? raw.surchargeOverrides : {};
 ...
 *  `tablesList` (tuỳ chọn) là danh sách bàn từ /api/mgr/tables — có thì lấy TÊN RIÊNG của bàn
 *  (migration 091) để thẻ đơn khớp với ô bàn ở màn Quản lý bàn; không có thì vẫn "Bàn <số>". */
export function cartTabLabel(cart, carts, tablesList = null) {
  if (cart.deliveryType === 'tai-ban' && cart.tableNo) {
    const t = tablesList?.find((x) => Number(x.table_no) === Number(cart.tableNo));
    return t ? tableName(t) : `Bàn ${cart.tableNo}`;
  }
  ...
  return `
      <section class="qs-dock hidden" id="qs-dock" aria-label="Bán nhanh">
        ...
        <div class="qs-dock-body">
          ...
          <div class="qs-col">
            <span class="qs-lbl">Giá bán</span>
...
export async function render(container) {
  ...
  container.innerHTML = `
    <div class="pos-bar">
      <!-- Việc "Website v2" (03/09/2026) — ảnh mẫu app.sobanhang.com/pos xếp 3 nút điều hướng
           (Trang chủ, Đơn hàng, Quản lý bàn) ở ĐẦU thanh xanh, TRƯỚC ô tìm kiếm. Bản cũ để ô tìm
           kiếm đứng đầu và chỉ có Quản lý bàn. Nút × thoát màn Bán hàng ở cuối giữ nguyên. -->
      <a class="pos-bar-ico pos-bar-nav" href="#/trang-chu" title="Trang chủ" aria-label="Trang chủ">${icon('trang-chu')}</a>
      <a class="pos-bar-ico pos-bar-nav" href="#/don-hang" title="Đơn hàng" aria-label="Đơn hàng">${icon('don-hang')}</a>
      ...
      <div class="pos-search">
        ...
        <input id="sell-search" type="search" placeholder="Tìm tên sản phẩm, mã SKU (F3)" />
        <button class="pos-scan-btn" id="sell-scan-btn" type="button" title="Quét mã vạch" aria-label="Quét mã vạch">
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" width="16" height="16"><rect x="2" y="3" width="2" height="18"/><rect x="6" y="3" width="4" height="18"/><rect x="12" y="3" width="2" height="18"/><rect x="16" y="3" width="1" height="18"/><rect x="19" y="3" width="3" height="18" opacity=".45"/></svg>
        </button>
      </div>
      ...
      <div class="pos-bar-right">
        ...
        <button class="pos-bar-ico" id="sell-req-btn" type="button" title="Thông báo mới" aria-label="Thông báo mới">${icon('chuong')}</button>
        <button class="pos-bar-ico ${viewMode === 'luoi' ? 'active' : ''}" id="sell-view-grid" type="button" title="Hiển thị lưới" aria-label="Hiển thị lưới">
        ...
        <button class="pos-bar-ico ${viewMode !== 'luoi' ? 'active' : ''}" id="sell-view-list" type="button" title="Hiển thị danh sách" aria-label="Hiển thị danh sách">
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" width="21" height="21"><path d="M8 6h13M8 12h13M8 18h13M3 6h.01M3 12h.01M3 18h.01"/></svg>
        </button>
        <div class="pos-sf-wrap" id="pos-more-wrap">
          <button class="pos-bar-ico" id="sell-more-btn" type="button" title="Thêm tuỳ chọn" aria-label="Thêm tuỳ chọn">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" width="21" height="21"><circle cx="12" cy="5" r="1.8"/><circle cx="12" cy="12" r="1.8"/><circle cx="12" cy="19" r="1.8"/></svg>
          </button>
          <div class="pos-sf-drop pos-more-drop hidden" id="sell-more-drop">
            <button type="button" class="pos-sf-opt" id="sell-shortcut-opt">Chú thích phím tắt</button>
            <button type="button" class="pos-sf-opt" id="sell-sync-opt">Đồng bộ dữ liệu</button>
            <button type="button" class="pos-sf-opt" id="sell-mute-opt">Tắt âm báo yêu cầu mới</button>
          </div>
        </div>
      ...
      <div class="cart-head">
        <div class="customer-panel">
          <div class="customer-search-row">
            <div class="customer-search-wrap" id="customer-search-wrap">
              <span class="cust-search-ico">${icon('tim-kiem')}</span>
              <input id="customer-phone" type="tel" value="${escapeHtml(cart.customerPhone)}"
                placeholder="Tìm tên/số điện thoại khách (ALT+C)" autocomplete="off" />
            </div>
            <button class="cart-type-btn" id="cart-type-btn" type="button"></button>
          </div>
          <div class="customer-dropdown hidden" id="customer-dropdown">
            ...
            <div id="customer-search-area">
              <div class="suggestion-list cust-inline-list hidden" id="customer-suggest-list"></div>
              <button type="button" class="customer-add-btn" id="customer-add-btn">+ Thêm khách hàng</button>
            </div>
      ...
      <div class="cart-foot">
        ...
        <div class="cart-disc-row">
          ...
          <div class="cart-disc-ctl">
            ...
            <input id="disc-value" type="text" inputmode="numeric"
              value="${formatNumberInput(Number(cart.discountValue) || 0)}" aria-label="Số tiền chiết khấu" />
          </div>
          <span class="cart-disc-amount" id="disc-amount">0đ</span>
        </div>
        <div class="cart-note-row">
          <input id="order-note" class="cart-note-input" type="text" value="${escapeHtml(cart.note || '')}"
            placeholder="Ghi chú cho đơn (ALT+N)" aria-label="Ghi chú cho đơn" />
          <button class="note-icon-btn" id="note-img-btn" type="button" title="Thêm ảnh đính kèm" aria-label="Thêm ảnh đính kèm">
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" width="16" height="16"><rect x="3" y="3" width="18" height="18" rx="2"/><circle cx="8.5" cy="8.5" r="1.5"/><path d="m21 15-5-5L5 21"/></svg>
          </button>
          <button class="note-icon-btn" id="note-kitchen-btn" type="button" title="In bếp ngay" aria-label="In bếp">
          ...
          <div class="pos-sf-wrap" id="note-more-wrap">
            <button class="note-icon-btn" id="note-more-btn" type="button" title="Thêm tuỳ chọn" aria-label="Thêm tuỳ chọn">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" width="15" height="15"><circle cx="12" cy="5" r="1.5"/><circle cx="12" cy="12" r="1.5"/><circle cx="12" cy="19" r="1.5"/></svg>
            </button>
            <div class="pos-sf-drop note-more-drop hidden" id="note-more-drop">
              <button type="button" class="pos-sf-opt" id="note-opt-kitchen">In bếp</button>
              <button type="button" class="pos-sf-opt" id="note-opt-history">Lịch sử đơn</button>
              <button type="button" class="pos-sf-opt" id="note-opt-sched">Hẹn giao sau</button>
            </div>
          </div>
          <input type="file" id="note-img-file" accept="image/*" class="hidden" aria-hidden="true" />
        </div>
        ...
        <div class="cart-pay-row">
          <button id="cart-print-preview" class="btn cart-preview-btn" type="button" title="In tạm tính (in phiếu trước khi thanh toán)">
            ${icon('in')}<span>In tạm tính</span>
          </button>
  ...
  // Task 2b (13/09/2026): per_item dùng itemCounts (đếm suất từ giỏ hàng) + override qty từ thu ngân.
  function buildItemCounts() {
    const counts = {};
    for (const it of cart.items) {
      const mi = menuData.items.find((m) => m.id === it.menuId);
      if (mi && mi.category) counts[mi.category] = (counts[mi.category] || 0) + (it.qty || 0);
    }
    return counts;
  }
  function surchargeEstimate() {
    const subtotal = itemsSubtotal();
    const itemCounts = buildItemCounts();
    const overrides = cart.surchargeOverrides || {};
    function lineAmount(s) {
      const ov = overrides[s.id] || {};
      if (s.value_type === 'percent') {
        const pct = ov.value != null ? Number(ov.value) : s.value;
        return Math.round((subtotal * pct) / 100);
      }
      if (s.value_type === 'per_item') {
        const unitPrice = ov.value != null ? Number(ov.value) : s.value;
        const qty = ov.qty != null ? Number(ov.qty) : (itemCounts[s.per_item_category] || 0);
        return Math.round(unitPrice * qty);
      }
      // amount
      return Math.round(ov.value != null ? Number(ov.value) : s.value);
    }
  ...
  function renderTabs() {
    ...
    el.innerHTML = state.carts.map((c) => `
      <button class="pos-tab ${c.id === state.activeId ? 'active' : ''}" type="button" data-tab="${c.id}">
        <span class="pos-tab-label">${escapeHtml(cartTabLabel(c, state.carts, tables))}</span>
  ...
  async function closeTab(id) {
    ...
    if (target && target.items.length && !(await confirmDialog(`Đóng "${cartTabLabel(target, state.carts, tables)}"? Món đã chọn trong đơn này sẽ mất.`))) return;
  ...
  // Task 2b (13/09/2026): per_item → tick mở popup qty; amount/percent → tick thêm vào đơn ngay,
  // có icon bút bên cạnh để override giá trị tạm cho đơn này (không đổi cấu hình gốc).
  function renderSurchargesUI() {
    ...
    const itemCounts = buildItemCounts();
    const overrides = cart.surchargeOverrides || {};
    // Luật giao diện: icon SVG, không emoji — dùng cho dấu "đã ghi đè" và 2 nút sửa bên dưới.
    const penIco = `<span class="inline-ico">${icon('but')}</span>`;
    function lineLabel(s) {
      const ov = overrides[s.id] || {};
      if (s.value_type === 'percent') {
        const pct = ov.value != null ? Number(ov.value) : s.value;
        return `${escapeHtml(s.name)} (+${pct}%${ov.value != null ? ' ' + penIco : ''})`;
      }
      if (s.value_type === 'per_item') {
        const unitPrice = ov.value != null ? Number(ov.value) : s.value;
        const qty = ov.qty != null ? Number(ov.qty) : (itemCounts[s.per_item_category] || 0);
        return `${escapeHtml(s.name)} (${formatVND(unitPrice)}×${qty}${ov.qty != null ? ' ' + penIco : ''})`;
      }
      const amt = ov.value != null ? Number(ov.value) : s.value;
      return `${escapeHtml(s.name)} (+${formatVND(amt)}${ov.value != null ? ' ' + penIco : ''})`;
    }
    el.innerHTML = `
      ${autoSurcharges.map((s) => `
        <div class="cart-item-size" style="display:flex;align-items:center;gap:6px">
          <span>${lineLabel(s)} — tự động</span>
          <button data-override-sc="${s.id}" style="background:none;border:none;cursor:pointer;padding:0;font-size:.9em" title="Ghi đè giá trị cho đơn này">${penIco}</button>
        </div>`).join('')}
      ${manualSurcharges.map((s) => `
        <label style="display:flex;align-items:center;gap:8px;margin:4px 0">
          <input type="checkbox" name="surcharge-${s.id}" data-surcharge="${s.id}" style="width:auto;min-height:auto" ${cart.surchargeIds.includes(s.id) ? 'checked' : ''} ${subtotal ? '' : 'disabled'} />
          <span>${lineLabel(s)}</span>
          ${cart.surchargeIds.includes(s.id) ? `<button data-override-sc="${s.id}" style="background:none;border:none;cursor:pointer;padding:0;font-size:.9em" title="Ghi đè giá trị cho đơn này">${penIco}</button>` : ''}
        </label>`).join('')}
    `;
    el.querySelectorAll('[data-surcharge]').forEach((input) => {
      input.addEventListener('change', async () => {
        const id = Number(input.dataset.surcharge);
        const s = manualSurcharges.find((x) => x.id === id);
        if (input.checked && s && s.value_type === 'per_item') {
          // per_item: mở popup hỏi số suất trước khi thêm vào đơn
          const defaultQty = (cart.surchargeOverrides[id]?.qty ?? (itemCounts[s.per_item_category] || 0));
          const raw = await promptDialog(`Số suất "${s.per_item_category || s.name}"`, {
            title: `Phụ thu: ${s.name}`,
            value: String(defaultQty),
            type: 'number', format: true,
            hint: `Đơn giá: ${formatVND(s.value)}/suất. Điền số lượng áp dụng cho đơn này.`,
            okText: 'Thêm',
          });
          if (raw === null) { input.checked = false; return; } // huỷ
          const qty = Math.max(0, parseNumberInput(raw) || 0);
          cart.surchargeIds = [...new Set([...cart.surchargeIds, id])];
          cart.surchargeOverrides = { ...cart.surchargeOverrides, [id]: { ...cart.surchargeOverrides[id], qty } };
        } else if (input.checked) {
          cart.surchargeIds = [...new Set([...cart.surchargeIds, id])];
        } else {
          cart.surchargeIds = cart.surchargeIds.filter((x) => x !== id);
          // Xoá override khi bỏ tích
          const ov = { ...cart.surchargeOverrides };
          delete ov[id];
          cart.surchargeOverrides = ov;
        }
        persist();
        renderCartItems();
      });
    });
    el.querySelectorAll('[data-override-sc]').forEach((btn) => {
      btn.addEventListener('click', async (e) => {
        e.preventDefault();
        const id = Number(btn.dataset.overrideSc);
        const s = [...autoSurcharges, ...manualSurcharges].find((x) => x.id === id);
        if (!s) return;
        const curOv = cart.surchargeOverrides[id] || {};
        if (s.value_type === 'per_item') {
          const curQty = curOv.qty != null ? curOv.qty : (itemCounts[s.per_item_category] || 0);
          const raw = await promptDialog(`Số suất "${s.per_item_category || s.name}"`, {
            title: `Ghi đè: ${s.name}`,
            value: String(curQty),
            type: 'number', format: true,
            hint: `Đơn giá: ${formatVND(s.value)}/suất.`,
            okText: 'Lưu',
          });
          if (raw === null) return;
          cart.surchargeOverrides = { ...cart.surchargeOverrides, [id]: { ...curOv, qty: Math.max(0, parseNumberInput(raw) || 0) } };
        } else {
          // amount / percent: ghi đè giá trị
          const curVal = curOv.value != null ? curOv.value : s.value;
          const raw = await promptDialog(s.value_type === 'percent' ? 'Tỷ lệ % cho đơn này' : 'Số tiền cho đơn này (VNĐ)', {
            title: `Ghi đè: ${s.name}`,
            value: String(curVal),
            type: 'number', format: true,
            hint: 'Chỉ áp dụng cho đơn này, không đổi cấu hình gốc.',
            okText: 'Lưu',
          });
          if (raw === null) return;
          cart.surchargeOverrides = { ...cart.surchargeOverrides, [id]: { ...curOv, value: Math.max(0, parseNumberInput(raw) || 0) } };
        }
        persist();
        renderCartItems();
      });
      ...
      el.innerHTML = `
        <div class="field">
          ...
          <select id="table-select">
            ...
            ${tables.map((t) => `<option value="${t.table_no}" ${Number(cart.tableNo) === t.table_no ? 'selected' : ''}>${escapeHtml(tableName(t))} (${escapeHtml(zoneLabel(t.zone))})</option>`).join('')}
          </select>
        </div>`;
  ...
  function renderCartItems() {
    ...
    if (cart.items.length === 0) {
      // Việc "Website v2" (03/09/2026) — ảnh mẫu Website v2\Bán hàng\POS bán hàng\*: khoảng trống
      // giỏ hàng có TIÊU ĐỀ đậm "Chưa có hàng trong đơn" rồi mới tới câu hướng dẫn, không phải một
      // dòng chữ xám trơn. Câu hướng dẫn viết theo đúng cách app này thêm món (bấm ở lưới bên trái).
      el.innerHTML = `<div class="cart-empty">
        <span class="cart-empty-ico">${icon('ban-hang')}</span>
        <b class="cart-empty-title">Chưa có hàng trong đơn</b>
        <span class="cart-empty-hint">Tìm món ở ô tìm kiếm phía trên hoặc bấm món
          ở lưới bên trái để thêm vào đơn.</span></div>`;
    } else {
  ...
  function renderDiscountUI() {
    ...
    if (!valEl) return;
    if (document.activeElement !== valEl) valEl.value = formatNumberInput(Number(cart.discountValue) || 0);
  ...
  function openAddonModal(menuItem, size, groups, choice) {
    ...
    function optionRow(g, o) {
      ...
      return `<label class="addon-opt-row" style="display:flex;align-items:center;gap:8px;margin:4px 0">
        ...
        ${g.allow_qty ? `<input type="text" inputmode="numeric" value="${qty}" data-qty-g="${g.id}" data-qty-o="${o.id}" style="width:56px" ${isSel ? '' : 'disabled'} />` : ''}
      </label>`;
    }
    ...
    function bindInputs() {
      ...
      modal.overlay.querySelectorAll('[data-qty-g]').forEach((input) => {
        attachNumberInput(input, { decimals: 0 });
        input.addEventListener('input', () => {
          const gid = Number(input.dataset.qtyG);
          const oid = Number(input.dataset.qtyO);
          const n = Math.max(1, parseNumberInput(input.value) || 1);
  ...
  function onItemClick(menuItem) {
    // Task 2 (23/09/2026): ưu tiên size_order (đúng thứ tự chủ quán sắp trong form Sản phẩm) — object
    // size_prices KHÔNG giữ được thứ tự khoá qua Postgres jsonb, nên khi thiếu size_order (dữ liệu cũ)
    // mới rơi về Object.keys().
    const sizeKeys = menuItem.size_prices
      ? (menuItem.size_order || []).filter((s) => menuItem.size_prices[s] != null)
        .concat(Object.keys(menuItem.size_prices).filter((s) => !(menuItem.size_order || []).includes(s)))
      : [];
  ...
  function renderToolbar() {
    // Nút lưới / danh sách: nút nào đang chọn thì sáng lên.
    const gridBtn = container.querySelector('#sell-view-grid');
    const listBtn = container.querySelector('#sell-view-list');
    if (gridBtn) gridBtn.classList.toggle('active', viewMode === 'luoi');
    if (listBtn) listBtn.classList.toggle('active', viewMode !== 'luoi');
  ...
  // Lưới / danh sách — hai nút riêng giống SoBanHang
  ...
  container.querySelector('#sell-view-grid').addEventListener('click', () => setViewMode('luoi'));
  container.querySelector('#sell-view-list').addEventListener('click', () => setViewMode('danh-sach'));
  ...
  // Quét mã vạch: đưa focus vào ô tìm kiếm để dùng với đầu đọc mã vạch USB/Bluetooth
  container.querySelector('#sell-scan-btn').addEventListener('click', () => {
    searchInput.focus();
    searchInput.select();
    toast('Đặt con trỏ sẵn — quét mã bằng súng đọc mã vạch để tìm món', 'info');
  });

  // Menu ⋯ (ba chấm) — gộp phím tắt + đồng bộ + tắt âm vào 1 nơi như SoBanHang
  const SHORTCUTS = [
  ...
  const moreWrap = container.querySelector('#pos-more-wrap');
  const moreDrop = container.querySelector('#sell-more-drop');
  container.querySelector('#sell-more-btn').addEventListener('click', (e) => {
    e.stopPropagation();
    moreDrop.classList.toggle('hidden');
  });
  document.addEventListener('click', (e) => {
    if (moreWrap && !moreWrap.contains(e.target)) moreDrop.classList.add('hidden');
  }, { capture: true, passive: true });
  container.querySelector('#sell-shortcut-opt').addEventListener('click', () => {
  ...
  container.querySelector('#sell-sync-opt').addEventListener('click', async () => {
    moreDrop.classList.add('hidden');
    toast('Đang đồng bộ…');
  ...
  container.querySelector('#sell-mute-opt').addEventListener('click', () => {
    muteAlerts = !muteAlerts;
    const muteBtn = container.querySelector('#sell-mute-opt');
    if (muteBtn) muteBtn.textContent = muteAlerts ? 'Bật âm báo yêu cầu mới' : 'Tắt âm báo yêu cầu mới';
    moreDrop.classList.add('hidden');
    toast(muteAlerts ? 'Đã tắt âm báo yêu cầu mới' : 'Đã bật âm báo yêu cầu mới');
  });
  ...
  // ── Panel tìm kiếm khách hàng (SoBanHang style: dropdown mở khi focus) ─────
  let customerGroupFilter = null;
  ...
  function openCustomerDropdown() { customerDropdown.classList.remove('hidden'); }
  function closeCustomerDropdown() { customerDropdown.classList.add('hidden'); }
  ...
  // Đóng khi click ra ngoài panel khách hàng
  document.addEventListener('click', (e) => {
    const panel = container.querySelector('.customer-panel');
    if (panel && !panel.contains(e.target)) closeCustomerDropdown();
  }, { capture: true, passive: true });

  // Nút "Thêm khách hàng" → mở màn Khách hàng
  container.querySelector('#customer-add-btn')?.addEventListener('click', () => {
    closeCustomerDropdown();
    location.hash = '#/khach-hang';
  });
  ...
  phoneInput.addEventListener('focus', () => openCustomerDropdown());
  ...
  async function doCheckout({ payNow = true } = {}) {
    ...
    // Chỉ đổi CHỮ của <b>, KHÔNG thay innerHTML cả nút: #cart-pay-amount (số tiền) là con của nút, xoá nó đi thì
    // "Lưu đơn (F2)" → clearCart() → renderCartItems() ghi vào phần tử đã biến mất → TypeError sau khi đơn ĐÃ tạo,
    // catch báo "Không tạo được đơn" (đơn có rồi mà thu ngân tưởng chưa → bấm lại thành đơn trùng).
    const payLabel = checkoutBtn.querySelector('b');
    const oldLabel = payLabel.textContent;
    payLabel.textContent = 'Đang tạo đơn…';
    try {
      const res = await api.post('/api/mgr/orders', {
        ...
        surcharge_overrides: Object.keys(cart.surchargeOverrides || {}).length ? cart.surchargeOverrides : undefined,
      ...
      console.error('[sell] tạo/lưu đơn lỗi:', err);
      toast(err?.body?.message || 'Không tạo được đơn', 'error');
    } finally {
      checkoutBtn.disabled = false;
      if (saveBtn) saveBtn.disabled = false;
      payLabel.textContent = oldLabel;
      renderCartItems();
    }
  }
  ...
  attachNumberInput(discValueEl, { decimals: 0 });
  ...
  discValueEl.addEventListener('input', () => {
    ...
    cart.discountValue = Math.max(0, Math.min(max, parseNumberInput(discValueEl.value) || 0));
    persist();
    renderCartItems();
  });
  orderNoteEl.addEventListener('input', () => { cart.note = orderNoteEl.value; persist(); });

  // ── Nút ảnh đính kèm đơn ─────────────────────────────────────────────────
  container.querySelector('#note-img-btn').addEventListener('click', () => {
    container.querySelector('#note-img-file').click();
  });
  container.querySelector('#note-img-file').addEventListener('change', (e) => {
    const file = e.target.files?.[0];
    if (!file) return;
    toast(`Tính năng đính kèm ảnh cho đơn sẽ sớm có — "${file.name}"`, 'info');
    e.target.value = '';
  });

  // ── Nút In bếp ngay ──────────────────────────────────────────────────────
  container.querySelector('#note-kitchen-btn').addEventListener('click', () => {
    if (cart.items.length === 0) { toast('Giỏ hàng đang trống', 'error'); return; }
    doCheckout({ payNow: false });
  });

  // ── Dropdown ⋯ của ghi chú ────────────────────────────────────────────────
  const noteMoreWrap = container.querySelector('#note-more-wrap');
  const noteMoreDrop = container.querySelector('#note-more-drop');
  container.querySelector('#note-more-btn').addEventListener('click', (e) => {
    e.stopPropagation();
    noteMoreDrop.classList.toggle('hidden');
  });
  document.addEventListener('click', (e) => {
    if (noteMoreWrap && !noteMoreWrap.contains(e.target)) noteMoreDrop.classList.add('hidden');
  }, { capture: true, passive: true });
  container.querySelector('#note-opt-kitchen').addEventListener('click', () => {
    noteMoreDrop.classList.add('hidden');
    if (cart.items.length === 0) { toast('Giỏ hàng đang trống', 'error'); return; }
    doCheckout({ payNow: false });
  });
  container.querySelector('#note-opt-history').addEventListener('click', () => {
    noteMoreDrop.classList.add('hidden');
    location.hash = '#/don-hang';
  });
  container.querySelector('#note-opt-sched').addEventListener('click', () => {
    noteMoreDrop.classList.add('hidden');
    const schedEl = container.querySelector('#sched-input');
    if (schedEl) { schedEl.scrollIntoView({ behavior: 'smooth', block: 'center' }); schedEl.focus(); }
  });

  // ── Nút "In tạm tính" — in phiếu xem trước không lưu đơn ─────────────────
  const printPreviewBtn = container.querySelector('#cart-print-preview');
  if (printPreviewBtn) {
    printPreviewBtn.addEventListener('click', () => {
      if (cart.items.length === 0) { toast('Giỏ hàng đang trống', 'error'); return; }
      const lines = cart.items.map((it) =>
        `${it.name}${it.size ? ' (' + it.size + ')' : ''} x${it.qty}    ${formatVND(it.price * it.qty)}`
      ).join('\n');
      const total = cartTotal();
      const disc = manualDiscount();
      const win = window.open('', '_blank', 'width=420,height=640');
      if (!win) { toast('Trình duyệt chặn cửa sổ pop-up — vui lòng cho phép pop-up rồi thử lại', 'error'); return; }
      const style = win.document.createElement('style');
      style.textContent = 'body{font-family:monospace;font-size:14px;padding:16px} h2{font-size:16px;text-align:center} hr{border:none;border-top:1px dashed #888;margin:8px 0} pre{white-space:pre-wrap;margin:0}';
      win.document.head.appendChild(style);
      win.document.title = 'Phiếu tạm tính';
      const body = win.document.body;
      const h2 = win.document.createElement('h2');
      h2.textContent = 'PHIẾU TẠM TÍNH';
      body.appendChild(h2);
      const addHr = () => { const hr = win.document.createElement('hr'); body.appendChild(hr); };
      addHr();
      const dateEl = win.document.createElement('pre');
      dateEl.textContent = new Date().toLocaleString('vi-VN');
      body.appendChild(dateEl);
      addHr();
      const linesEl = win.document.createElement('pre');
      linesEl.textContent = lines;
      body.appendChild(linesEl);
      addHr();
      if (disc > 0) {
        const discEl = win.document.createElement('pre');
        discEl.textContent = `Chiết khấu: -${formatVND(disc)}`;
        body.appendChild(discEl);
      }
      const totalEl = win.document.createElement('pre');
      totalEl.textContent = `TỔNG CỘNG: ${formatVND(total)}`;
      body.appendChild(totalEl);
      addHr();
      const note = win.document.createElement('p');
      note.style.cssText = 'text-align:center;font-size:12px';
      note.textContent = '(Phiếu tạm — chưa thanh toán)';
