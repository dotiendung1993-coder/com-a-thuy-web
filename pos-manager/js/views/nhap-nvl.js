...
import { formatVND, escapeHtml, toast, openModal, promptDialog, pageTabsHtml, formatNumberInput, parseNumberInput, attachNumberInput } from '../ui.js';
...
export async function render(container, { staff } = {}) {
  ...
  container.innerHTML = `
    ...
    <div class="sbh-card-tools">
      ...
      <select id="nvl-dir" style="flex:0 1 140px"><option value="">Tất cả</option><option value="nhap">Nhập</option><option value="xuat">Xuất</option></select>
      <button id="nvl-search" class="btn">Lọc</button>
    </div>
    <div id="nvl-summary-move" style="padding:0 14px 8px"></div>
    <div id="nvl-move-list"><p style="padding:16px">Đang tải…</p></div>
    </div>
  `;
  ...
  async function openMoveModal(defaultDir) {
    ...
    const modal = openModal(`
      ...
      <div class="field"><label>Số lượng *</label>
        <input id="nm-qty" type="text" inputmode="decimal" placeholder="0" /></div>
      ${defaultDir === 'nhap' ? `
      <div class="field"><label>Giá nhập (1 đơn vị)</label>
        <input id="nm-cost" type="text" inputmode="numeric" value="0" /></div>
    ...
    attachNumberInput(ov.querySelector('#nm-qty'), { decimals: 3 });
    if (defaultDir === 'nhap') attachNumberInput(ov.querySelector('#nm-cost'), { decimals: 0 });
    ov.querySelector('#nm-save').addEventListener('click', async () => {
      const ingId = ov.querySelector('#nm-ing').value;
      const qty = parseNumberInput(ov.querySelector('#nm-qty').value);
      ...
      const body = {
        ...
        ingredient_id: ingId,
        qty,
        unit_cost: defaultDir === 'nhap' ? (parseNumberInput(ov.querySelector('#nm-cost').value) || 0) : 0,
