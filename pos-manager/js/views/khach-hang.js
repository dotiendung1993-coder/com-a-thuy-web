...
export async function render(container, { staff } = {}) {
  ...
  container.innerHTML = `
    ...
    <div id="kh-kpi" class="sbh-kpi"></div>

    <div class="sbh-card" style="padding:0">
    <div class="sbh-card-tools">
      <input id="kh-q" class="sbh-card-search" type="search" placeholder="Tìm khách hàng" />
      <div class="sbh-tools-right">
      <input id="kh-from" type="date" aria-label="Từ ngày" style="height:32px;padding:0 6px;border:1px solid var(--border,#ddd);border-radius:6px;font-size:13px" />
      <input id="kh-to" type="date" aria-label="Đến ngày" style="height:32px;padding:0 6px;border:1px solid var(--border,#ddd);border-radius:6px;font-size:13px" />
      <select id="kh-tag" style="height:32px;padding:0 6px;border:1px solid var(--border,#ddd);border-radius:6px;font-size:13px"><option value="">Nhãn</option></select>
      <select id="kh-group" style="height:32px;padding:0 6px;border:1px solid var(--border,#ddd);border-radius:6px;font-size:13px"><option value="">Nhóm khách hàng</option></select>
      <div style="position:relative">
        <button id="kh-col-btn" class="btn pm-col-btn ord-cols-icon" aria-label="Hiển thị cột" title="Hiển thị cột">${icon('cot-hien-thi')}</button>
  ...
  function renderKpi() {
    ...
    el.innerHTML = statKeys.map((k) => {
      ...
      if (!def) return '';
      return `<div class="kpi-card"><div class="kpi-label">${escapeHtml(def[1])}</div><div class="kpi-val">${def[2](summary)}</div></div>`;
    }).join('');
  }
