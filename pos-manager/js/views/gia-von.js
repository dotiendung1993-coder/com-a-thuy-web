...
import { formatVND, escapeHtml, toast, pageTabsHtml, formatNumberInput, parseNumberInput, attachNumberInput } from '../ui.js';
...
function marginHtml(price, costStr) {
  ...
  const cost = parseNumberInput(costStr);
...
export async function render(container, { staff } = {}) {
  ...
  function renderList() {
    ...
    // Món CÓ công thức thì giá vốn TỰ TÍNH từ công thức (máy chủ cập nhật mỗi khi đổi công thức/giá nhập
    // NVL), nên ô bị khoá — gõ đè thì lần sau đổi công thức/giá NVL sẽ ghi đè mất số chủ quán vừa nhập.
    const locked = (it) => it.recipe_count > 0;
    el.innerHTML = `
      ${items.some(locked) ? '<p class="rp-readonly-note">Món đã có công thức: giá vốn tự tính từ công thức và giá nhập nguyên vật liệu nên ô bị khoá. Muốn đổi, sửa công thức (Hàng hoá &gt; Sản phẩm &gt; Sửa &gt; Công thức) hoặc giá nhập ở màn Nguyên vật liệu.</p>' : ''}
      <div class="sbh-card" style="padding:0"><div class="table-scroll">
        <table class="sp-table" style="width:100%;border-radius:0">
          ...
          <tbody>
            ${items.map((it) => `
              <tr data-item-row="${it.id}">
                ...
                <td>
                  <input class="rp-cost-input" type="text" inputmode="numeric" data-cost="${it.id}"
                    value="${formatNumberInput(shownCost(it))}" ${canManage && !locked(it) ? '' : 'disabled'} />
                  ${locked(it) ? `<div class="hint" style="margin:2px 0 0;font-size:12px">Tự tính từ công thức (${it.recipe_count} nguyên vật liệu)</div>` : ''}
                  ${locked(it) && it.recipe_missing_cost ? `<div style="margin:0;font-size:12px;color:var(--warn,#b45309)">Còn ${it.recipe_missing_cost} nguyên vật liệu chưa có giá nhập, giá vốn đang thiếu</div>` : ''}
                </td>
