...
import { escapeHtml, formatVND, toast, openModal, zoneLabel, tableName, formatNumberInput, parseNumberInput, attachNumberInput } from '../ui.js';
  ...
  function renderItems() {
    ...
    el.innerHTML = items.map((it, i) => `
      <div class="cart-item-top" style="border-bottom:1px solid #eee;padding:6px 0;gap:8px">
        ...
        <input type="text" inputmode="numeric" data-pick="${i}" max="${it.qty}" value="${formatNumberInput(picked[i])}"
    ...
    el.querySelectorAll('[data-pick]').forEach((input) => {
      ...
      input.addEventListener('input', () => {
        ...
        let v = Math.floor(parseNumberInput(input.value));
        if (!Number.isFinite(v) || v < 0) v = 0;
        if (v > max) { v = max; input.value = formatNumberInput(max); }
        picked[i] = v;
        renderSummary();
      });
    });
