...
import { escapeHtml, formatVND, toast, openModal, zoneLabel } from '../ui.js';
  ...
  async function loadTables() {
    ...
    try {
      ...
      select.innerHTML = options.length
        ...
        ? options.map((t) => `<option value="${t.table_no}">Bàn ${t.table_no} — ${escapeHtml(zoneLabel(t.zone))}${wantBusy ? ' · ' + formatVND(t.open_total) : ''}</option>`).join('')
