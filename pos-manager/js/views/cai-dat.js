...
import { escapeHtml, formatVND, toast, confirmDialog, openModal, formatNumberInput, parseNumberInput, attachNumberInput } from '../ui.js';
...
export const SETTING_MENU = [
  ...
  { group: 'Tối ưu bán hàng', items: [
    ...
    { key: 'in-phieu-bep', ico: 'in',          label: 'In phiếu bếp',         hint: 'Khi nào in, máy in nhiệt, mẫu phiếu bếp' },
  ...
  { group: 'Cài đặt tin nhắn', items: [
    ...
    { key: 'nhom-telegram',     ico: 'thong-bao', label: 'Nhóm Telegram',   hint: 'Nhóm nhận thông báo đơn mới, xác nhận, bếp' },
  ] },
...
// Task 2 (29/08/2026) — chủ quán: "thêm icon nhỏ cho từng loại hình kinh doanh giống trên
// SoBanHang". Mỗi loại hình một icon + một màu riêng, đặt trong ô vuông bo góc nền nhạt (đúng
// kiểu ảnh mẫu). Để ngay đây thay vì nhét vào js/icons.js: icons.js đánh khoá theo TÊN MÀN
// (route) để cột trái/tab dùng chung, còn 9 icon này chỉ đúng một chỗ này dùng.
// [màu, nội dung <svg>] — nét 1.8px, khung 24×24, tô currentColor như mọi icon khác trong app.
const BTYPE_ICON = {
  'tap-hoa': ['#F2994A', '<path d="M3.5 9.5 5 4.5h14l1.5 5"/><path d="M4.5 9.5v9A1.5 1.5 0 0 0 6 20h12a1.5 1.5 0 0 0 1.5-1.5v-9"/><path d="M3.5 9.5a2.6 2.6 0 0 0 4.2 0 2.6 2.6 0 0 0 4.3 0 2.6 2.6 0 0 0 4.3 0 2.6 2.6 0 0 0 4.2 0"/>'],
  'an-uong': ['#EB5757', '<path d="M6.5 3v6.5a2.5 2.5 0 0 0 5 0V3"/><path d="M9 9.5V21"/><path d="M17.5 3c-1.7 1.2-2.5 3.2-2.5 5.5 0 1.7.8 2.8 2.5 3V21"/>'],
  'my-pham': ['#EB5CA0', '<rect x="8.5" y="9.5" width="7" height="11" rx="1.6"/><path d="M10.5 9.5V6.2a1.7 1.7 0 0 1 1.7-1.7h.6a1.7 1.7 0 0 1 1.7 1.7v3.3"/><path d="M8.5 13.5h7"/>'],
  'giai-tri': ['#9B51E0', '<circle cx="8" cy="17.5" r="2.8"/><path d="M10.8 17.5V6l8-2v10"/><circle cx="16.6" cy="14" r="2.8"/>'],
  'dien-thoai': ['#2D9CDB', '<rect x="7" y="2.8" width="10" height="18.4" rx="2.2"/><path d="M10.6 5.5h2.8"/><path d="M11.4 18.3h1.2"/>'],
  'duoc': ['#27AE60', '<rect x="3.2" y="8.6" width="17.6" height="12.2" rx="2"/><path d="M8.2 8.6V5.4a1.6 1.6 0 0 1 1.6-1.6h4.4a1.6 1.6 0 0 1 1.6 1.6v3.2"/><path d="M12 12.2v5M9.5 14.7h5"/>'],
  'phan-phoi': ['#F2C94C', '<path d="M2.8 6.5h9.4v9.8H2.8z"/><path d="M12.2 10h4l3.2 3v3.3h-7.2z"/><circle cx="6.6" cy="18.4" r="1.9"/><circle cx="16.4" cy="18.4" r="1.9"/>'],
  'san-xuat': ['#56CCF2', '<path d="M3.4 20.4V10l5.4 3.4V10l5.4 3.4V10l6.4 4v6.4z"/><path d="M3.4 10 4.2 3.6h3.4L8.4 10"/>'],
  'khac': ['#828282', '<circle cx="6" cy="12" r="1.7"/><circle cx="12" cy="12" r="1.7"/><circle cx="18" cy="12" r="1.7"/>'],
};

function btypeIcon(key) {
  const [color, body] = BTYPE_ICON[key] || BTYPE_ICON.khac;
  return `<span class="btype-ico" style="--btc:${color}"><svg viewBox="0 0 24 24" fill="none"
...
export async function render(container, { staff, params } = {}) {
  ...
  // Lưu vị trí scroll của sidebar trước khi render lại (tránh nhảy lên đầu khi đổi tab)
  const savedNavScroll = container.querySelector('.set-nav')?.scrollTop || 0;
  ...
  `;
  // Khôi phục vị trí scroll sidebar
  if (savedNavScroll) {
  ...
  else if (sub === 'nhom-telegram') await renderTelegramGroups();
  else if (sub === 'in-phieu-bep') renderKitchenPrint();
  ...
  function bindSave(settingKey, okMsg) {
    ...
    btn.addEventListener('click', async () => {
      ...
      body.querySelectorAll('[data-f]').forEach((elm) => {
        ...
        if (elm.dataset.kind === 'number') { patch[k] = parseNumberInput(elm.value) || 0; return; }
  ...
  function renderRevenueGoal() {
    ...
    body.innerHTML = `
      <div class="field"><label>Mục tiêu doanh thu mỗi tháng (đồng)</label>
        <input id="cd-goal" type="text" inputmode="numeric" value="${formatNumberInput(goal)}" ${canManage ? '' : 'readonly'} /></div>
    ...
    `;
    attachNumberInput(body.querySelector('#cd-goal'), { decimals: 0 });
    if (!canManage) return;
    body.querySelector('#cd-goal-save').addEventListener('click', async () => {
      const value = Math.max(0, parseNumberInput(body.querySelector('#cd-goal').value) || 0);
      try {
      ...
      <div class="field"><label>Loại hình kinh doanh <small>(chọn tối đa 2 loại hình)</small></label>
        <div class="set-list" id="cd-btypes">
          ${BUSINESS_TYPES.map(([v, label]) => `
            <label class="sp-row">
              <span class="sp-text btype-label">${btypeIcon(v)}<span>${escapeHtml(label)}</span></span>
  ...
  function renderLoyalty() {
    ...
    body.innerHTML = `
      ...
      <div class="field"><label>Mỗi X đồng được 1 điểm</label>
        <input id="cd-learn" type="text" inputmode="numeric" value="${formatNumberInput(loyalty.earn_per_point ?? 10000)}" ${canManage ? '' : 'readonly'} /></div>
      <div class="field"><label>1 điểm = X đồng khi đổi</label>
        <input id="cd-lvalue" type="text" inputmode="numeric" value="${formatNumberInput(loyalty.redeem_value ?? 1000)}" ${canManage ? '' : 'readonly'} /></div>
      <div class="field"><label>Số điểm tối thiểu để đổi</label>
        <input id="cd-lmin" type="text" inputmode="numeric" value="${formatNumberInput(loyalty.min_redeem_points ?? 10)}" ${canManage ? '' : 'readonly'} /></div>
      <label style="display:flex;align-items:center;gap:8px;margin:8px 0">
    ...
    `;
    if (!canManage) return;
    attachNumberInput(body.querySelector('#cd-learn'), { decimals: 0 });
    attachNumberInput(body.querySelector('#cd-lvalue'), { decimals: 0 });
    attachNumberInput(body.querySelector('#cd-lmin'), { decimals: 0 });
    body.querySelector('#cd-save-loyalty').addEventListener('click', async () => {
      try {
        await api.patch('/api/mgr/settings/loyalty', {
          ...
          earn_per_point: parseNumberInput(body.querySelector('#cd-learn').value) || 10000,
          redeem_value: parseNumberInput(body.querySelector('#cd-lvalue').value) || 1000,
          min_redeem_points: parseNumberInput(body.querySelector('#cd-lmin').value) || 10,
  ...
  function renderInvoiceForm() {
    ...
    const PAPER = { k80: 'Khổ K80 (giấy nhiệt 80mm)', k58: 'Khổ K58 (giấy nhiệt 58mm)', a5: 'Khổ A5', a4: 'Khổ A4' };
    const BANKS = { vcb: 'Vietcombank', tcb: 'Techcombank', acb: 'ACB', mb: 'MB Bank', vtb: 'Vietinbank', bidv: 'BIDV' };
    const SEP = { dashed: 'Nét đứt (– – –)', double: 'Nét đôi (═══)', dotted: 'Chấm chấm (· · ·)' };
    ...
    // Task 1c (11/09/2026) — vị trí chèn + kiểu hiển thị cho "dòng phân cách tuỳ ý" (khác
    // separator_style: cái đó áp 1 kiểu cho MỌI đường kẻ có sẵn, cái này chèn thêm dòng RIÊNG ở
    // vị trí chủ quán chọn trong "Thông tin chung"). Lưu mỗi dòng "vi_tri:kieu" — xem sepLine().
    const SEP_POS = {
      after_logo: 'Sau logo', after_name: 'Sau tên cửa hàng', after_tagline: 'Sau khẩu hiệu',
      after_address: 'Sau địa chỉ', after_phone: 'Sau số điện thoại',
    };
    const SEP_STYLE_LABEL = {
      dashed: 'Nét đứt (– – –)', spaced: 'Gạch thưa (—   —   —)',
      dotted: 'Chấm chấm (· · ·)', double: 'Nét đôi (═══)',
    };
    body.innerHTML = `
      <div style="display:flex;gap:16px;align-items:flex-start;flex-wrap:wrap">
        <div style="flex:1;min-width:260px">
          ...
          <div class="sp-group">
            ...
            <!-- Task 3 (10/09/2026, hoàn thiện) — upload logo THẬT thay vì chỉ công tắc ẩn/hiện.
                 Ảnh tự thu nhỏ (canvas, tối đa 200×200, JPEG) rồi lưu base64 vào data-f="logo_base64"
                 (input ẩn) — tái dùng nguyên vẹn cơ chế thu patch/preview sẵn có của màn này, không
                 cần sửa hàm on()/nút Lưu. -->
            <div class="field">
              <div style="display:flex;align-items:center;gap:10px">
                <img id="cd-logo-preview" src="${escapeHtml(inv.logo_base64 || '')}"
                     style="width:44px;height:44px;border-radius:50%;object-fit:cover;background:#eee;${inv.logo_base64 ? '' : 'display:none'}" alt="Logo" />
                <input id="cd-logo-file" type="file" accept="image/*" ${canManage ? '' : 'disabled'} />
              </div>
              <input type="hidden" data-f="logo_base64" value="${escapeHtml(inv.logo_base64 || '')}" />
              <small>Ảnh tự thu nhỏ trước khi lưu. Bấm "Lưu mẫu hoá đơn" để lưu logo mới.</small>
            </div>
            ${chk('show_title', 'Tiêu đề', '', true)}
            <div class="field">
              <input data-f="title_text" type="text" placeholder="HÓA ĐƠN THANH TOÁN"
                value="${escapeHtml(inv.title_text || '')}" ${canManage ? '' : 'readonly'} />
            </div>
            <!-- Task 3 (10/09/2026) — khẩu hiệu (tagline) dưới tên quán + địa chỉ dòng 2, theo ảnh
                 hoá đơn mẫu. 2 field text riêng của Mẫu hoá đơn — KHÔNG lấy từ màn Thông tin quán
                 (store.address vẫn nguyên là dòng 1, hành vi cũ không đổi). -->
            ${chk('show_tagline', 'Khẩu hiệu (tagline)', 'Dòng nhỏ in nghiêng dưới tên quán')}
            <div class="field">
              <input data-f="tagline_text" type="text" placeholder="VD: Cơm gia đình & Ăn ngon mỗi ngày"
                value="${escapeHtml(inv.tagline_text || '')}" ${canManage ? '' : 'readonly'} />
            </div>
            ${chk('show_store_phone', 'Số điện thoại cửa hàng', '')}
            <div class="field">
              <input data-f="store_phone_text" type="text"
                placeholder="${escapeHtml(storeInfo.phone || 'VD: 0901234567')}"
                value="${escapeHtml(inv.store_phone_text || '')}" ${canManage ? '' : 'readonly'} />
              <small>Để trống để dùng SĐT ở màn Thông tin cửa hàng.</small>
            </div>
            ${chk('show_store_address', 'Địa chỉ cửa hàng', '', true)}
            <div class="field">
              <textarea data-f="store_address_text" rows="2" ${canManage ? '' : 'readonly'}
                placeholder="${escapeHtml(storeInfo.address || 'VD: 123 Đường ABC, Quận 1')}"
              >${escapeHtml(inv.store_address_text || '')}</textarea>
              <small>Xuống dòng ở đâu, hoá đơn in đúng ở đó.</small>
            </div>
            <div class="field">
              <label>Dòng phân cách tuỳ ý</label>
              <div id="cd-sep-list"></div>
              ${canManage ? `
              <div style="display:flex;gap:6px;flex-wrap:wrap;margin-top:6px;align-items:center">
                <select id="cd-sep-pos" style="flex:1;min-width:140px">
                  ${Object.entries(SEP_POS).map(([v, t]) => `<option value="${v}">${escapeHtml(t)}</option>`).join('')}
                </select>
                <select id="cd-sep-style" style="flex:1;min-width:120px">
                  ${Object.entries(SEP_STYLE_LABEL).map(([v, t]) => `<option value="${v}">${escapeHtml(t)}</option>`).join('')}
                </select>
                <button type="button" id="cd-sep-add" class="btn btn-ghost">+ Thêm dòng phân cách</button>
              </div>` : ''}
              <input type="hidden" data-f="custom_separators" value="${escapeHtml((inv.custom_separators || []).join('\n'))}" />
            </div>
          </div>
          <div class="sp-group">
          ...
          <div class="sp-group">
            ...
            ${chk('show_unit_price', 'Cột đơn giá trong bảng món', '')}
          </div>
          <div class="sp-group">
            <div class="sp-group-title">Thông tin thanh toán</div>
            ${chk('show_packaging_fee', 'Phí đóng gói', 'Hiện dòng phí đóng gói riêng trên hoá đơn', true)}
          ...
          <div class="sp-group">
            ...
            <div class="field">
              ...
              <input data-f="store_bank_account_no" type="text" placeholder="VD: 0123456789"
                value="${escapeHtml(inv.store_bank_account_no || '')}" ${canManage ? '' : 'readonly'} />
            </div>
            <div class="field">
              <label>Tên chủ tài khoản</label>
              <input data-f="store_bank_account_name" type="text" placeholder="VD: NGUYEN VAN A"
                value="${escapeHtml(inv.store_bank_account_name || '')}" ${canManage ? '' : 'readonly'} />
              <small style="color:#d97706;font-weight:600">Lưu ý: đổi số này sẽ đổi luôn mã QR khách quét để trả tiền — kiểm tra kỹ số tài khoản trước khi lưu.</small>
            </div>
            ${chk('show_store_name', 'Tên cửa hàng', '')}
            <div class="field">
              <input data-f="store_name_override" type="text"
                placeholder="${escapeHtml(storeInfo.name || 'Cơm A Thuý')}"
                value="${escapeHtml(inv.store_name_override || '')}" ${canManage ? '' : 'readonly'} />
              <small>Để trống để dùng tên mặc định của quán.</small>
            </div>
            ${chk('show_payment_method', 'Hiển thị thanh toán tiền mặt và chuyển khoản', '')}
            <div class="field">
              <label>Chân hoá đơn</label>
              <textarea data-f="footer_text" rows="2" placeholder="Cảm ơn quý khách — Hẹn gặp lại!" ${canManage ? '' : 'readonly'}>${escapeHtml(inv.footer_text || '')}</textarea>
            </div>
          ...
          <div class="sp-group">
            ...
            <div class="field">
              <label>Kiểu gạch ngang</label>
              <select data-f="separator_style" ${canManage ? '' : 'disabled'}>
                ${Object.entries(SEP).map(([v, t]) =>
                  `<option value="${v}" ${(inv.separator_style || 'dashed') === v ? 'selected' : ''}>${escapeHtml(t)}</option>`).join('')}
              </select>
            </div>
          </div>
          <div style="display:flex;gap:8px;flex-wrap:wrap">
            ${canManage ? '<button id="cd-save-inv" class="btn btn-primary">Lưu mẫu hoá đơn</button>' : readonlyNote()}
            <button id="cd-test-print-inv" type="button" class="btn btn-ghost">In thử</button>
          </div>
        </div>
        <div id="cd-inv-preview" style="flex:1;min-width:220px;max-width:320px;border:1px solid #ccc;border-radius:6px;padding:12px;font-family:monospace;font-size:12px;line-height:1.6;background:#fff;align-self:flex-start;position:sticky;top:16px;max-height:calc(100vh - 80px);overflow-y:auto"></div>
      </div>
    `;

    // Đọc giá trị đang gõ trên form (kể cả CHƯA lưu) — dùng chung cho preview lẫn nút "In thử" nên
    // cả hai luôn khớp với những gì chủ quán vừa chỉnh, khỏi phải bấm Lưu trước mới xem được.
    function on(k) {
    ...
    // Task 3 (10/09/2026) — dựng THÂN hoá đơn (không kèm khung/khổ giấy) bằng dữ liệu giả, dùng
    // chung cho khung xem trước nhỏ trong màn Cài đặt VÀ cửa sổ "In thử" bên dưới. Thứ tự theo
    // đúng ảnh mẫu chủ quán gửi: logo → tên quán → khẩu hiệu → địa chỉ (2 dòng) → SĐT → tiêu đề
    // hoá đơn → thông tin đơn → bảng món → phí đóng gói → tổng tiền → thanh toán/QR → chân hoá đơn.
    // Kiểu gạch ngang (Task 3 Đợt E, 10/09/2026) — 1 nơi tính CSS border dùng chung cho mọi đường
    // kẻ phân cách trong thân hoá đơn, đổi theo lựa chọn "Kiểu gạch ngang" của chủ quán.
    function sepCss(side, color) {
      const style = on('separator_style') || 'dashed';
      const width = style === 'double' ? '3px' : '1px';
      return `border-${side}:${width} ${style} ${color};`;
    }
    // Task 1c (11/09/2026) — đọc custom_separators (mảng "vi_tri:kieu") từ input ẩn qua on(), rồi
    // trả HTML của các dòng phân cách khớp 1 vi_tri, chèn ngay tại điểm gọi trong buildInvoiceBody().
    function customSeparators() {
      // on() trả CHUỖI (input ẩn, mỗi dòng "vi_tri:kieu") — nhưng khi không tìm thấy input nó rơi về giá trị
      // đã lưu của cài đặt, vốn là MẢNG (inv.custom_separators) → phải chịu được cả hai, không thì .split nổ.
      const raw = on('custom_separators') || '';
      return (Array.isArray(raw) ? raw : raw.split('\n')).filter(Boolean).map((s) => {
        const i = s.indexOf(':');
        return i < 0 ? { pos: s, style: 'dashed' } : { pos: s.slice(0, i), style: s.slice(i + 1) };
      });
    }
    function sepLine(posKey) {
      return customSeparators().filter((s) => s.pos === posKey).map((s) => {
        if (s.style === 'spaced') return `<div style="text-align:center;letter-spacing:4px;color:#999;margin:3px 0">— — — —</div>`;
        const cssStyle = ['dashed', 'double', 'dotted'].includes(s.style) ? s.style : 'dashed';
        const width = cssStyle === 'double' ? '3px' : '1px';
        return `<div style="margin:3px 0;border-top:${width} ${cssStyle} #999"></div>`;
      }).join('');
    }
    function buildInvoiceBody() {
      ...
      let html = '';
      if (on('show_logo')) {
        const logoSrc = on('logo_base64');
        html += logoSrc
          ? `<div style="text-align:center;margin-bottom:4px"><img src="${logoSrc}" alt="" style="width:56px;height:56px;border-radius:50%;object-fit:cover"></div>`
          : `<div style="text-align:center;margin-bottom:4px"><div style="width:56px;height:56px;border-radius:50%;background:#eee;display:inline-flex;align-items:center;justify-content:center;font-size:.692em;color:#999">Logo</div></div>`;
      }
      html += sepLine('after_logo');
      if (on('show_store_name')) html += `<div style="text-align:center;font-weight:bold;font-size:1.077em">${escapeHtml(on('store_name_override') || store)}</div>`;
      html += sepLine('after_name');
      if (on('show_tagline') && on('tagline_text')) html += `<div style="text-align:center;font-style:italic;font-size:.846em">${escapeHtml(on('tagline_text'))}</div>`;
      html += sepLine('after_tagline');
      if (on('show_store_address')) {
        const addrText = on('store_address_text') || storeInfo.address || '';
        if (addrText) addrText.split('\n').forEach(function(line) {
          if (line.trim()) html += `<div style="text-align:center;font-size:.846em">${escapeHtml(line)}</div>`;
        });
      }
      html += sepLine('after_address');
      // Task 1b (11/09/2026) — SĐT riêng cho hoá đơn (store_phone_text); để trống thì lùi về SĐT
      // quán (storeInfo.phone, hành vi cũ không đổi).
      const invoicePhone = on('store_phone_text') || storeInfo.phone;
      if (on('show_store_phone') && invoicePhone) html += `<div style="text-align:center;font-size:.846em">ĐT: ${escapeHtml(invoicePhone)}</div>`;
      html += sepLine('after_phone');
      if (on('show_title')) {
        html += `<div style="text-align:center;font-weight:bold;font-size:1em;margin:6px 0">` +
          `<span style="display:inline-block;width:20px;${sepCss('top', '#333')}vertical-align:middle;margin-right:6px"></span>` +
          `${escapeHtml(on('title_text') || 'HÓA ĐƠN THANH TOÁN')}` +
          `<span style="display:inline-block;width:20px;${sepCss('top', '#333')}vertical-align:middle;margin-left:6px"></span></div>`;
      }
      // Task 3 (10/09/2026, hoàn thiện) — khối "định danh đơn" (loại đơn/mã đơn/giờ in) LUÔN hiện,
      // không có công tắc riêng — giống bill-print.js::receiveWhereLine (mẫu thật luôn in dòng
      // này). Xếp 2 cột: TRÁI = thông tin đơn, PHẢI = thông tin khách hàng (theo công tắc) —
      // đúng bố cục ảnh mẫu chủ quán gửi (Mang về | KH:... / Mã đơn:... | ĐT:... / giờ).
      const nowStr = new Date().toLocaleString('vi-VN');
      // \u{1F961} = emoji hộp mang về: đây là NỘI DUNG dòng hoá đơn (khớp đúng chữ hoá đơn thật ở bill-print.js
      // receiveWhereLine), không phải icon giao diện — viết bằng escape để luật "giao diện không emoji" vẫn quét sạch.
      let orderLeft = `<div>\u{1F961} Mang về</div><div>Mã đơn: MV0001</div><div>${nowStr}</div>`;
      if (on('order_table')) orderLeft += `<div>Bàn: Bàn 5</div>`;
      if (on('order_barcode')) orderLeft += `<div>[|||||||||||]</div>`;
      if (on('order_staff')) orderLeft += `<div>NV: Nhân Viên</div>`;
      if (on('order_service_time')) orderLeft += `<div>Giờ phục vụ: 12:30</div>`;
      if (on('order_note')) orderLeft += `<div>Ghi chú: Không hành, ít cay</div>`;
      let custRight = '';
      if (on('cust_name')) custRight += `<div>KH: Nguyễn Văn A</div>`;
      if (on('cust_code')) custRight += `<div>Mã KH: KH0001</div>`;
      if (on('cust_phone')) custRight += `<div>ĐT: 0901234567</div>`;
      if (on('cust_points')) custRight += `<div>Điểm tích luỹ: 120 điểm</div>`;
      if (on('cust_address')) custRight += `<div>Địa chỉ: 123 Đường ABC</div>`;
      html += `<div style="display:flex;justify-content:space-between;gap:8px;font-size:.846em;${sepCss('bottom', '#999')}padding-bottom:4px;margin-bottom:4px">`;
      html += `<div>${orderLeft}</div><div style="text-align:right">${custRight}</div>`;
      html += `</div>`;
      const showUnit = on('show_unit_price');
      html += `<table style="width:100%;border-collapse:collapse;font-size:.846em">`;
      html += `<tr style="font-weight:bold;border-bottom:1px solid #999"><td>Món</td>${showUnit ? '<td style="text-align:right">Đơn giá</td>' : ''}<td style="text-align:right">SL</td><td style="text-align:right">Tiền</td></tr>`;
      html += `<tr><td>Cà phê sữa</td>${showUnit ? '<td style="text-align:right">25.000</td>' : ''}<td style="text-align:right">2</td><td style="text-align:right">50.000</td></tr>`;
      if (on('item_discount')) html += `<tr><td colspan="${showUnit ? 3 : 2}" style="padding-left:8px;color:#888">  Giảm</td><td style="text-align:right;color:#888">-5.000</td></tr>`;
      html += `<tr><td>Bánh mì thịt</td>${showUnit ? '<td style="text-align:right">20.000</td>' : ''}<td style="text-align:right">1</td><td style="text-align:right">20.000</td></tr>`;
      html += `</table>`;
      if (on('show_packaging_fee')) html += `<div style="font-size:.846em;color:#888;margin-top:4px">Phí đóng gói: 2.000đ × 2 = 4.000đ</div>`;
      html += `<div style="${sepCss('top', '#999')}margin-top:4px;padding-top:4px;font-size:.846em">`;
      ...
      if (on('show_payment_method')) html += `<div style="color:#888;font-size:.769em">Tiền mặt / Chuyển khoản</div>`;
      html += `</div>`;
      if (on('bank_account') || on('show_qr')) {
        html += `<div style="${sepCss('top', '#999')}margin-top:4px;padding-top:4px;font-size:.846em;text-align:center">`;
        if (on('bank_account')) {
          const acctNo = on('store_bank_account_no') || '(chưa nhập số tài khoản)';
          const acctName = on('store_bank_account_name');
          html += `<div>TK: ${escapeHtml(acctNo)} - ${escapeHtml(on('qr_bank') || 'VCB')}</div>`;
          if (acctName) html += `<div>${escapeHtml(acctName)}</div>`;
        }
        if (on('show_qr')) {
          html += `<div style="margin:4px auto;width:60px;height:60px;background:#eee;display:flex;align-items:center;justify-content:center;font-size:.692em">QR</div>`;
          html += `<div style="font-size:.769em;color:#888">Quét mã để chuyển khoản</div>`;
        }
        html += `</div>`;
      }
      const footer = on('footer_text');
      if (footer) html += `<div style="text-align:center;font-size:.769em;color:#888;margin-top:4px;${sepCss('top', '#999')}padding-top:4px">${escapeHtml(footer)}</div>`;
      if (on('show_thanks')) html += `<div style="text-align:center;font-size:.846em;margin-top:4px">Cảm ơn quý khách và hẹn gặp lại!</div>`;
      return html;
    }

    // Task 1a (11/09/2026, fix) — bản cũ tính `scale` (%) áp vào DIV bọc ngoài, nhưng mọi phần tử
    // con trong buildInvoiceBody() dùng px TUYỆT ĐỐI nên % của cha không ảnh hưởng gì tới con —
    // chọn cỡ chữ không đổi được gì trên preview. Nay buildInvoiceBody() dùng em (mốc 13px = base
    // "vừa", xem các `font-size:.846em` v.v.) nên chỉ cần đặt đúng px THẬT (11/13/15) ở đây.
    function invoiceBoxSize() {
      const paper = on('paper_size') || 'k80';
      const fontSize = on('font_size') || 'vua';
      const fontPx = fontSize === 'nho' ? 11 : fontSize === 'lon' ? 15 : 13;
      const width = paper === 'k58' ? 200 : paper === 'a5' ? 300 : paper === 'a4' ? 380 : 240;
      return { width, fontPx };
    }
    ...
    function drawInvoicePreview() {
      ...
      if (!preview) return;
      const { width, fontPx } = invoiceBoxSize();
      preview.innerHTML = `<div style="width:${width}px;margin:0 auto;font-size:${fontPx}px">${buildInvoiceBody()}</div>`;
    }

    // Task 3 (10/09/2026, hoàn thiện) — thu nhỏ ảnh logo TRƯỚC khi lưu (canvas, tối đa 200×200,
    // JPEG q=0.82): tránh lưu ảnh gốc vài MB vào cột JSONB pos_settings (chậm tải mọi màn Cài đặt
    // đọc settings.invoice_form) và tránh vượt giới hạn body JSON (xem server.js, đã nới riêng
    // route này lên 2mb làm lưới an toàn thứ hai — ảnh thu nhỏ đúng cách sẽ chỉ vài chục KB).
    ...
    // Task 3 (10/09/2026) — "In thử": mở cửa sổ mới, dựng CÙNG dữ liệu giả + đúng khổ giấy/cỡ chữ
    // đang chọn trên form (kể cả chưa bấm Lưu), rồi gọi print() để chủ quán xem hoá đơn thật sẽ
    // trông ra sao trước khi in cho khách. Không đụng luồng in hoá đơn thật (bill-print.js) — đây
    // chỉ là bản xem trước bằng dữ liệu giả, tách biệt hoàn toàn với đơn hàng thật.
    function openTestPrint() {
      const { width, fontPx } = invoiceBoxSize();
      const win = window.open('', '_blank', 'width=420,height=640');
      if (!win) { toast('Trình duyệt chặn cửa sổ in — cho phép popup rồi bấm lại "In thử"', 'error'); return; }
      win.document.write(`<!doctype html><html><head><meta charset="utf-8"><title>In thử hoá đơn</title>
        <style>body{margin:16px;font-family:monospace;background:#fff} @media print { body{margin:0} }</style>
        </head><body><div style="width:${width}px;margin:0 auto;font-size:${fontPx}px">${buildInvoiceBody()}</div></body></html>`);
      win.document.close();
      win.focus();
      win.print();
    }

    // Task 1c (11/09/2026) — danh sách "dòng phân cách tuỳ ý": nguồn sự thật DUY NHẤT là input ẩn
    // data-f="custom_separators" (giống pattern logo_base64) — thêm/xoá chỉ đọc-sửa-ghi lại value
    // của nó rồi vẽ lại, không giữ mảng JS riêng để khỏi lệch trạng thái.
    function drawSepList() {
      const box = body.querySelector('#cd-sep-list');
      if (!box) return;
      const list = customSeparators();
      box.innerHTML = list.length ? list.map((s, idx) => `
        <div class="sp-row" style="padding:4px 0">
          <span class="sp-text">${escapeHtml(SEP_POS[s.pos] || s.pos)} — ${escapeHtml(SEP_STYLE_LABEL[s.style] || s.style)}</span>
          ${canManage ? `<button type="button" data-rm-sep="${idx}" class="btn-ghost" style="padding:2px 8px">×</button>` : ''}
        </div>`).join('') : '<p class="hint">Chưa có dòng phân cách nào.</p>';
      box.querySelectorAll('[data-rm-sep]').forEach((btn) => {
        btn.addEventListener('click', () => {
          const list2 = customSeparators();
          list2.splice(Number(btn.dataset.rmSep), 1);
          const hidden = body.querySelector('[data-f="custom_separators"]');
          if (hidden) hidden.value = list2.map((s) => `${s.pos}:${s.style}`).join('\n');
          drawSepList();
          drawInvoicePreview();
        });
      });
    ...
    body.querySelector('#cd-test-print-inv').addEventListener('click', openTestPrint);
    const sepAddBtn = body.querySelector('#cd-sep-add');
    if (sepAddBtn) sepAddBtn.addEventListener('click', () => {
      const list = customSeparators();
      list.push({ pos: body.querySelector('#cd-sep-pos').value, style: body.querySelector('#cd-sep-style').value });
      const hidden = body.querySelector('[data-f="custom_separators"]');
      if (hidden) hidden.value = list.map((s) => `${s.pos}:${s.style}`).join('\n');
      drawSepList();
      drawInvoicePreview();
    });

    const logoFileInput = body.querySelector('#cd-logo-file');
    if (logoFileInput) {
      logoFileInput.addEventListener('change', async () => {
        ...
        try {
          ...
          body.querySelector('[data-f="logo_base64"]').value = dataUrl;
          const preview = body.querySelector('#cd-logo-preview');
          preview.src = dataUrl;
          preview.style.display = '';
          drawInvoicePreview();
        } catch (err) {
          toast('Không đọc được ảnh — thử ảnh khác', 'error');
        }
      });
    }
  ...
  // ── Phụ thu ──────────────────────────────────────────────────────────────
  // Task 2 (11/09/2026) — Áp dụng cho: lọc phụ thu TỰ ĐỘNG theo loại đơn (migration 092). Khớp
  // NGUYÊN VĂN delivery_type đã có ('tai-ban'|'mang-ve'|'giao-hang') + 'all' — xem surcharge-service.js.
  // Task 1 (13/09/2026) — renderSurcharges: bỏ inline edit (đổi sang modal), bỏ #cd-pkgfee block
  // (phí đóng gói gộp vào per_item surcharge), thêm per_item option + per_item_category field.
  // Task 1a: dùng confirmDialog() thay confirm() (bug-565 TDZ: hàm phụ TRONG render PHẢI là function,
  // KHÔNG const — hàm này đứng ngoài render nên OK khi khai function cùng cấp).
  function renderSurcharges() {
    // BUG phát hiện 12/09/2026 khi tự kiểm bằng chrome-devtools: khai APPLIES_TO bằng const Ở
    // NGOÀI renderSurcharges() (cùng hàng với renderInvoiceForm) bị TDZ y hệt bug-565 — nhánh
    // `if (sub === 'phu-thu') { await loadSurcharges(); return; }` ở ĐẦU hàm cha chạy TỚI đây
    // và gọi renderSurcharges() TRƯỚC KHI dòng const APPLIES_TO (nằm SAU trong file) kịp chạy ->
    // ReferenceError nuốt trong try/catch của loadSurcharges() -> "Không tải được danh sách phụ
    // thu." dù API trả 200 đúng dữ liệu. Sửa: khai bên TRONG hàm, giống FONT/BANKS/SEP đã làm
    // trong renderInvoiceForm() — luôn khởi tạo lại mỗi lần hàm chạy, không phụ thuộc thứ tự file.
    const APPLIES_TO = { all: 'Tất cả đơn', 'tai-ban': 'Ăn tại quán', 'mang-ve': 'Mang về', 'giao-hang': 'Giao hàng' };
    function scValueLabel(s) {
      if (s.value_type === 'percent') return `${s.value}%`;
      if (s.value_type === 'per_item') return `${formatVND(s.value)}/suất${s.per_item_category ? ' (' + escapeHtml(s.per_item_category) + ')' : ''}`;
      return formatVND(s.value);
    }
    body.innerHTML = `
        ...
        ${surcharges.length ? surcharges.map((s) => `
          <div class="stock-row ${s.active ? '' : 'inactive'}">
            <div class="stock-main">
              ...
              <div class="stock-meta">${scValueLabel(s)}${s.code ? ' · Mã: ' + escapeHtml(s.code) : ''} · ${escapeHtml(APPLIES_TO[s.applies_to] || APPLIES_TO.all)}</div>
            </div>
            ${canManage ? `<div class="stock-actions">
              <button data-edit-sc="${s.id}">Sửa</button>
              <button data-toggle-sc="${s.id}">${s.active ? 'Tắt' : 'Bật'}</button>
              <button data-delete-sc="${s.id}" style="color:#c00;margin-left:6px" title="Xoá phụ thu này">Xoá</button>
            </div>` : ''}
      ...
      ${canManage ? `
        ...
        <div class="field"><label>Loại &amp; Giá trị</label>
          <div style="display:flex;gap:6px">
            <select id="sc-type" style="flex:0 0 auto;width:42%">
              <option value="amount">VNĐ cố định</option>
              <option value="percent">Phần trăm %</option>
              <option value="per_item">Theo số suất</option>
            </select>
            <input id="sc-value" type="text" inputmode="numeric" value="0" style="flex:1" />
          </div>
        </div>
        <div class="field" id="sc-category-wrap" style="display:none">
          <label>Danh mục đếm suất</label>
          <input id="sc-category" type="text" placeholder="VD: Cơm phần" />
          <small>Nhập ĐÚNG tên danh mục trong thực đơn. Phụ thu = giá trị × số suất thuộc danh mục này.</small>
        </div>
        <div class="field"><label>Áp dụng cho</label>
          <select id="sc-applies">
            ${Object.entries(APPLIES_TO).map(([v, t]) => `<option value="${v}">${escapeHtml(t)}</option>`).join('')}
          </select>
        </div>
        <label style="display:flex;align-items:center;gap:8px;margin:8px 0">
    ...
    // Hiện/ẩn trường Danh mục khi đổi loại phụ thu (form Thêm mới)
    body.querySelector('#sc-type').addEventListener('change', function () {
      body.querySelector('#sc-category-wrap').style.display = this.value === 'per_item' ? '' : 'none';
    });
    ...
    body.querySelectorAll('[data-delete-sc]').forEach((btn) => {
      btn.addEventListener('click', async () => {
        const s = surcharges.find((x) => String(x.id) === btn.dataset.deleteSc);
        if (!s) return;
        // Task 1a: dùng confirmDialog() thay confirm() native
        const ok = await confirmDialog(`Xoá phụ thu "${s.name}"?\nHành động này không thể hoàn tác.`, { danger: true, okText: 'Xoá' });
        if (!ok) return;
        try {
          // bug-566 tái phát (11/09/2026): api.js chỉ có api.del(), KHÔNG có api.delete() — gọi
          // sai tên bị TypeError nuốt trong catch, nút Xoá bấm hoài không ăn mà không rõ vì sao.
          await api.del(`/api/mgr/surcharges/${s.id}`);
          toast('Đã xoá phụ thu');
          await loadSurcharges();
        } catch (err) { toast(err?.body?.message || 'Không xoá được', 'error'); }
      });
    });
    // Task 1b: Sửa mở modal thay inline form
    body.querySelectorAll('[data-edit-sc]').forEach((btn) => {
      btn.addEventListener('click', () => {
        const s = surcharges.find((x) => String(x.id) === btn.dataset.editSc);
        if (!s) return;
        const APPLIES_TO_EDIT = { all: 'Tất cả đơn', 'tai-ban': 'Ăn tại quán', 'mang-ve': 'Mang về', 'giao-hang': 'Giao hàng' };
        const { overlay, close } = openModal(`
          <h3 style="margin:0 0 14px">Sửa phụ thu</h3>
          <div class="field"><label>Tên phụ thu</label><input id="sc-edit-name" type="text" value="${escapeHtml(s.name)}" /></div>
          <div class="field"><label>Mã (tuỳ chọn)</label><input id="sc-edit-code" type="text" value="${escapeHtml(s.code || '')}" /></div>
          <div class="field"><label>Loại &amp; Giá trị</label>
            <div style="display:flex;gap:6px">
              <select id="sc-edit-type" style="flex:0 0 auto;width:42%">
                <option value="amount" ${s.value_type === 'amount' ? 'selected' : ''}>VNĐ cố định</option>
                <option value="percent" ${s.value_type === 'percent' ? 'selected' : ''}>Phần trăm %</option>
                <option value="per_item" ${s.value_type === 'per_item' ? 'selected' : ''}>Theo số suất</option>
              </select>
              <input id="sc-edit-value" type="text" inputmode="numeric" value="${formatNumberInput(s.value)}" style="flex:1" />
            </div>
          </div>
          <div class="field" id="sc-edit-category-wrap" style="display:${s.value_type === 'per_item' ? '' : 'none'}">
            <label>Danh mục đếm suất</label>
            <input id="sc-edit-category" type="text" value="${escapeHtml(s.per_item_category || '')}" placeholder="VD: Cơm phần" />
            <small>Nhập ĐÚNG tên danh mục trong thực đơn.</small>
          </div>
          <div class="field"><label>Áp dụng cho</label>
            <select id="sc-edit-applies">
              ${Object.entries(APPLIES_TO_EDIT).map(([v, t]) => `<option value="${v}" ${(s.applies_to || 'all') === v ? 'selected' : ''}>${escapeHtml(t)}</option>`).join('')}
            </select>
          </div>
          <label style="display:flex;align-items:center;gap:8px;margin:8px 0">
            <input id="sc-edit-auto" type="checkbox" style="width:auto;min-height:auto" ${s.auto_apply ? 'checked' : ''} />
            Tự động ghi vào mọi đơn hàng mới
          </label>
          <div style="display:flex;gap:8px;margin-top:12px">
            <button id="sc-edit-save" class="btn btn-primary">Lưu</button>
            <button id="sc-edit-cancel">Huỷ</button>
          </div>
        `);
        overlay.querySelector('#sc-edit-type').addEventListener('change', function () {
          overlay.querySelector('#sc-edit-category-wrap').style.display = this.value === 'per_item' ? '' : 'none';
        });
        attachNumberInput(overlay.querySelector('#sc-edit-value'), { decimals: 0 });
        overlay.querySelector('#sc-edit-cancel').addEventListener('click', close);
        overlay.querySelector('#sc-edit-save').addEventListener('click', async () => {
          const vType = overlay.querySelector('#sc-edit-type').value;
          const payload = {
            name: overlay.querySelector('#sc-edit-name').value.trim(),
            code: overlay.querySelector('#sc-edit-code').value.trim(),
            value: parseNumberInput(overlay.querySelector('#sc-edit-value').value) || 0,
            value_type: vType,
            per_item_category: vType === 'per_item' ? overlay.querySelector('#sc-edit-category').value.trim() : null,
            applies_to: overlay.querySelector('#sc-edit-applies').value,
            auto_apply: overlay.querySelector('#sc-edit-auto').checked,
          };
          if (!payload.name) { toast('Nhập tên phụ thu', 'error'); return; }
          try {
            await api.patch(`/api/mgr/surcharges/${s.id}`, payload);
            toast('Đã lưu phụ thu');
            close();
            await loadSurcharges();
          } catch (err) { toast(err?.body?.message || 'Không lưu được', 'error'); }
        });
    ...
    attachNumberInput(body.querySelector('#sc-value'), { decimals: 0 });
    body.querySelector('#sc-add').addEventListener('click', async () => {
      const vType = body.querySelector('#sc-type').value;
      const payload = {
        name: body.querySelector('#sc-name').value.trim(),
        code: body.querySelector('#sc-code').value.trim(),
        value: parseNumberInput(body.querySelector('#sc-value').value) || 0,
        value_type: vType,
        per_item_category: vType === 'per_item' ? body.querySelector('#sc-category').value.trim() : null,
        applies_to: body.querySelector('#sc-applies').value,
  ...
  // ── In phiếu bếp (10/09/2026) ────────────────────────────────────────────
  function renderKitchenPrint() {
    const v = settings.kitchen_print || {};
    const methods = Array.isArray(v.notify_methods) ? v.notify_methods : [];
    const TRIGGER_OPTS = {
      order_submit: 'Ngay khi khách gửi đơn (POS web order)',
      confirm: 'Khi nhân viên bấm Xác nhận đơn (POS Manager)',
      off: 'Tắt (không tự động)',
    };
    const VARIABLES_HINT = `
      <details style="margin-top:8px">
        <summary style="cursor:pointer;font-size:13px;color:var(--text-3)">Biến hỗ trợ trong mẫu</summary>
        <ul style="font-size:12px;color:var(--text-3);margin-top:6px;padding-left:16px;line-height:1.9">
          <li><code>{hinh_thuc}</code> — Ăn tại bàn / Mang về / Giao hàng</li>
          <li><code>{hinh_thuc_upper}</code> — ĂN TẠI BÀN / MANG VỀ / GIAO HÀNG</li>
          <li><code>{ban_label}</code> — BÀN 05 (hoặc MANG VỀ)</li>
          <li><code>{ma_don}</code> — A-023</li>
          <li><code>{gio}</code> — 14:32</li>
          <li><code>{thoi_gian_in}</code> — 14:32:05</li>
          <li><code>{nguon}</code> — POS web / POS nhân viên</li>
          <li><code>{danh_sach_mon}</code> — danh sách món đã format</li>
          <li><code>{ghi_chu_don_line}</code> — GHI CHÚ: … hoặc rỗng</li>
          <li><code>{ten_quan}</code> — tên cửa hàng</li>
        </ul>
      </details>`;
    body.innerHTML = `
      <h4>A. Khi nào kích hoạt?</h4>
      <div class="sp-group">
        ${Object.entries(TRIGGER_OPTS).map(([v2, lbl]) => `
          <label class="sp-row">
            <span class="sp-text"><b>${escapeHtml(lbl)}</b></span>
            <input type="radio" data-f="trigger" value="${v2}" ${(v.trigger || 'off') === v2 ? 'checked' : ''} ${canManage ? '' : 'disabled'} />
          </label>`).join('')}
      </div>
      <h4>B. Phương thức thông báo bếp</h4>
      <div class="sp-group" id="kp-methods">
        <label class="sp-row">
          <span class="sp-text"><b>In tự động qua máy in nhiệt (ESC/POS 80mm)</b></span>
          <input type="checkbox" id="kp-m-print" ${methods.includes('print') ? 'checked' : ''} ${canManage ? '' : 'disabled'} />
        </label>
        <label class="sp-row">
          <span class="sp-text"><b>Gửi tin nhắn Telegram (theo nhóm đã cấu hình)</b></span>
          <input type="checkbox" id="kp-m-telegram" ${methods.includes('telegram') ? 'checked' : ''} ${canManage ? '' : 'disabled'} />
        </label>
        <label class="sp-row">
          <span class="sp-text"><b>Hiển thị trên màn Quản lý bếp</b><small>Luôn bật — không thể tắt</small></span>
          <input type="checkbox" checked disabled />
        </label>
      </div>
      <div id="kp-print-cfg" style="${methods.includes('print') ? '' : 'display:none'}">
        <h4>C. Cài đặt máy in nhiệt</h4>
        ${sel('print_protocol', 'Giao thức in', { browser: 'In qua trình duyệt (Ctrl+P)', escpos: 'USB/LAN ESC/POS' }, v.print_protocol || 'browser')}
        <div id="kp-addr-row" style="${(v.print_protocol || 'browser') === 'escpos' ? '' : 'display:none'}">
          ${txt('printer_address', 'Địa chỉ máy in', v.printer_address || '', { ph: '192.168.1.100:9100' })}
        </div>
        <button type="button" id="kp-test-print" class="btn" style="margin-top:4px">In thử</button>
      </div>
      <h4>D. Mẫu phiếu bếp</h4>
      <div class="field">
        <label>Mẫu phiếu <small>(textarea để chỉnh)</small></label>
        <textarea data-f="print_template" data-kind="text" rows="8"
          style="font-family:monospace;font-size:13px" ${canManage ? '' : 'readonly'}>${escapeHtml(v.print_template || '')}</textarea>
        ${VARIABLES_HINT}
      </div>
      ${saveBtn('Lưu cài đặt in phiếu bếp')}
    `;
    // Ẩn/hiện khu cài máy in khi tích/bỏ tích "In tự động"
    body.querySelector('#kp-m-print').addEventListener('change', (e) => {
      body.querySelector('#kp-print-cfg').style.display = e.target.checked ? '' : 'none';
    });
    // Ẩn/hiện ô địa chỉ khi đổi giao thức
    const protoSel = body.querySelector('[data-f="print_protocol"]');
    if (protoSel) protoSel.addEventListener('change', (e) => {
      body.querySelector('#kp-addr-row').style.display = e.target.value === 'escpos' ? '' : 'none';
    });
    // Nút In thử — mở print dialog với nội dung mẫu đơn giản
    const testPrintBtn = body.querySelector('#kp-test-print');
    if (testPrintBtn) testPrintBtn.addEventListener('click', () => {
      const tmpl = body.querySelector('[data-f="print_template"]')?.value || '';
      const preview = tmpl
        .replace('{hinh_thuc_upper}', 'ĂN TẠI BÀN')
        .replace('{hinh_thuc}', 'Ăn tại bàn')
        .replace('{ban_label}', 'BÀN 01')
        .replace('{ma_don}', 'T01-TEST')
        .replace('{gio}', new Date().toLocaleTimeString('vi-VN', { hour: '2-digit', minute: '2-digit' }))
        .replace('{thoi_gian_in}', new Date().toLocaleTimeString('vi-VN'))
        .replace('{nguon}', 'POS nhân viên')
        .replace('{danh_sach_mon}', '2x Cơm sườn nướng\n   ↳ Không hành\n1x Trà đá')
        .replace('{ghi_chu_don_line}', 'GHI CHÚ: Ăn chay')
        .replace('{ten_quan}', settings.store?.name || 'Cửa hàng');
      const w = window.open('', '_blank', 'width=400,height=600');
      if (w) {
        const pre = w.document.createElement('pre');
        pre.style.cssText = 'font-family:monospace;font-size:14px;white-space:pre-wrap;padding:16px';
    ...
    if (!canManage) return;
    const saveButton = body.querySelector('#cd-save');
    if (!saveButton) return;
    saveButton.addEventListener('click', async () => {
      const methods2 = [];
      if (body.querySelector('#kp-m-print')?.checked) methods2.push('print');
      if (body.querySelector('#kp-m-telegram')?.checked) methods2.push('telegram');
      const patch = { notify_methods: methods2 };
      body.querySelectorAll('[data-f]').forEach((elm) => {
        const k = elm.dataset.f;
        if (elm.type === 'radio') { if (elm.checked) patch[k] = elm.value; return; }
        if (elm.dataset.kind === 'text' || !elm.dataset.kind) patch[k] = elm.value.trim();
      });
      try {
        const res = await api.patch('/api/mgr/settings/kitchen_print', patch);
        settings.kitchen_print = res.value;
        toast('Đã lưu cài đặt in phiếu bếp');
      } catch (err) { toast(err?.body?.message || 'Không lưu được', 'error'); }
    });
  }

  // ── Nhóm Telegram (10/09/2026) ───────────────────────────────────────────
  function renderTelegramGroups() {
    // Mẫu tin TELEGRAM mặc định: emoji là NỘI DUNG tin gửi đi (không phải icon giao diện) nên giữ nguyên, nhưng
    // viết bằng escape để luật "giao diện không emoji" (test t60/t83 quét chữ trong file, kể cả comment) vẫn
    // quét sạch — xoá emoji đi là đổi nội dung tin nhân viên nhận. KHÔNG chép ký tự emoji vào comment ở đây.
    const DEFAULT_KITCHEN_TPL = '\u{1F373} {hinh_thuc_upper} - {ban_label}\nĐơn #{ma_don} | {gio}\n{danh_sach_mon}\n{ghi_chu_don_line}';
    const DEFAULT_ORDER_TPL = '\u{1F6D2} Đơn mới #{ma_don}\n{hinh_thuc} - {ban_label} | {gio}\n{danh_sach_mon}\n\u{1F4B0} Tổng: {tong_tien}đ\n{ghi_chu_don_line}';
    const DEFAULT_PAY_TPL = '\u{2705} Đã thanh toán #{ma_don}\n{hinh_thuc} - {ban_label}\n\u{1F4B0} {tong_tien}đ | {phuong_thuc_tt}';
    const DEFAULT_CANCEL_TPL = '\u{274C} Đơn #{ma_don} bị huỷ\n{hinh_thuc} - {ban_label}';
    const NOTIFY_TYPE_LABELS = {
      new_order: 'Đơn mới từ POS web order',
      payment:   'Xác nhận thanh toán',
      kitchen:   'Thông báo bếp (khi có đơn cần làm)',
      cancel:    'Đơn huỷ',
    };
    let groups = [];
    let editId = null;
    ...
    async function load() {
      try {
        const res = await api.get('/api/mgr/settings/telegram-groups');
        groups = res.groups || [];
      } catch (_) { groups = []; }
    }
    ...
    function drawList() {
      body.innerHTML = `
        <p class="hint">Mỗi nhóm Telegram có thể nhận một hoặc nhiều loại thông báo với template riêng.</p>
        <div class="set-list" id="tg-list">
          ${groups.length ? groups.map((g) => `
            <div class="set-item" style="flex-direction:column;align-items:flex-start;gap:6px;padding:12px 16px">
              <div style="display:flex;justify-content:space-between;width:100%">
                <b>${escapeHtml(g.name)}</b>
                <span style="display:flex;gap:8px">
                  <button class="btn btn-sm tg-edit" data-id="${g.id}">Sửa</button>
                  <button class="btn btn-sm tg-test" data-id="${g.id}">Gửi thử</button>
                  ${canManage ? `<button class="btn btn-sm tg-del" data-id="${g.id}" style="color:var(--money-out)">Xoá</button>` : ''}
                </span>
              </div>
              <small style="color:var(--text-3)">Chat ID: <code>${escapeHtml(String(g.chat_id))}</code></small>
              <small style="color:var(--text-3)">${(g.notify_types || []).map((t) => NOTIFY_TYPE_LABELS[t] || t).join(' · ') || 'Chưa chọn loại thông báo'}</small>
            </div>`).join('') : '<p class="hint" style="text-align:center;padding:20px">Chưa có nhóm nào. Bấm "+ Thêm nhóm" để thêm.</p>'}
        </div>
        ${canManage ? '<button class="btn btn-primary" id="tg-add" style="margin-top:12px">+ Thêm nhóm</button>' : ''}
      `;
      body.querySelector('#tg-add')?.addEventListener('click', () => { editId = null; drawForm({}); });
      body.querySelectorAll('.tg-edit').forEach((btn) => btn.addEventListener('click', () => {
        const g = groups.find((x) => x.id === btn.dataset.id);
        if (g) { editId = g.id; drawForm(g); }
      }));
      body.querySelectorAll('.tg-test').forEach((btn) => btn.addEventListener('click', async () => {
        btn.disabled = true; btn.textContent = '…';
        try {
          await api.post(`/api/mgr/settings/telegram-groups/${btn.dataset.id}/test`);
          toast('Đã gửi tin thử thành công');
        } catch (err) { toast(err?.body?.message || 'Gửi thất bại', 'error'); }
        btn.disabled = false; btn.textContent = 'Gửi thử';
      }));
      body.querySelectorAll('.tg-del').forEach((btn) => btn.addEventListener('click', async () => {
        if (!confirm('Xoá nhóm này?')) return;
        try {
          const res = await api.del(`/api/mgr/settings/telegram-groups/${btn.dataset.id}`);
          groups = res.groups;
          drawList();
          toast('Đã xoá nhóm');
        } catch (err) { toast(err?.body?.message || 'Xoá thất bại', 'error'); }
      }));
    }

    function drawForm(g) {
      const types = g.notify_types || [];
      const tpl = g.templates || {};
      body.innerHTML = `
        <a class="set-back" href="#" id="tg-back" style="text-decoration:none">${icon('quay-lai')} Danh sách nhóm</a>
        <h4>${editId ? 'Sửa nhóm' : 'Thêm nhóm mới'}</h4>
        <div class="field"><label>Tên nhóm</label>
          <input id="tg-name" type="text" value="${escapeHtml(g.name || '')}" placeholder="VD: Nhóm bếp" ${canManage ? '' : 'readonly'} /></div>
        <div class="field"><label>Chat ID</label>
          <input id="tg-chatid" type="text" value="${escapeHtml(String(g.chat_id || ''))}" placeholder="-1001234567890" ${canManage ? '' : 'readonly'} />
          <small class="hint">Cách lấy: Thêm bot vào nhóm Telegram → nhắn <code>/chatid</code> trong nhóm → bot trả về số → paste vào đây</small></div>
        <h4>Loại thông báo</h4>
        <div class="sp-group">
          ${Object.entries(NOTIFY_TYPE_LABELS).map(([k, lbl]) => `
            <label class="sp-row">
              <span class="sp-text"><b>${escapeHtml(lbl)}</b></span>
              <input type="checkbox" class="tg-type" data-type="${k}" ${types.includes(k) ? 'checked' : ''} ${canManage ? '' : 'disabled'} />
            </label>`).join('')}
        </div>
        <h4>Template tin nhắn</h4>
        <div class="field"><label>Template đơn mới</label>
          <textarea id="tg-tpl-new_order" rows="5" style="font-family:monospace;font-size:13px" ${canManage ? '' : 'readonly'}>${escapeHtml(tpl.new_order || DEFAULT_ORDER_TPL)}</textarea></div>
        <div class="field"><label>Template thông báo bếp</label>
          <textarea id="tg-tpl-kitchen" rows="5" style="font-family:monospace;font-size:13px" ${canManage ? '' : 'readonly'}>${escapeHtml(tpl.kitchen || DEFAULT_KITCHEN_TPL)}</textarea></div>
        <div class="field"><label>Template xác nhận thanh toán</label>
          <textarea id="tg-tpl-payment" rows="4" style="font-family:monospace;font-size:13px" ${canManage ? '' : 'readonly'}>${escapeHtml(tpl.payment || DEFAULT_PAY_TPL)}</textarea></div>
        <div class="field"><label>Template đơn huỷ</label>
          <textarea id="tg-tpl-cancel" rows="3" style="font-family:monospace;font-size:13px" ${canManage ? '' : 'readonly'}>${escapeHtml(tpl.cancel || DEFAULT_CANCEL_TPL)}</textarea></div>
        <details style="margin:8px 0 12px"><summary style="cursor:pointer;font-size:13px;color:var(--text-3)">Biến hỗ trợ</summary>
          <ul style="font-size:12px;color:var(--text-3);margin-top:6px;padding-left:16px;line-height:1.9">
            <li><code>{hinh_thuc}</code>, <code>{hinh_thuc_upper}</code>, <code>{ban_label}</code>, <code>{ma_don}</code></li>
            <li><code>{gio}</code>, <code>{danh_sach_mon}</code>, <code>{ghi_chu_don_line}</code></li>
            <li><code>{tong_tien}</code> — tổng tiền có dấu phẩy</li>
            <li><code>{phuong_thuc_tt}</code> — Tiền mặt / Chuyển khoản / Khác</li>
          </ul></details>
        ${canManage ? `<div class="tables-top-actions" style="margin-top:12px">
          <button id="tg-save" class="btn btn-primary">${editId ? 'Lưu thay đổi' : 'Thêm nhóm'}</button>
          <button id="tg-cancel-btn">Huỷ</button>
        </div>` : ''}
      `;
      body.querySelector('#tg-back').addEventListener('click', (e) => { e.preventDefault(); drawList(); });
      body.querySelector('#tg-cancel-btn')?.addEventListener('click', () => drawList());
      body.querySelector('#tg-save')?.addEventListener('click', async () => {
        const name = body.querySelector('#tg-name')?.value.trim();
        const chat_id = body.querySelector('#tg-chatid')?.value.trim();
        if (!name || !chat_id) return toast('Thiếu tên nhóm hoặc Chat ID', 'error');
        const notify_types = [...body.querySelectorAll('.tg-type:checked')].map((cb) => cb.dataset.type);
        const templates = {
          new_order: body.querySelector('#tg-tpl-new_order')?.value || DEFAULT_ORDER_TPL,
          kitchen:   body.querySelector('#tg-tpl-kitchen')?.value || DEFAULT_KITCHEN_TPL,
          payment:   body.querySelector('#tg-tpl-payment')?.value || DEFAULT_PAY_TPL,
          cancel:    body.querySelector('#tg-tpl-cancel')?.value || DEFAULT_CANCEL_TPL,
        };
        try {
          let res;
          if (editId) {
            res = await api.put(`/api/mgr/settings/telegram-groups/${editId}`, { name, chat_id, notify_types, templates });
          } else {
            res = await api.post('/api/mgr/settings/telegram-groups', { name, chat_id, notify_types, templates });
          }
          groups = res.groups;
          // Chọn câu toast TRƯỚC khi xoá editId — gán null rồi mới hỏi `editId ?` thì sửa nhóm cũng báo "Đã thêm nhóm".
          const msg = editId ? 'Đã cập nhật nhóm' : 'Đã thêm nhóm';
          editId = null;
          drawList();
          toast(msg);
        } catch (err) { toast(err?.body?.message || 'Không lưu được', 'error'); }
    ...
    // return: để render() chờ vẽ xong (trước đây bỏ lửng → khung rỗng trong lúc chờ, và đổi tab nhanh thì
    // drawList() muộn có thể ghi đè lên màn khác).
    return load().then(() => drawList());
  }
