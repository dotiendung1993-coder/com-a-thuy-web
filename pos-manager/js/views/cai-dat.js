...
export async function render(container, { staff, params } = {}) {
  ...
  else if (sub === 'nhom-telegram') renderTelegramGroups();
  ...
  function renderInvoiceForm() {
    ...
    function customSeparators() {
      return (on('custom_separators') || '').split('\n').filter(Boolean).map((s) => {
        const i = s.indexOf(':');
    ...
    function buildInvoiceBody() {
      ...
      let orderLeft = `<div>🥡 Mang về</div><div>Mã đơn: MV0001</div><div>${nowStr}</div>`;
      ...
      ${canManage ? `
        ...
        <div class="field"><label>Loại &amp; Giá trị</label>
          <div style="display:flex;gap:6px">
            ...
            <input id="sc-value" type="number" min="0" value="0" style="flex:1" />
          </div>
        </div>
      ...
      btn.addEventListener('click', () => {
        ...
        const { overlay, close } = openModal(`
          ...
          <div class="field"><label>Loại &amp; Giá trị</label>
            <div style="display:flex;gap:6px">
              ...
              <input id="sc-edit-value" type="number" min="0" value="${s.value}" style="flex:1" />
            </div>
          </div>
        ...
        overlay.querySelector('#sc-edit-save').addEventListener('click', async () => {
          ...
          const payload = {
            ...
            value: Number(overlay.querySelector('#sc-edit-value').value) || 0,
            value_type: vType,
  ...
  function renderTelegramGroups() {
    const DEFAULT_KITCHEN_TPL = '🍳 {hinh_thuc_upper} - {ban_label}\nĐơn #{ma_don} | {gio}\n{danh_sach_mon}\n{ghi_chu_don_line}';
    const DEFAULT_ORDER_TPL = '🛒 Đơn mới #{ma_don}\n{hinh_thuc} - {ban_label} | {gio}\n{danh_sach_mon}\n💰 Tổng: {tong_tien}đ\n{ghi_chu_don_line}';
    const DEFAULT_PAY_TPL = '✅ Đã thanh toán #{ma_don}\n{hinh_thuc} - {ban_label}\n💰 {tong_tien}đ | {phuong_thuc_tt}';
    const DEFAULT_CANCEL_TPL = '❌ Đơn #{ma_don} bị huỷ\n{hinh_thuc} - {ban_label}';
    ...
    function drawList() {
      ...
      body.querySelectorAll('.tg-del').forEach((btn) => btn.addEventListener('click', async () => {
        ...
        try {
          const res = await api.delete(`/api/mgr/settings/telegram-groups/${btn.dataset.id}`);
          groups = res.groups;
          drawList();
          toast('Đã xoá nhóm');
    ...
    function drawForm(g) {
      ...
      body.querySelector('#tg-save')?.addEventListener('click', async () => {
        ...
        try {
          ...
          editId = null;
          drawList();
          toast(editId ? 'Đã cập nhật nhóm' : 'Đã thêm nhóm');
        } catch (err) { toast(err?.body?.message || 'Không lưu được', 'error'); }
      });
    }
