...
import { formatVND, escapeHtml, toast, openModal, todayVN, dateVN, monthStartVN, pageTabsHtml, formatNumberInput, parseNumberInput, attachNumberInput } from '../ui.js';
...
export async function render(container, { staff } = {}) {
  ...
  function openConfigModal() {
    ...
    const modal = openModal(`
      ...
      <div class="field"><label>Thuế suất GTGT (%)</label><input id="ts-vat" type="text" inputmode="decimal" value="${formatNumberInput(s.vat_rate, { decimals: 1 })}" /></div>
      <div class="field"><label>Thuế suất TNCN (%)</label><input id="ts-pit" type="text" inputmode="decimal" value="${formatNumberInput(s.pit_rate, { decimals: 1 })}" /></div>
      <div class="field"><label>Ngưỡng doanh thu miễn thuế/năm</label><input id="ts-threshold" type="text" inputmode="numeric" value="${formatNumberInput(s.exempt_threshold_year)}" /></div>
    ...
    `);
    attachNumberInput(modal.overlay.querySelector('#ts-vat'), { decimals: 1 });
    attachNumberInput(modal.overlay.querySelector('#ts-pit'), { decimals: 1 });
    attachNumberInput(modal.overlay.querySelector('#ts-threshold'), { decimals: 0 });
    modal.overlay.querySelector('#ts-submit').addEventListener('click', async () => {
      const payload = {
        ...
        vat_rate: parseNumberInput(modal.overlay.querySelector('#ts-vat').value),
        pit_rate: parseNumberInput(modal.overlay.querySelector('#ts-pit').value),
        exempt_threshold_year: parseNumberInput(modal.overlay.querySelector('#ts-threshold').value),
