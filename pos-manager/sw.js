...
const CACHE_NAME = 'posmgr-shell-v116'; // v116: 24/09 — nâng lại số bản vì v115 đã kịp phát hành một bản sell.js TRUNG GIAN (mới có console.error, chưa sửa lỗi bên dưới). // v115: 24/09 — sell.js: "Lưu đơn (F2)" tạo đơn thật xong vẫn hiện toast đỏ "Không tạo được đơn" (thu ngân tưởng chưa lưu → dễ bấm lại thành đơn trùng): doCheckout() thay innerHTML nút Thanh toán làm mất #cart-pay-amount, clearCart() ghi vào phần tử null → TypeError bị catch báo nhầm; nay chỉ đổi chữ của <b>. Catch cũng ghi console.error (trước nuốt im lặng). // v114: 24/09 — (1) Đơn hàng (orders.js): thu ngân/quản lý mở màn lần nào cũng thấy toast đỏ "Bạn không có quyền…" vì màn gọi GET /api/mgr/staff (chỉ chủ quán) rồi để 403 rơi vào catch — api.js toast mọi 403; nay chỉ gọi khi là chủ quán. (2) Quản lý bếp (kitchen.js): không có quyền bếp mà vẫn dựng màn → polling 403 lặp lại kèm toast + hiện "Không có món chờ chế biến" gây hiểu lầm; nay báo "Bạn không có quyền xem màn hình này". // v113: 24/09 — sửa màn Tích điểm (tich-diem.js) hỏng hoàn toàn: đọc data.rows trong khi API trả entries → luôn hiện "Không tải được sổ tích điểm"; gửi sai tên tham số (limit/customer_id thay vì pageSize/customerId), form Cộng/Đổi điểm nay chạy bằng SĐT (server nhận phone), toast dùng đúng r.points / r.awarded; bộ lọc Tích/Đổi + ô tìm tên/SĐT nay có tác dụng (server thêm direction/q). // v112: 23/09 — sửa 9 file test fail có sẵn + 3 bug thật phía máy nhân viên: (1) cai-dat.js: nút Xoá nhóm Telegram gọi api.delete (không tồn tại → luôn báo "Xoá thất bại", bug-566 tái phát lần 3) nay api.del; customSeparators() chịu được giá trị mảng (màn Mẫu hoá đơn không sập khi không thấy input ẩn); view Nhóm Telegram được await (render() chờ vẽ xong, hết khung rỗng/ghi đè màn khác); (2) luật "giao diện không emoji": sell.js nút/nhãn sửa phụ thu → icon SVG, doi-chieu-von.js banner/cột Đạt-Vượt → icon SVG, khuyen-mai.js + cai-dat.js dấu đóng → ×, mẫu tin Telegram mặc định + dòng "Mang về" xem trước hoá đơn viết bằng escape \u{...} (giữ NGUYÊN nội dung). // v111: 23/09 — QA toàn diện giá nhiều mức (TASK-qa-toan-dien-gia-nhieu-muc.md): (1) hậu tố tên món khi lên đơn đổi từ "(mâm <size>)" cứng sang "(<size>)" trung lập, đồng bộ order-service.js/pos-server.js×2/booking-modal.js/order.html×2; (2) sync-sheets.js thêm guard isStandardMamSizeTiers() bỏ qua đẩy/kéo Sheet cho món Cơm mâm cấu trúc mức giá không khớp đúng 4 mức XS/S/M/L chuẩn (giữ nguyên CSDL, log cảnh báo); (3) order.html (3 chỗ) + booking-modal.js (1 chỗ) hiện nút chọn size trước đây lọc CỨNG theo mảng MAM_SIZES=[S,M,L] và category==='Cơm mâm' — món tên mức tự do (vd "Suất nhỏ") hoặc category khác bị mất hẳn nút chọn size, rơi về hiện như món 1 giá; nay đọc size_order/size_prices động theo hàm dùng chung sizeTiersOf()/tương đương, áp dụng mọi category; (4) route GET /api/menu (pos-server.js, dùng chung cho order.html + booking-modal.js) thiếu cột size_order trong SELECT — bổ sung; (5) migration 096 backfill size_order cho 48/49 món Cơm mâm cũ tạo trước migration 095 (trước đó rơi về Object.keys(size_prices) bị Postgres sắp sai thành L,M,S,XS thay vì XS,S,M,L). // v110: 23/09 — fix bug tự phát hiện lúc kiểm tay v109: bấm "Nhiều mức giá" không hiện khối nhập mức (đã viết `#sp-price-tiers-wrap` với thuộc tính HTML `hidden` nhưng lại đổi bằng `classList.toggle('hidden',…)` — 2 cơ chế khác nhau, đổi class không có tác dụng khi hiển thị đang do THUỘC TÍNH điều khiển; sửa thành gán thẳng `.hidden = single`). // v109: 23/09 — Task 1: Hàng hoá > Sản phẩm, mở Công thức/Đơn vị quy đổi/Phân loại từ form Sửa món KHÔNG còn đóng form cha trước (cùng họ bug-655) — cửa sổ con mở CHỒNG LÊN (z-index 60, mở sau nên nổi trên), đóng cửa sổ con là form còn nguyên mọi ô đang gõ dở; lưu công thức xong form tự cập nhật lại (ô Giá vốn khoá/mở, nhãn, nút Sửa/Thêm công thức) không cần đóng-mở lại (san-pham.js::applyRecipeState). Task 2: giá bán nhiều mức (size/suất/đĩa) — form Tạo/Sửa sản phẩm thêm "Kiểu giá bán" (Một giá/Nhiều mức), thêm/sửa/xoá/đổi thứ tự từng mức (tên tự do, nút điền nhanh XS/S/M/L), lưu vào size_prices (đã có) + size_order MỚI (migration 095, giữ thứ tự — jsonb object không giữ được); menu.price luôn = mức thấp nhất; danh sách Sản phẩm hiện khoảng giá "thấp–cao" (priceCell()); màn Bán hàng (sell.js) hiện nút chọn size đúng thứ tự size_order. // v108: 22/09 — Định dạng số cho MỌI ô nhập số (TASK-dinh-dang-so.md): đổi ~68 ô type=number (tiền/số lượng) sang type=text+inputmode kèm dấu chấm ngăn hàng nghìn, dấu phẩy phần lẻ (js/ui.js thêm formatNumberInput/parseNumberInput/liveFormatNumberInput/attachNumberInput, giữ vị trí con trỏ lúc gõ); rà lại toàn bộ nơi đọc giá trị (Number/parseFloat/parseInt/FormData) sang parseNumberInput ở 24 file view; loại trừ SĐT/PIN/mã vạch/mã bàn/OTP. // v105: 22/09 — Hàng hoá > Sản phẩm: mục Công thức ngay trong form sửa món (xem/thêm/sửa/xoá dòng NVL, tổng giá vốn, cảnh báo NVL chưa có giá); tạo nhanh NVL ngay lúc soạn công thức (js/recipe-shared.js MỚI, dùng chung với màn Công thức); giá vốn món CÓ công thức nay TỰ TÍNH từ công thức ở backend nên ô Giá vốn bị khoá (form sửa món + màn Giá vốn). // v104: 21/09 — bug-655: Hàng hoá > Sản phẩm, bấm Sửa ở bảng chi tiết bên phải lại mở form "Tạo sản phẩm mới" trống. closeDetail() xoá detailProduct = null nên openForm(detailProduct) gọi sau đó nhận null; giữ tham chiếu vào biến cục bộ trước khi đóng bảng (san-pham.js). // v103: 19/09 — Task 2: Chat Center chuyển từ mã PIN riêng sang dùng CHUNG tài khoản/phiên đăng nhập POS Manager — hoi-thoai.js bỏ hẳn màn nhập PIN, tự lấy token phiên hiện có qua GET /api/mgr/auth/chat-center-token rồi nhúng vào URL iframe (?t=...), không cần đăng nhập lần 2 (backend: sales-bot/access.js::resolveUserByToken thay resolveUserByPin, /admin/api/login+forgot-password mới cho Chat Center chạy độc lập PWA). // v102: 19/09 — fix 2 bug màn Hội thoại: (1) flicker modal PIN khi sai mã — dùng module-level handler tránh tích luỹ, ẩn iframe hiện "Đang kết nối…" đến khi nhận cc-auth-ok từ Chat Center; (2) search overlay Ctrl+K đè lên modal PIN — đóng search khi đổi route, bỏ qua Ctrl+K khi focus đang trong ô password. // v101: 18/09 — fix bug tìm sản phẩm trong "Tạo công thức" gõ chữ bị ĐẢO NGƯỢC: rebuildModal() thay innerHTML mỗi lần gõ huỷ+dựng lại input, focus() đặt con trỏ về đầu -> ký tự sau chèn ngược. Ép setSelectionRange về cuối chuỗi sau focus() (cong-thuc.js). // v100: 18/09 — Báo cáo > Đối chiếu vốn NVL (doi-chieu-von.js MỚI, độc lập với gia-von.js): giá vốn tạm tính theo XUẤT (qua công thức) tách riêng theo món/size, banner cảnh báo % công thức chưa khai, đối chiếu với Vốn mục tiêu tự nhập. // v99: 18/09 — Task 2: Đối tác > Hội thoại nhúng Chat Center iframe thật (hoi-thoai.js), PIN lưu sessionStorage, postMessage khi sai PIN (chat-center.html). // v98: 15/09 — fix bug: Cài đặt > Quản lý vận chuyển (và 4 màn khác: Xoá cửa hàng, Hoá đơn điện tử, Website > Cài đặt Website, Mẫu hoá đơn > Thiết lập in) chữ dính sát mép thẻ — .sp-group không tự đệm, .field/.hint đặt trực tiếp trong .sp-group thiếu padding ngang. Thêm rule dùng chung `.sp-group > .field, .sp-group > .hint` (app.css) + bỏ 12 inline style padding-left:8px thừa/lệch trong renderInvoiceForm (cai-dat.js). // v97: 13/09 — fix bug-645: ô tìm kiếm header nhấp nháy không hiện ra — cú bấm MỞ modal (lúc còn nhỏ) bị chính handler "bấm ra ngoài thì đóng" của modal vừa mở tự đóng ngay do reflow đổi target giữa mousedown/click. Ghi nhận trạng thái mở/đóng lúc mousedown thay vì suy luận qua target lúc click. // v96: 13/09 — Task 1: confirmDialog() thay confirm() native (delete surcharge), sửa modal cho nút Sửa phụ thu (thay inline edit), thêm per_item value_type + per_item_category field. Task 2: sell.js per_item popup qty khi tick, ✏️ icon override giá trị/số suất tạm cho đơn, gộp phí đóng gói vào per_item surcharge (bỏ packagingFee riêng). // v95: 12/09 — Task 1: STK/ngân hàng QR chuyển khoản THẬT nay đọc từ Cài đặt > Mẫu hoá đơn (trước chỉ đổi khung xem trước), cảnh báo rõ trong ghi chú khi đổi. Task 2a: thêm ô nhập Phí đóng gói ngay trong màn Quản lý phụ thu (POS Manager là nơi lưu data gốc, không cần sửa Google Sheet). // v94: 12/09 — fix bug-643 TDZ: const APPLIES_TO khai ngoài renderSurcharges() bị gọi sớm qua nhánh early-return, làm "Quản lý phụ thu" báo lỗi "Không tải được" dù API 200 // v93: 12/09 — Task 3 Mẫu hoá đơn: preview sticky cuộn theo, thêm STK + tên chủ TK (khung xem trước/In thử — KHÔNG đổi QR thanh toán thật) // v92: 12/09 — Task 2 Quản lý phụ thu: applies_to lọc theo loại đơn (migration 092), nút Sửa, fix api.delete->api.del (bug-566 tái phát), nối vào POS web order (pos-server.js + order.html) // v91: 11/09 — Task 1 Mẫu hoá đơn: fix cỡ chữ preview (px->em, invoiceBoxSize trả px thật), SĐT riêng hoá đơn (store_phone_text), dòng phân cách tuỳ ý (custom_separators) // v90: 11/09 — Quản lý phụ thu: thêm nút Xoá (DELETE API + UI + confirm dialog) // v89: 11/09 — giữ vị trí scroll sidebar khi đổi tab cài đặt // v88: 11/09 — fix ReferenceError storeInfo undefined trong renderInvoiceForm() // v87: 11/09 — Cài đặt hoá đơn: thêm ô nhập tên cửa hàng override, gộp 2 dòng địa chỉ thành 1 textarea đa dòng (xuống dòng ở đâu in đúng ở đó) // v86: 10/09 — Task 3: logo hoá đơn tải lên thật (base64, tự thu nhỏ), khối mã đơn/giờ/loại đơn luôn hiện đúng mẫu thật, chọn kiểu gạch ngang // v85: 10/09 — Task 2: bỏ nút "Đổi giao diện" trùng ở header user-menu, gộp về 1 nút icon-only cạnh "Cài đặt" trong sidebar (giống v1) ở cả 2 giao diện // v84: 10/09 — Cân bằng 2 cột Cài đặt, nút Đổi giao diện trên header, màn In hoá đơn mới (preview + In thử) // v83: 10/09 — In phiếu bếp + Nhóm Telegram (2 mục mới Cài đặt) // v75: 04/09 — Đơn hàng Thao tác thêm 2 mục, Thu chi Thao tác button + Loại giao dịch, Sổ quỹ Loại giao dịch. // v73: 04/09 — thư mục 5 Sản phẩm: KPI dải chung, hàng lọc 1 hàng, tab viên thuốc, ảnh SP, cột sắp xếp. Trước đó v72: 04/09 — Việc 'Website v2' thư mục 4 Đơn hàng: hàng lọc gọn + hộp thoại Bộ lọc, KPI 1 thẻ, tab viên thuốc, phân trang, menu Thao tác (orders.js, icons.js, app.css, theme-v2.css).
